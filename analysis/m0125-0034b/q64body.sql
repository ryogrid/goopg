...
cross_sales as
 (select i_item_sk item_sk ,s_store_name store_name ,s_zip store_zip
     ,d1.d_year as syear ,d2.d_year as fsyear ,d3.d_year s2year ,count(*) cnt
  ...
  WHERE ss_store_sk = s_store_sk AND ss_sold_date_sk = d1.d_date_sk AND
        ...
        hd1.hd_income_band_sk = ib1.ib_income_band_sk and hd2.hd_income_band_sk = ib2.ib_income_band_sk and
        cd1.cd_marital_status <> cd2.cd_marital_status and
        i_color in ('orange','lace','lawn','misty','blush','pink') and
        i_current_price between 48 and 48 + 10 and i_current_price between 48 + 1 and 48 + 15
  group by i_item_sk ,s_store_name ,s_zip ,d1.d_year ,d2.d_year ,d3.d_year)
select syear, count(*) as groups, sum(cnt) as rows_in
from cross_sales group by syear order by syear;
