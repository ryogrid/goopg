...
for arm in head cand head cand; do
  B=$R/tmp/m0145-0008t/goopg-$arm; D=$R/tmp/m0145-0008t/data-$arm
  rm -rf $D; $B init -D $D >/dev/null 2>&1
  GOOPG_CG_UNIT=m0145-0008t-$arm $R/scripts/goopg-test-run.sh $B start -D $D --listen 127.0.0.1:5534 > $R/tmp/m0145-0008t/srv-$arm.log 2>&1 &
...
' ' ')
  ...
  systemctl --user stop m0145-0008t-$arm.scope >/dev/null 2>&1; systemctl --user reset-failed m0145-0008t-$arm.scope >/dev/null 2>&1
  rm -rf $D; sleep 2
done
