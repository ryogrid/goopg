...
D=$R/tmp/m0145-0008u/data
for arm in head cand head cand; do
  B=$R/tmp/m0145-0008u/goopg-$arm
  GOOPG_CG_UNIT=m0145-0008u-$arm $R/scripts/goopg-test-run.sh $B start -D $D --listen 127.0.0.1:5534 --hba $D/pg_hba.conf > $R/tmp/m0145-0008u/srv-$arm.log 2>&1 &
...
SQL
  ...
  systemctl --user stop m0145-0008u-$arm.scope >/dev/null 2>&1; systemctl --user reset-failed m0145-0008u-$arm.scope >/dev/null 2>&1
  sleep 2
done
