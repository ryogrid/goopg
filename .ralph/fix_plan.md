...
immediately after M-NIGHTLY's regression fixes. M0132 and M0133 are COMPLETE,
...
189 cases, one task each: 87 `failed` (M0134-0001..0087, CSV order) then 102
`not-tried` (M0134-0088..0189, CSV order). Per-case gate:
...
- [ ] **M0134-0009 — create_procedure.sql** — regress-sql `failed`: make the case match PG 18.3 (normalise against `./postgres/`). On pass, flip the CSV row to `pass` / `pass_required=yes` in the same commit.
- [ ] **M0134-0010 — create_table.sql** — regress-sql `failed`: make the case match PG 18.3 (normalise against `./postgres/`). On pass, flip the CSV row to `pass` / `pass_required=yes` in the same commit.
- [ ] **M0134-0011 — create_table_like.sql** — regress-sql `failed`: make the case match PG 18.3 (normalise against `./postgres/`). On pass, flip the CSV row to `pass` / `pass_required=yes` in the same commit.
- [ ] **M0134-0012 — create_view.sql** — regress-sql `failed`: make the case match PG 18.3 (normalise against `./postgres/`). On pass, flip the CSV row to `pass` / `pass_required=yes` in the same commit.
- [ ] **M0134-0013 — date.sql** — regress-sql `failed`: make the case match PG 18.3 (normalise against `./postgres/`). On pass, flip the CSV row to `pass` / `pass_required=yes` in the same commit.
- [ ] **M0134-0014 — domain.sql** — regress-sql `failed`: make the case match PG 18.3 (normalise against `./postgres/`). On pass, flip the CSV row to `pass` / `pass_required=yes` in the same commit.
- [ ] **M0134-0015 — drop_if_exists.sql** — regress-sql `failed`: make the case match PG 18.3 (normalise against `./postgres/`). On pass, flip the CSV row to `pass` / `pass_required=yes` in the same commit.
- [ ] **M0134-0016 — equivclass.sql** — regress-sql `failed`: make the case match PG 18.3 (normalise against `./postgres/`). On pass, flip the CSV row to `pass` / `pass_required=yes` in the same commit.
- [ ] **M0134-0017 — explain.sql** — regress-sql `failed`: make the case match PG 18.3 (normalise against `./postgres/`). On pass, flip the CSV row to `pass` / `pass_required=yes` in the same commit.
- [ ] **M0134-0018 — expressions.sql** — regress-sql `failed`: make the case match PG 18.3 (normalise against `./postgres/`). On pass, flip the CSV row to `pass` / `pass_required=yes` in the same commit.
- [ ] **M0134-0019 — fast_default.sql** — regress-sql `failed`: make the case match PG 18.3 (normalise against `./postgres/`). On pass, flip the CSV row to `pass` / `pass_required=yes` in the same commit.
- [ ] **M0134-0020 — float4.sql** — regress-sql `failed`: make the case match PG 18.3 (normalise against `./postgres/`). On pass, flip the CSV row to `pass` / `pass_required=yes` in the same commit.
- [ ] **M0134-0021 — float8.sql** — regress-sql `failed`: make the case match PG 18.3 (normalise against `./postgres/`). On pass, flip the CSV row to `pass` / `pass_required=yes` in the same commit.
- [ ] **M0134-0022 — foreign_key.sql** — regress-sql `failed`: make the case match PG 18.3 (normalise against `./postgres/`). On pass, flip the CSV row to `pass` / `pass_required=yes` in the same commit.
- [ ] **M0134-0023 — generated_stored.sql** — regress-sql `failed`: make the case match PG 18.3 (normalise against `./postgres/`). On pass, flip the CSV row to `pass` / `pass_required=yes` in the same commit.
...
- [ ] **M0134-0027 — hash_index.sql** — regress-sql `failed`: make the case match PG 18.3 (normalise against `./postgres/`). On pass, flip the CSV row to `pass` / `pass_required=yes` in the same commit.
...
- [ ] **M0134-0031 — indexing.sql** — regress-sql `failed`: make the case match PG 18.3 (normalise against `./postgres/`). On pass, flip the CSV row to `pass` / `pass_required=yes` in the same commit.
...
- [ ] **M0134-0033 — insert.sql** — regress-sql `failed`: make the case match PG 18.3 (normalise against `./postgres/`). On pass, flip the CSV row to `pass` / `pass_required=yes` in the same commit.
...
- [ ] **M0134-0036 — join.sql** — regress-sql `failed`: make the case match PG 18.3 (normalise against `./postgres/`). On pass, flip the CSV row to `pass` / `pass_required=yes` in the same commit.
...
- [ ] **M0134-0048 — mvcc.sql** — regress-sql `failed`: make the case match PG 18.3 (normalise against `./postgres/`). Re-verify at HEAD first (possible regression); if it already passes, flip the CSV row to `pass` with a "stale — already fixed" note instead of implementing.
...
- [ ] **M0134-0066 — select_having.sql** — regress-sql `failed`: make the case match PG 18.3 (normalise against `./postgres/`). Re-verify at HEAD first (possible regression); if it already passes, flip the CSV row to `pass` with a "stale — already fixed" note instead of implementing.
- [ ] **M0134-0067 — select_implicit.sql** — regress-sql `failed`: make the case match PG 18.3 (normalise against `./postgres/`). Re-verify at HEAD first (possible regression); if it already passes, flip the CSV row to `pass` with a "stale — already fixed" note instead of implementing.
- [ ] **M0134-0068 — select_views.sql** — regress-sql `failed`: make the case match PG 18.3 (normalise against `./postgres/`). On pass, flip the CSV row to `pass` / `pass_required=yes` in the same commit.
...
- [ ] **M0134-0071 — subselect.sql** — regress-sql `failed`: make the case match PG 18.3 (normalise against `./postgres/`). On pass, flip the CSV row to `pass` / `pass_required=yes` in the same commit.
...
- [ ] **M0134-0082 — update.sql** — regress-sql `failed`: make the case match PG 18.3 (normalise against `./postgres/`). On pass, flip the CSV row to `pass` / `pass_required=yes` in the same commit.
...
- [ ] **M0134-0084 — vacuum.sql** — regress-sql `failed`: make the case match PG 18.3 (normalise against `./postgres/`). On pass, flip the CSV row to `pass` / `pass_required=yes` in the same commit.
- [ ] **M0134-0085 — window.sql** — regress-sql `failed`: make the case match PG 18.3 (normalise against `./postgres/`). On pass, flip the CSV row to `pass` / `pass_required=yes` in the same commit.
...
- [ ] **M0134-0153 — predicate.sql** — regress-sql `not-tried`: make the case match PG 18.3 (normalise against `./postgres/`). Run the case, fix the divergence; on pass, flip the CSV row to `pass` / `pass_required=yes` in the same commit.
...
- [ ] **M0134-0166 — select_parallel.sql** — regress-sql `not-tried`: make the case match PG 18.3 (normalise against `./postgres/`). Run the case, fix the divergence; on pass, flip the CSV row to `pass` / `pass_required=yes` in the same commit.
...
- [ ] **M0134-0171 — stats.sql** — regress-sql `not-tried`: make the case match PG 18.3 (normalise against `./postgres/`). Run the case, fix the divergence; on pass, flip the CSV row to `pass` / `pass_required=yes` in the same commit.
...
- [ ] **M0134-0187 — write_parallel.sql** — regress-sql `not-tried`: make the case match PG 18.3 (normalise against `./postgres/`). Run the case, fix the divergence; on pass, flip the CSV row to `pass` / `pass_required=yes` in the same commit.
