...
Milestone: `docs/milestones/0132-extended-protocol-explicit-transactions.md`.
Authoritative decomposition: `docs/design/0132-extended-protocol-explicit-transactions.md`.
Core design: `docs/design/0132-0001-extended-protocol-explicit-txn-state-machine.md`.
Supersedes the `design`-status sketch `analysis/perf-optimize3-dash/08-improvement-designs/09-extended-protocol-explicit-txn.md` (2026-07-14) and discharges the deferral-ledger row of that date ("doc 09 extended-protocol explicit-txn", deferred because "a partial land would silently skip INITIALLY DEFERRED constraints").

**The bug, verified at HEAD `28de7c44` (2026-08-12).** goopg's extended query protocol (Parse/Bind/Execute) begins and commits its own transaction on **every** `Execute`, and treats the client's `BEGIN`/`COMMIT`/`ROLLBACK` as accepted-but-ignored command tags — `dispatch_extended.go:110-112` returns a bare `CommandTag` for a `*planner.Transaction` node, `:134-139` unconditionally begins a fresh READ COMMITTED transaction on an offset proc slot, `:361-365` unconditionally commits it. `connTx *connTxState` is already threaded into `executeExtendedQueryViaExecutor` (`:30`) but is read only for `NonSuperuserRole` and statement logging; `connTx.Begin`/`InExplicit`/`Tx`/`End` have **zero** call sites in `dispatch_extended.go` or `extended.go`. The simple-query path does the real thing (`dispatch.go:227-229` reuses `connTx.Tx()`; `:2696-2984` is the block state machine), so this is a sibling-path divergence, not a missing feature.

**Why it is a correctness milestone, not a perf one.** `ROLLBACK` over the extended protocol **does not roll back** — a `BEGIN` → `UPDATE` → `UPDATE` → `ROLLBACK` sequence has already durably committed both updates when the `ROLLBACK` arrives, which returns a success tag. Every driver that implements `conn.rollback()` on the extended protocol (JDBC, `pgx`, `psycopg3`, most ORMs — i.e. the default path) is exposed. Atomicity across statements is lost (a failure at statement *k* leaves `1..k-1` committed); `ReadyForQuery` reports `I` where PG reports `T` (the status byte comes from `connTxState.wireStatus`, `conn_tx.go:415`, installed at `server.go:1526` — an *observable protocol* divergence drivers gate on); and isolation levels above READ COMMITTED are unreachable (`:139` hardcodes RC; the comment at `:119-123` says SERIALIZABLE "cannot apply"). The doc-09 perf finding stands as an acceptance gate: `-N -M prepared` at scale 100 measured 6,749 TPS vs 9,898 simple-mode, with the commit displaced from `END` (3.263 → 0.247 ms) onto each write (UPDATE 1.022 → 3.283, INSERT 0.279 → 3.331) = two fsyncs per transaction instead of one.

**LAND-TOGETHER RULE (non-negotiable).** S2+S3 without S4 is *worse than HEAD*: today each statement commits individually so its constraints are checked individually, whereas a block model that reaches COMMIT without `transactionOp.execCommit`'s deferred pass would commit a whole block violating an `INITIALLY DEFERRED` constraint. S2, S3 and S4 land in ONE commit. This is the risk the 2026-07-14 ledger row recorded.

- [ ] **M0132-S1 — Verification + characterisation tests (no behaviour change)** (est ~1 loop). Confirm and record the two doc-09 corrections: (a) its S1 "thread `connTxState` into `executeExtendedQueryViaExecutor`" is **already done** (the parameter exists at `dispatch_extended.go:30`), so the first *code* slice is the state machine itself; (b) `Sync` (`server.go:1761-1765`) is **already correct** — it clears `syncRequired` and writes `ReadyForQuery` without touching transaction state, which is exactly PG's behaviour for an explicit block, so the tempting "end the transaction at Sync" edit would silently re-break every block. Add the four tests that must FAIL against today's HEAD and become the acceptance bar: block-spans-Executes, rollback-actually-rolls-back, status-byte, aborted-block. A milestone whose first slice cannot produce a red test has not reproduced the bug. Gates: `go test ./internal/server/`.
- [ ] **M0132-S2 — `BEGIN`/`COMMIT`/`ROLLBACK` drive the real block state machine** (est ~2 loops; design `0132-0001` §3 D1). Replace the short-circuit at `dispatch_extended.go:110-112`. **Extract, do not mirror:** the simple path's arm is ~290 lines (`dispatch.go:2696-2984`) covering the PG rule that COMMIT in a failed block performs a ROLLBACK and reports `ROLLBACK`, enum-DDL undo, the `Begin/EndLocalTransaction` hooks, and the pending enum/composite/range-type queues. Lift it into one `applyTransactionVerb(ctx, connTx, txNode, autoCommitPtr) (tag, err)` helper both dispatchers call; a second copy would rot exactly as the `TxCommit`-bypasses-`execCommit` bug did (`deferred_fk_check_simple_query_commit_bypass`). The one real asymmetry: the simple path *promotes* an already-begun transaction, the extended path reaches the verb before beginning anything, so the helper takes the transaction as an argument. The refactor must be proven behaviour-preserving for the simple path BEFORE the new extended behaviour is switched on. Free consequence: `ReadyForQuery` starts reporting `T`, since `w.TxStatusFn` is already shared. Gates: `go test ./internal/server/ ./internal/executor/`, D-002 isolation. **Lands with S3+S4.**
- [ ] **M0132-S3 — In-block `Execute` reuses `connTx.Tx()`** (est ~1 loop; design `0132-0001` §3 D2). Sites `dispatch_extended.go:134-139` (begin) and `:361-365` (commit) plus the rollback `defer` at `:143-150` become conditional on `connTx.InExplicit()`, mirroring `dispatch.go:227-229`. Use the `connTx.Tx()` **accessor**, not a cached handle — it returns the session's current transaction once an XID has been materialised (`conn_tx.go:442-455`, M0100-0002) so a statement self-sees the block's earlier writes. Gates: as S2 plus `scripts/tpch-spotcheck.sh` (canonical Q12=2/Q13=35 — the extended path carries bench traffic). **Lands with S2+S4.**
- [ ] **M0132-S4 — Deferred-constraint checks at the extended `COMMIT`** (est ~1 loop; design `docs/design/0132-0002-extended-commit-deferred-constraints.md`, write BEFORE code). The extended COMMIT runs the same deferred FK / UNIQUE / EXCLUDE passes the simple COMMIT does via `transactionOp.execCommit` (`internal/executor/operators_tx.go:118`, whose arms roll back and end the explicit transaction on violation). Gates: FK isolation specs + `internal/testport/` deferred-constraint tests — a bespoke unit test alone does not discharge this. **Lands with S2+S3.**
- [ ] **M0132-S5 — Aborted-block semantics** (est ~1 loop). An error inside an open block marks it failed via `connTxState.Fail()` (`conn_tx.go:276`) so `wireStatus` reports `E` and subsequent `Execute`s fail 25P02 `current transaction is aborted` until `ROLLBACK` (or a `COMMIT` that PG converts to one). Inherit the simple path's behaviour through the shared helper; do not re-derive it. Gates: `go test ./internal/server/`, D-002 isolation.
- [ ] **M0132-S6 — `BEGIN ISOLATION LEVEL …` over the extended protocol** (est ~1 loop; design `0132-0001` §3 D4). Honour `txNode.IsolationLevel` the way `dispatch.go:2703-2725` does, including its subtlety that the placeholder transaction at the wrong level must be rolled back before the correctly-levelled one is begun so no XID or SSI bookkeeping leaks. Retires the `dispatch_extended.go:119-123` comment recording SERIALIZABLE as unreachable. Gates: an SSI write-skew isolation spec that must abort when the block is opened over the extended protocol.
- [ ] **M0132-S7 — Proc-slot discipline** (est ~1-2 loops; design `docs/design/0132-0003-extended-txn-proc-slot-discipline.md`, write BEFORE code). `(procNum + halfSize) % mvcc.ConnSlotCount` (`dispatch_extended.go:137-138`) exists only to keep the per-`Execute` autocommit transaction off the connection's own ProcArray slot; after S3 it is correct ONLY for the out-of-block case. Doc 09 §5 I3 records three `-S -M prepared` clients aborting with `mvcc: unknown transaction` at 50 sustained clients, consistent with the offset scheme colliding — **reproduce it, then close it**; it is in scope, not a follow-up. Gates: `make race-gate`, a ≥50-client `-S -M prepared` run with zero aborts.
- [ ] **M0132-S8 — Mixed simple↔extended blocks** (est ~1 loop). `BEGIN` via simple query then `Execute`s via the extended protocol, and the mirror, must form ONE coherent block — one commit, one rollback, one status byte. Both paths share the same `*connTxState`, so this should compose for free; the slice is the spec that proves it plus whatever it uncovers. Gates: D-002 isolation spec.
- [ ] **M0132-S9 — `Sync` guard test** (est ~0.5 loop). A `Sync` between two in-block `Execute`s must leave the block open (`T` after the `Sync`; the second `Execute`'s work still rolls back on `ROLLBACK`). Locks in S1's finding (b) so a future reader does not "fix" `Sync` into ending the block. Gates: `go test ./internal/server/`.
- [ ] **M0132-S10 — SAVEPOINT / subtransactions over the extended protocol: implement or defer EXPLICITLY** (est ~1 loop to decide; design `docs/design/0132-0004-extended-protocol-savepoints.md` or a ledger row). Doc 09 O-XP-2. `planner.TxSavepoint`/`TxRelease`/`TxRollbackTo` reach the same `*planner.Transaction` node, so the shared helper MUST handle them explicitly — either implemented, or returning `0A000 feature_not_supported` on the extended path. Falling through to a bare tag is today's silent-no-op bug wearing a different verb and is not an acceptable outcome. Model: `operators_tx.go:456`/`:497`/`:518`; known hazards: `savepoint_before_write_parent_zero`, `savepoint_rollback_visibility_three_fixes`.
- [ ] **M0132-S11 — Perf acceptance** (est ~1 loop). `run_rw50.sh` `-M prepared` at scale 100: (1) `-N` shows the commit back on `END` (one fsync per transaction) and TPS ≥ the simple-mode 9,898 baseline; (2) `-S` TPS exceeds the simple-mode 89,955 — the parse/plan ceiling doc 06-03 predicted, unmasked once the per-`Execute` transaction overhead is gone; (3) `aux2_fsync_probe.sh` `-N` fsync count per 60 s back to ~one per transaction group. Per the benchmarking practice card: hold server age constant across the A/B, run capped via `scripts/goopg-test-run.sh`, and record the numbers in `analysis/` with the commit hash. Runs LAST.

**Open questions (answer before the slice that depends on them):** O-1 — do portals already support an open portal spanning statements inside a block (doc 09 O-XP-3)? Answer before S3 closes. O-2 — `executeExtendedQuery`'s pre-parse fast paths (`extended.go:434+`, e.g. the literal `SELECT 1` short-circuit at `:439`) return without touching a transaction; each needs an explicit ruling, because a fast path that bypasses the block is exactly the shape of bug this milestone exists to fix. O-3 — `PREPARE TRANSACTION` over the extended protocol currently reaches the same short-circuited node while `twophase.go:227` calls `connTx.End()`; confirm whether S2 changes its behaviour and gate it if so.

**Acceptance bar:** the 12 items in `docs/milestones/0132-extended-protocol-explicit-transactions.md`. Headline gates: `ROLLBACK` over the extended protocol leaves the row at its original value (verified from a second connection); a block's `INSERT` is invisible to another connection until `COMMIT`; `ReadyForQuery` reports `T`/`E`/`I` correctly at the frame level; an unrepaired `INITIALLY DEFERRED` FK fails at the `COMMIT`, not at the statement. No regressions: UNITS/SMOKE/SPOT green, `scripts/tpch-spotcheck.sh` canonical Q12=2/Q13=35, `make race-gate` clean, `make ralph-state-guard` clean.
