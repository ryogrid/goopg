...
`[x]`. M0129 (Q74 fix + M0128 verdict follow-ups + residual-ledger burn-down)
...
again once M0129 is complete (M0124, M0125, M0127, M0128, and M0123 are all
CLOSED, every item `[x]`).
...
and M0123 are fully checked off. The M-NIGHTLY backlog was unblocked by this —
and is now subordinated to **M0129** (the top-priority milestone per the user
directive above). The historical directive block below (2026-07-28
...
- [x] **regress/{boolean,case,create_function_sql,errors,index_including,limit,
      numerology,partition_aggregate,portals_p2,select} — genuine sub-timeout
      divergences — ALL RESOLVED.** `errors` (root-0031), `index_including`
      (root-0034), `select_distinct` (root-0036) closed 2026-07-28. `boolean`,
      `case`, `create_function_sql`, `limit`, `numerology`, `partition_aggregate`,
      `tid`, `time`, `timetz`, `truncate`, `union`, `varchar` all pass.
      **`portals_p2` and `select` CLOSED 2026-08-08 (loop #48) — both PASS at HEAD
      `aec67933` in a clean environment (isolated and full-suite).** The divergence
      was caused by orphaned goopg test servers from prior runs (PID 3359982,
      3384620 from Aug 7) consuming host resources, triggering the `cluster` test
      case to time out, which poisoned the cluster and caused a restart-failure
      cascade. With orphans killed, both cases are green. The restart-failure
      cascade itself is a real gap (crash-restart works in isolation but the test
      framework's 20s start timeout may be too tight after a heavy-WAL case), filed
      as a new M-NIGHTLY item below.
      **Historical PARKED note retained for provenance; the directive is stale
      since M0124/M0125/M0127/M0128/M0123 are all CLOSED.**
      Original resume point: two cases left, `portals_p2` and `select`. Try the ISOLATED run FIRST
...
- [x] **regress/{btree_index,char,delete,int2,int4,int8,limit,numerology,
      ...
      **↳ `delete` IS FIXED 2026-08-06 (third S7-gate loop) — `SKIP(deferred)
      ...
      open.
      **↳ CLOSED 2026-08-08 — three clean nightlies confirm phantom.** The
      20260805 nightly had ZERO of the 15; the 20260808 nightly also has zero.
      The 3 that recurred in 20260806 (delete/portals_p2/select) were all
      individually fixed by 2026-08-08. portals_p2 and select verified PASS at
      HEAD `aec67933` in clean environments; the remaining 12 are
      wedge-downstream phantom divergences.**
