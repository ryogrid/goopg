...
- [ ] **M0131-S15 — Runtime `CREATE DATABASE` must lay down the critical system indexes** (est ~2-3 loops; MEASURED by M0131-S4 F4). Connecting a hosted PG to a database goopg minted at runtime fails `PANIC: could not open critical system index 2662` (`pg_class_oid_index`) — `RelationCacheInitializePhase3` nails and opens a small critical set during `InitPostgres` and a failure there is a PANIC, so the whole cluster goes down with the connection attempt. **This is about the runtime CREATE DATABASE path, not the cold start:** the `postgres` database in the SAME directory serves every other S4 assertion. `initdb` writes each bootstrapped btree into `base/1` **and** `base/5` (`internal/initdb/btree_index_bootstrap.go` throughout); the runtime path clones only what goopg itself needs, and goopg reads its catalogs without those indexes, so nothing in the goopg-only world notices. Subtasks: S15.1 enumerate PG's critical set (`relcache.c` `RelationCacheInitializePhase3` / `criticalRelcachesBuilt`, plus the shared set in Phase 2) and check each against what CREATE DATABASE copies; S15.2 decide the mechanism — copy `base/1`'s bootstrapped index files as the template (which is what upstream's `CREATE DATABASE … TEMPLATE` effectively does) versus re-running the bootstrappers per database; S15.3 the PANIC is on 2662 but the set is larger, so fix the set not the one index; S15.4 INVERT `assertGoopgCreatedDatabaseStillUnopenableByPG` to a direct equality.
