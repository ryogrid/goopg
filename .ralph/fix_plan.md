...
- [ ] **M0110-0001 — Port pg_dump TAP tests**
      ...
      - **PROGRESS 2026-06-19 (loop #41):** **DU-002 slice 226 LANDED — first `RELOPT_KIND_TOAST` *integer*
        reloption (`toast.autovacuum_vacuum_threshold`) round-trips; three-element TOAST reloptions array.**
        `autovacuum_vacuum_threshold` shares `RELOPT_KIND_HEAP | RELOPT_KIND_TOAST` (`reloptions.c:229`, range
        `0–INT_MAX`, default `-1`), so PG accepts the `toast.` prefix and stores it (no prefix) on the TOAST
        relation's reloptions. **Executor** (`operators_ddl.go`): one extra gather block after the slice-225
        `toast.vacuum_truncate` arm, reusing the parent-table integer path (slice 198): `strconv.Atoi` +
        `0 ≤ N ≤ 2147483647` bounds check (non-int/out-of-range → 22023), appended as
        `autovacuum_vacuum_threshold=<N>`. **catalog**: NO change — `strings.Join` over `ToastReloptions`
        renders `{autovacuum_enabled=false,vacuum_truncate=false,autovacuum_vacuum_threshold=100}`. On dump-out
        pg_dump re-adds the prefix per element in array order →
        `WITH (toast.autovacuum_enabled='false', toast.vacuum_truncate='false', toast.autovacuum_vacuum_threshold='100')`.
        Files: `internal/executor/operators_ddl.go` (int gather block),
        `internal/testport/pgdump_connsetup_test.go` (`optoast` fixture carries all three options + updated
        combined-WITH assertion), `docs/design/0110-0001-pg-dump-tap-port.md` (Slice 226). Gates: gofmt OK;
        `go build ./internal/...` clean; executor+parser+catalog suites PASS; `TestPort_PgDumpConnectionSetup`
        PASS; pgbench pre-commit smoke on commit. **Next:** `toast.autovacuum_vacuum_scale_factor` (slice-199
        float path), `toast.autovacuum_vacuum_cost_delay/limit`, `toast.autovacuum_freeze_*_age`,
        `toast.log_autovacuum_min_duration` — each a one-line gather. NOTE: `toast.autovacuum_analyze_*` is
        HEAP-only (TOAST tables aren't analyzed), PG rejects it — do NOT add. After: composite types.
      - **PROGRESS 2026-06-19 (loop #42):** **DU-002 slice 227 LANDED — first `RELOPT_KIND_TOAST` *real*
        reloption (`toast.autovacuum_vacuum_scale_factor`) round-trips; four-element TOAST reloptions array.**
        `autovacuum_vacuum_scale_factor` is `RELOPT_TYPE_REAL` and shares `RELOPT_KIND_HEAP | RELOPT_KIND_TOAST`
        (`reloptions.c:404`, range `0.0–100.0`, default `-1`), so PG accepts the `toast.` prefix and stores it
        (no prefix) on the TOAST relation's reloptions. **Executor** (`operators_ddl.go`): one extra gather
        block after the slice-226 `toast.autovacuum_vacuum_threshold` arm, reusing the parent-table float path
        (slice 199): `strconv.ParseFloat` + `!(f >= 0 && f <= 100)` bounds check (also rejects NaN/±Inf;
        non-float/out-of-range → 22023), appended as `autovacuum_vacuum_scale_factor=<F>` via
        `FormatFloat(f,'g',-1,64)`. **catalog**: NO change — `strings.Join` over `ToastReloptions` renders
        `{autovacuum_enabled=false,vacuum_truncate=false,autovacuum_vacuum_threshold=100,autovacuum_vacuum_scale_factor=2.5}`.
        ...
        `internal/testport/pgdump_connsetup_test.go` (`optoast` fixture carries all four options + updated
        combined-WITH assertion), `docs/design/0110-0001-pg-dump-tap-port.md` (Slice 227). Gates: gofmt OK;
        `go build ./internal/...` clean; executor+parser+catalog suites PASS; `TestPort_PgDumpConnectionSetup`
      ...
      - **PROGRESS 2026-06-19 (loop #43):** **DU-002 slice 228 LANDED — second `RELOPT_KIND_TOAST` *real*
        ...
        `toast.log_autovacuum_min_duration` (INT, allows -1) — each a one-line gather. After: composite types.
      - **PROGRESS 2026-06-19 (loop #44):** **DU-002 slice 229 LANDED — second `RELOPT_KIND_TOAST` *integer*
        reloption (`toast.autovacuum_vacuum_cost_limit`) round-trips; six-element TOAST reloptions array.**
        `autovacuum_vacuum_cost_limit` is `RELOPT_TYPE_INT` and shares `RELOPT_KIND_HEAP | RELOPT_KIND_TOAST`
        (`reloptions.c:265`, range `1–10000`, default `-1`), so PG accepts the `toast.` prefix and stores it
        (no prefix) on the TOAST relation's reloptions. **Executor** (`operators_ddl.go`): one extra gather
        block after the slice-228 `toast.autovacuum_vacuum_cost_delay` arm, reusing the parent-table integer
        path (slice 198): `strconv.Atoi` + `1 ≤ N ≤ 10000` bounds check (non-int/out-of-range → 22023),
        appended as `autovacuum_vacuum_cost_limit=<N>` via `strconv.Itoa`. **catalog**: NO change —
        `strings.Join` over `ToastReloptions` renders
        `{...,autovacuum_vacuum_cost_delay=10.5,autovacuum_vacuum_cost_limit=500}`. On dump-out pg_dump re-adds
        the prefix per element in array order → `WITH (..., toast.autovacuum_vacuum_cost_limit='500')`.
        Files: `internal/executor/operators_ddl.go` (int gather block),
        `internal/testport/pgdump_connsetup_test.go` (`optoast` fixture carries all six options + updated
        combined-WITH assertion), `docs/design/0110-0001-pg-dump-tap-port.md` (Slice 229). Gates: gofmt OK;
        `go build ./internal/...` clean; executor+parser+catalog suites PASS; `TestPort_PgDumpConnectionSetup`
      ...
      - **PROGRESS 2026-06-19 (loop #45):** **DU-002 slice 230 LANDED — first `RELOPT_KIND_TOAST`
        ...
        **Executor** (`operators_ddl.go`): one extra gather block after the slice-229
        ...
        `toast.log_autovacuum_min_duration` (INT, allows -1) — each a one-line gather. After: composite types.
      - **PROGRESS 2026-06-19 (loop #46):** **DU-002 slice 231 LANDED — second `RELOPT_KIND_TOAST`
        autovacuum-age *integer* reloption (`toast.autovacuum_freeze_max_age`) round-trips; eight-element
        TOAST reloptions array.** `autovacuum_freeze_max_age` is `RELOPT_TYPE_INT` and shares
        `RELOPT_KIND_HEAP | RELOPT_KIND_TOAST` (`reloptions.c:1887/290`, range `100000–2000000000`, default `-1`),
        so PG accepts the `toast.` prefix and stores it (no prefix) on the TOAST relation's reloptions.
        **Executor** (`operators_ddl.go`): one extra gather block after the slice-230
        `toast.autovacuum_freeze_min_age` arm, mirroring the parent-table integer arm (slice 208):
        `strconv.Atoi` + `100000 ≤ N ≤ 2000000000` bounds check (non-int/out-of-range → 22023; the `100000`
        floor rejects an explicit `-1`), appended as `autovacuum_freeze_max_age=<N>` via `strconv.Itoa`.
        **catalog**: NO change — `strings.Join` over `ToastReloptions` renders
        `{...,autovacuum_freeze_min_age=200000000,autovacuum_freeze_max_age=500000000}`. On dump-out pg_dump
        re-adds the prefix per element in array order → `WITH (..., toast.autovacuum_freeze_max_age='500000000')`.
        Files: `internal/executor/operators_ddl.go` (int gather block),
        `internal/testport/pgdump_connsetup_test.go` (`optoast` fixture carries all eight options + updated
        combined-WITH assertion), `docs/design/0110-0001-pg-dump-tap-port.md` (Slice 231). Gates: gofmt OK;
        `go build ./internal/...` clean; executor+parser+catalog suites PASS; `TestPort_PgDumpConnectionSetup`
        PASS; pgbench pre-commit smoke on commit. **Next:** `toast.autovacuum_freeze_table_age` (INT,
        `0–2000000000`), multixact variants (`toast.autovacuum_multixact_freeze_min/max_age`,
        `…_table_age`), `toast.log_autovacuum_min_duration` (INT, allows -1) — each a one-line gather. After:
        composite types.
      - **PROGRESS 2026-06-19 (loop #48):** **DU-002 slice 232 LANDED — third `RELOPT_KIND_TOAST`
        autovacuum-age *integer* reloption (`toast.autovacuum_freeze_table_age`) round-trips; nine-element
        TOAST reloptions array.** `autovacuum_freeze_table_age` is `RELOPT_TYPE_INT` and shares
        `RELOPT_KIND_HEAP | RELOPT_KIND_TOAST` (`reloptions.c:1889/308`, range `0–2000000000`, default `-1`),
        so PG accepts the `toast.` prefix and stores it (no prefix) on the TOAST relation's reloptions.
        **Executor** (`operators_ddl.go`): one extra gather block after the slice-231
        `toast.autovacuum_freeze_max_age` arm, mirroring the parent-table integer arm (slice 209):
        `strconv.Atoi` + `0 ≤ N ≤ 2000000000` bounds check (non-int/out-of-range → 22023; the `0` floor
        rejects an explicit `-1`), appended as `autovacuum_freeze_table_age=<N>` via `strconv.Itoa`. The
        fixture uses `0` to exercise that `0` is a valid explicit value. **catalog**: NO change —
        `strings.Join` over `ToastReloptions` renders `{...,autovacuum_freeze_max_age=500000000,autovacuum_freeze_table_age=0}`.
        On dump-out pg_dump re-adds the prefix per element in array order →
        `WITH (..., toast.autovacuum_freeze_table_age='0')`. Files: `internal/executor/operators_ddl.go`
        (int gather block), `internal/testport/pgdump_connsetup_test.go` (`optoast` fixture carries all nine
        options + updated combined-WITH assertion), `docs/design/0110-0001-pg-dump-tap-port.md` (Slice 232).
        Gates: gofmt OK; `go build ./internal/...` clean; executor+parser+catalog suites PASS;
        `TestPort_PgDumpConnectionSetup` PASS; pgbench pre-commit smoke on commit. **Next:** the multixact
        variants (`toast.autovacuum_multixact_freeze_min_age` INT `0–1000000000`, `…max_age` INT
        `10000–2000000000`, `…table_age` INT `0–2000000000`), `toast.log_autovacuum_min_duration` (INT,
        allows -1) — each a one-line gather. After: composite types.
      - **PROGRESS 2026-06-19 (loop #49):** **DU-002 slice 233 LANDED — fourth `RELOPT_KIND_TOAST`
        autovacuum-age *integer* reloption (`toast.autovacuum_multixact_freeze_min_age`) round-trips;
        ten-element TOAST reloptions array.** `autovacuum_multixact_freeze_min_age` is `RELOPT_TYPE_INT`
        and shares `RELOPT_KIND_HEAP | RELOPT_KIND_TOAST` (`reloptions.c:1891/286`, range `0–1000000000`,
        default `-1`), so PG accepts the `toast.` prefix and stores it (no prefix) on the TOAST relation's
        reloptions. **Executor** (`operators_ddl.go`): one extra gather block after the slice-232
        `toast.autovacuum_freeze_table_age` arm, mirroring the parent-table integer arm (slice 210):
        `strconv.Atoi` + `0 ≤ N ≤ 1000000000` bounds check (non-int/out-of-range → 22023; the `0` floor
        rejects an explicit `-1`), appended as `autovacuum_multixact_freeze_min_age=<N>` via `strconv.Itoa`.
        **catalog**: NO change — `strings.Join` over `ToastReloptions` renders
        `{...,autovacuum_freeze_table_age=0,autovacuum_multixact_freeze_min_age=150000000}`. On dump-out
        pg_dump re-adds the prefix per element in array order →
        `WITH (..., toast.autovacuum_multixact_freeze_min_age='150000000')`. Files:
        `internal/executor/operators_ddl.go` (int gather block),
        `internal/testport/pgdump_connsetup_test.go` (`optoast` fixture carries all ten options + updated
        combined-WITH assertion), `docs/design/0110-0001-pg-dump-tap-port.md` (Slice 233). Gates: gofmt OK;
        `go build ./internal/...` clean; executor+parser+catalog suites PASS; `TestPort_PgDumpConnectionSetup`
        PASS; pgbench pre-commit smoke on commit. **Next:** `toast.autovacuum_multixact_freeze_max_age` (INT
        `10000–2000000000`), `…table_age` (INT `0–2000000000`), `toast.log_autovacuum_min_duration` (INT,
        allows -1 — special-case the floor) — each a one-line gather. After: composite types.
      - **PROGRESS 2026-06-19 (loop #50):** **DU-002 slice 234 LANDED — fifth `RELOPT_KIND_TOAST`
        autovacuum-age *integer* reloption (`toast.autovacuum_multixact_freeze_max_age`) round-trips;
        eleven-element TOAST reloptions array.** `autovacuum_multixact_freeze_max_age` is `RELOPT_TYPE_INT`
        and shares `RELOPT_KIND_HEAP | RELOPT_KIND_TOAST` (`reloptions.c:1893/304`, range `10000–2000000000`,
        default `-1`), so PG accepts the `toast.` prefix and stores it (no prefix) on the TOAST relation's
        reloptions. **Executor** (`operators_ddl.go`): one extra gather block after the slice-233
        `toast.autovacuum_multixact_freeze_min_age` arm, mirroring the parent-table integer arm (slice 211):
        `strconv.Atoi` + `10000 ≤ N ≤ 2000000000` bounds check (non-int/out-of-range → 22023; the `10000`
        floor rejects an explicit `-1`), appended as `autovacuum_multixact_freeze_max_age=<N>` via
        `strconv.Itoa`. **catalog**: NO change — `strings.Join` over `ToastReloptions` renders
        `{...,autovacuum_multixact_freeze_min_age=150000000,autovacuum_multixact_freeze_max_age=500000000}`. On
        dump-out pg_dump re-adds the prefix per element in array order →
        `WITH (..., toast.autovacuum_multixact_freeze_max_age='500000000')`. Files:
        `internal/executor/operators_ddl.go` (int gather block),
        `internal/testport/pgdump_connsetup_test.go` (`optoast` fixture carries all eleven options + updated
        combined-WITH assertion), `docs/design/0110-0001-pg-dump-tap-port.md` (Slice 234). Gates: gofmt OK;
        `go build ./internal/...` clean; executor+parser+catalog suites PASS; `TestPort_PgDumpConnectionSetup`
      ...
      - **PROGRESS 2026-06-20 (loop #23):** **DU-002 slice 257 LANDED — a composite-type field with an explicit
        per-field `COLLATE` (`CREATE TYPE x AS (a text COLLATE "C", …)`) now round-trips through pg_dump.**
        `CREATE TYPE`'s composite-field parser collected the *whole* field tail (incl. `COLLATE "C"`) into
        `ColType`, so `parseCompositeFieldType` mis-resolved the type and the collation was silently dropped —
        `dumpCompositeType` re-emitted the field with no `COLLATE` clause. Fix mirrors the table-column path
        (slice 188): **Parser** (`parseCreateType` composite loop, `ddl.go`) — the type-token loop now also
        stops at a top-level `COLLATE` ident keyword (Kind-agnostic break, like `ALTER ATTRIBUTE`), so the type
        stays clean (`text`, not `text COLLATE "C"`); the optional trailing `COLLATE <name>` is parsed via the
        existing `parseCollationName` (bare `"C"` + schema-qualified `pg_catalog."C"`, keeping the bare last
        component) into the new `TypeField.Collation`. **AST/catalog** — `parser.TypeField` +
        `catalog.CompositeField` each gain `Collation string`; the executor `CREATE TYPE` path
        (`operators_ddl.go` ~10257) propagates `f.Collation`. **Heap row**
        (`buildUserPGAttributeRowForCompositeField`, `pg18_user_catalog_rows.go`) — `attcollation` now applies the
        per-field override exactly as the table-column builder (slice 188): for a collatable type
        (`typcollation != 0`) and a name resolving via `collationNameToOID` (C→950, POSIX→951), stamp the
        override; a COLLATE on a non-collatable type (int, typcollation 0) is suppressed (no bogus OID). Flows
        through `syncCompositeTypeToCatalogHeap` (same builder) → the heap `pg_attribute` row pg_dump scans.
        No pg_dump-side change: `dumpCompositeType` reports `attcollation <> typcollation` and re-emits
        `COLLATE pg_catalog."<name>"` inline; uncollated fields stay clause-free. Scope: `CREATE TYPE` composite
        fields only (ALTER-TYPE ADD/ALTER ATTRIBUTE COLLATE still stub-consumes the trailing COLLATE). Guards:
        `parser.TestCompositeFieldCollateParsing` (bare / typmod-then-COLLATE / schema-qualified / uncollated) +
        `executor.TestUserPGAttributeCompositeFieldCollation` (C→950/POSIX→951 override, text default 100
        untouched, non-collatable int→0) + pg_dump TAP port creates
        `public.coll_comp AS (a text COLLATE "C", b text, p text COLLATE "POSIX")` and asserts the dump re-emits
        `a text COLLATE pg_catalog."C", b text, p text COLLATE pg_catalog."POSIX"` (bare `\tb text,` proves the
        middle field clause-free), byte-matching real pg_dump 18.3 (`TestPort_PgDumpConnectionSetup` PASS, 3.3s).
      ...
      - **PROGRESS 2026-06-20 (loop #50):** **DU-002 slice 283 LANDED — a STORED GENERATED column inherited onto a partition
        leaf prints its generation clause INLINE in the leaf body, not as a (illegal) standalone SET DEFAULT (no production
        change; different discriminator branch than 281/282).** The generated-column counterpart of slices 281/282. Where
        281/282 held `attrdefs[].separate=false` via `shouldPrintColumn` (every partition column prints because
        `ispartition`), slice 283 holds `separate=false` via **`attgenerated`**: pg_dump.c:9507 sets
        `attrdefs[adnum-1].separate = false` UNCONDITIONALLY whenever `tbinfo->attgenerated[adnum-1]` is non-empty — a
        generation expression can never be split into a standalone `ALTER ... SET DEFAULT`. Fixture: `CREATE TABLE public.pgna
        (ga integer, gb integer GENERATED ALWAYS AS (ga * 2) STORED) PARTITION BY LIST (ga)` + `CREATE TABLE public.pgna_1
        PARTITION OF public.pgna FOR VALUES IN (1)` → leaf prints `gb integer GENERATED ALWAYS AS (ga * 2) STORED` inline;
        partition key `ga` stays plain `ga integer`. NO production change — goopg already round-trips STORED generated columns
        on standalone tables (slice 59: attgenerated='s' + pg_attrdef deparse) and inherits parent columns onto partition
        leaves (slices 281/282); the two facts compose. A regression dropping the generation expression on the inherited
        column would print bare `gb integer`; one mis-setting `separate` would try the illegal standalone SET DEFAULT.
        **Guards:** `TestPort_PgDumpConnectionSetup` PASS (3.68s; byte-identical vs real pg_dump 18.3 — asserts the `pgna_1`
        block prints `ga integer` + inline `gb integer GENERATED ALWAYS AS (ga * 2) STORED`, NO standalone `ALTER COLUMN gb
        SET DEFAULT` anywhere, `ATTACH PARTITION public.pgna_1 FOR VALUES IN (1)` survives). gofmt + `go build ./...` clean;
        pgbench pre-commit smoke on commit. Design: `0110-0001` Slice 283.
        **Next (slice 284+):** a VIRTUAL generated column (`GENERATED ALWAYS AS … VIRTUAL`, slice 194's attgenerated='v' form)
        inherited onto a partition leaf — same `separate=false`-via-`attgenerated` branch but rendered WITHOUT trailing
        `STORED`; or a multi-column / NULL-typed DEFAULT variant on the partition-leaf ALTER path.
      - **PROGRESS 2026-06-20 (loop #52):** **DU-002 slice 284 LANDED — a VIRTUAL GENERATED column inherited onto a partition
        leaf prints its generation clause INLINE in the leaf body, rendered BARE without a trailing `STORED` keyword (no
        production change; same discriminator branch as 283, different render branch).** The VIRTUAL counterpart of slice 283.
        Both ride the same `separate=false`-via-`attgenerated` branch — pg_dump.c:9507 sets `attrdefs[].separate=false`
        UNCONDITIONALLY whenever `attgenerated` is non-empty, and `ATTRIBUTE_GENERATED_VIRTUAL` ('v') is non-empty exactly like
        `ATTRIBUTE_GENERATED_STORED` ('s') — but the render differs: pg_dump.c:17171 emits `GENERATED ALWAYS AS (%s)` with NO
        trailing keyword for a virtual column (the STORED branch at 17168 is skipped). Fixture: `CREATE TABLE public.pvna (va
        integer, vb integer GENERATED ALWAYS AS (va * 2) VIRTUAL) PARTITION BY LIST (va)` + `CREATE TABLE public.pvna_1
        PARTITION OF public.pvna FOR VALUES IN (1)` → leaf prints `vb integer GENERATED ALWAYS AS (va * 2)` inline, bare;
        partition key `va` stays plain `va integer`. NO production change — goopg already round-trips VIRTUAL generated columns
        on standalone tables (slice 194: attGeneratedFor reports 'v') and inherits parent columns onto partition leaves (slices
        281/282/283); the two facts compose. A regression that mis-mapped attgenerated 'v'→'s' would print a spurious trailing
        STORED; one that dropped the generation expression would print bare `vb integer`. **Guards:**
        `TestPort_PgDumpConnectionSetup` PASS (3.89s; vs real pg_dump 18.3 — asserts the `pvna_1` block prints `va integer` +
        inline `vb integer GENERATED ALWAYS AS (va * 2)`, NO trailing `STORED`, NO standalone `ALTER COLUMN vb SET DEFAULT`
        anywhere, `ATTACH PARTITION public.pvna_1 FOR VALUES IN (1)` survives). gofmt + `go build` clean; pgbench pre-commit
      ...
      - **PROGRESS 2026-06-20 (loop #57):** **DU-002 slice 289 LANDED — PRODUCTION fix: canonical `GeneratedExpr` deparse.**
        The FIRST parenthesised generation expression (`(fa + fb) * 2`) inherited onto a partition leaf round-trips. Every
        prior generation slice (283–288) used a FLAT operator chain whose captured lexer tokens space-join faithfully; a
        parenthesised (or function-call) body exposed a deparse defect — goopg captured the `GENERATED ALWAYS AS (...)` body
        as raw tokens and joined them with single spaces (`strings.Join(toks, " ")`), so `(fa + fb) * 2` became
        `( fa + fb ) * 2` (and `upper(fn)` → `upper ( fn )`). pg_dump wraps the source `(%s)`, so goopg emitted
        `(( fa + fb ) * 2)` where real pg_dump's `pg_get_expr` renders the precedence paren TIGHT: `((fa + fb) * 2)`. **Fix:**
        new `joinGeneratedExprTokens` (parser/ddl.go) reconstructs `pg_get_expr`'s spacing from the captured token stream —
        tight function calls (`upper(fn)`), `, `-separated args (`coalesce(a, b)`), tight grouping parens (`(fa + fb) * 2`),
        tight qualified names, spaced binary operators — keying off punctuation + the previous token's `Kind` (a `(` is tight
        after an identifier/`)` = call paren, spaced otherwise = grouping paren). BOTH `GeneratedExpr` capture sites (column-
        def + `PARTITION OF (... WITH OPTIONS GENERATED ALWAYS AS ...)` override) now route through it. The result re-parses
        to the same node, so `evalGeneratedExpr` (re-parses the stored string for materialization) is unaffected and every
        flat-chain slice (283–288) keeps its byte-identical stored source. Render path otherwise identical to 283–288
        (attgenerated 's' → `attrdefs[].separate=false` pg_dump.c:9507; ispartition → shouldPrintColumn every column).
        Fixture: `CREATE TABLE public.pgpp (fa integer, fb integer, fc integer GENERATED ALWAYS AS ((fa + fb) * 2) STORED)
        PARTITION BY LIST (fa)` + `CREATE TABLE public.pgpp_1 PARTITION OF public.pgpp FOR VALUES IN (1)`. **Guards:**
        `TestPort_PgDumpConnectionSetup` PASS (4.07s; vs real pg_dump 18.3 — asserts the `pgpp_1` block prints inline
        `fc integer GENERATED ALWAYS AS ((fa + fb) * 2) STORED` with the inner paren TIGHT, in attnum order, ATTACH survives);
        `TestGeneratedColumnExprCanonicalSpacing` (parser, no server — pins canonical spacing for func calls, two-arg calls,
        grouping parens, nested call in arithmetic, plain chains, `||`); `go test ./internal/{parser,executor,catalog}/`
        PASS; gofmt + `go vet` clean; pgbench pre-commit smoke on commit. Files: `internal/parser/ddl.go` (helper + both
        capture sites → `[]Token`), `internal/parser/gen_override_test.go` (new test), `internal/testport/
        pgdump_connsetup_test.go` (pgpp fixture + assertion), `docs/design/0110-0001-pg-dump-tap-port.md` (Slice 289).
        **Next (slice 290+):** function-call generation deparse END-TO-END (`upper(fn)`) — parser canonicalization is in
      ...
      - **PROGRESS 2026-06-20 (loop #61):** **DU-002 slice 293 LANDED — THREE-argument function-call generation expression
        ...
        **Next (slice 294+):** a function-call generation expression with a LITERAL argument (`concat(ka, '-', la)`) to pin
        ...
        ALTER path.
      - **PROGRESS 2026-06-20 (loop #62):** **DU-002 slice 294 LANDED — PRODUCTION fix: string-literal argument in a function-call
        generation expression `concat(ka, '-', la)` round-trips end-to-end on the pg_dump oracle.** Slices 291–293 pinned the
        call-paren / comma-spacing branches of `joinGeneratedExprTokens` for IDENTIFIER arguments only. A string-literal argument
        exposed a latent production bug: the lexer stores a literal's UNQUOTED body (`'-'` → `Token.Value` `"-"`), and the helper
        space-joined token values raw — so `concat(ka, '-', la)` would have round-tripped as the MALFORMED `concat(ka, -, la)`
        (quotes dropped, the literal indistinguishable from a minus operator). **Fix (`internal/parser/ddl.go`):** new `renderTok`
        closure re-quotes `TokenStringLit` (doubling embedded single quotes); the punctuation spacing rules are gated on
        `TokenSymbol` so a literal body of `)`/`,`/`(`/`.` can't be mistaken for a punctuator. For the operator/call/grouping-paren
        expressions of slices 283–293 (no string literals) every punctuator is already a `TokenSymbol`, so the gating is a NO-OP
        there — byte-identical output, zero regression. Because the test dumps a LIVE goopg server, real pg_dump reads goopg's stored
        source verbatim, so the assertion pins goopg's own canonical `concat(ka, '-', la)` (goopg does NOT add the `::text` cast
        real PG's `pg_get_expr` injects; out-of-scope divergence, like the lowercase-name divergence of 290–293). Fixture:
        `CREATE TABLE public.pglc (ka text, la text, na text GENERATED ALWAYS AS (concat(ka, '-', la)) STORED) PARTITION BY LIST (ka)` +
        `pglc_1 PARTITION OF`. **Guards:** `TestGeneratedColumnExprCanonicalSpacing` PASS (3 new cases: `concat(fa, '-', fb)`,
        `fa || '-' || fb` literal operand, `concat(fa, '''', fb)` embedded-quote literal re-quoted as `''''`);
        `TestPort_PgDumpConnectionSetup` PASS (3.63s vs real pg_dump 18.3 — asserts `pglc_1` prints the inline
        `na text GENERATED ALWAYS AS (concat(ka, '-', la)) STORED`, the malformed `concat(ka, -, la)` is ABSENT, attnum order,
        ATTACH survives); gofmt + `go vet` + `go build ./...` clean; pgbench pre-commit smoke on commit. Files:
        `internal/parser/ddl.go` (PRODUCTION fix to `joinGeneratedExprTokens`), `internal/parser/gen_override_test.go` (3 unit
        cases), `internal/testport/pgdump_connsetup_test.go` (pglc fixture + assertion), `docs/design/0110-0001-pg-dump-tap-port.md`
        (Slice 294).
