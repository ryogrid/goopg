...
- [ ] **M0131-S30 — crash recovery loses committed work (MEASURED + CONFIRMED 2026-08-11; re-scoped from "measure" to "fix" — see `docs/design/0131-0020`)** (est ~4 loops, split into three defects). The re-measurement S30 demanded has been RUN: proven kill (pid from `postmaster.pid`, `/proc/<pid>/cmdline` verified before the signal and `/proc` verified after, run ABORTS if either check fails) and index-independent assertions (`count(*) = count(distinct aid) = 500000, min=1, max=500000` pins the aid set exactly, no anti-join needed). **3/3 runs FAIL, with three DISTINCT defects** — the loss is real and larger than S30 reported. Probe + gate: `RUNS=3 bash analysis/crashprobe30.sh` must print `OVERALL: PASS`.
    ...
    - [ ] **S30.8 — committed `UPDATE pgbench_accounts` new tuples are still not visible after replay** (the remaining half of S30.7, split out 2026-08-11 loop #140 once the XID-reuse mechanism was removed; `docs/design/0131-0023` §"What this did NOT fix"). `RUNS=2 crashprobe30` now diverges in ONE direction in both runs — `sum(history.delta)` > `sum(abalance)` (118007/108024, 616047/600730). Only committed transactions' history rows are visible, so the deficit is committed accounts-UPDATE work; `count(*) = count(distinct aid) = 500000`, `min=1`, `max=500000` still hold, so the OLD version of those rows survived and the update's NEW tuple is missing (a missing `xmax` stamp instead would leave both versions live and break the distinct/count assertions). `restart.log` shows no early end-of-WAL, so it is not an S30.1 recurrence. Resume: the non-HOT heap-update replay arm in `internal/wal/recovery.go` and its `pd_lsn` skip guard (`pgbench_accounts` is fillfactor 100, so most updates move the tuple to another page) — same runtime-vs-WAL page-divergence family as S30.3/S30.5. Gate: `RUNS=2 bash analysis/crashprobe30.sh` must print `OVERALL: PASS`. **PREMISE REFUTED 2026-08-11 (loop #141) — see S30.9 below and `docs/design/0131-0024`.** This item was filed as a replay defect purely because `crashprobe30` only ever evaluates the atomicity invariant after a SIGKILL. The control was finally run: the SAME invariant fails with NO crash anywhere (clean 16-client pgbench, clean shutdown: history 60279 rows vs 60593 committed txns; accounts/tellers/branches/history sums all four different). So the post-restart divergence is not evidence of a replay bug, the "unidirectional" reading carries no information, and NO replay code should be changed on this item's strength until S30.9's gate passes. Item stays open but BLOCKED on S30.9.
    - [ ] **S30.9 — concurrent committed INSERTs are silently lost on a LIVE server (no crash)** (found 2026-08-11 loop #141 while working S30.8; `docs/design/0131-0024`, ledger row same date). **This is a live data-loss defect, not a recovery defect, and it blocks S30.8/S30.2/S30.4 from being measurable at all** — every S30 conclusion drawn from `crashprobe30`'s atomicity line is confounded by it. Minimal repro `analysis/lostrows-concurrent-insert.sh`: fresh cluster, one table with a primary key, 8 clients each running 100 single-statement `INSERT ... generate_series(lo,hi)` transactions over DISJOINT deterministic id ranges — no updates, no deletes, no row contention, nothing killed. Result `rows=75922 want=80000 heap_missing=6328 index_unreachable=5837`: 7.9% of committed rows are absent from the HEAP entirely, every client exited 0 under `ON_ERROR_STOP=1` with zero error lines. The two counts are not nested (~491 ids absent from a seq scan yet returned by an index scan), so heap and btree diverge in both directions. Loss is page-tail CONTIGUOUS (`689-700`, `1400-1419`, `1983-2000`, `2058-2067`, ...), i.e. whole tails of appends to one page vanish together. Second, separable effect on pgbench: ~1.4% of `pgbench_accounts` rows become index-unreachable (heap row correct, primary key does not find it), so every later `UPDATE ... WHERE aid = ?` matches ZERO rows and silently drops the increment — 2000 sequential updates of one such row left `abalance` at 1; `REINDEX TABLE` repairs it, so entries are genuinely missing. **Ruled out, do not re-test:** a crash/replay (nothing is killed; the count comes from the live buffer pool), client-side error, a pre-corrupted datadir (fresh `goopg init`), single-client sequential appends (exact at `-c 1`, and exact over 12 updates on a 200000-row `fillfactor=100` table), and the ENTIRE S30.3 duplicate-buffer mechanism — `GOOPG_PAGEIDENT_PROBE=1` was enabled for a full reproducing run and fired ZERO `PAGEIDENT-DUPSLOT`/`-REGRESS`/`-REEXTEND` events while 4985 rows were lost. Resume (cheapest first, `docs/design/0131-0024` §5): (1) re-run with `shared_buffers` above the working set to establish whether eviction is required at all — that alone splits flush-path from append-path; (2) `Pool.flushBatch` (`internal/storage/bufpool.go:2483`) — audit the window between `WriteBlockAIO` issue (:2539) and `h.Wait()` (:2559) and confirm every append path really mutates under `contentMu.Lock()`; (3) the dirty-bit clear at :2571, which re-checks the tag but not whether the page changed after the bytes went to AIO; (4) `batchExtendAndRegisterFSM`/`selectFSMCandidatePage` (`internal/executor/operators_storage.go:8636-8703`) — every batch-extended block must be initialised and visible to `NBlocks` before an FSM consumer can pin it. Gate: `bash analysis/lostrows-concurrent-insert.sh` must print `OVERALL: PASS`; only then is `RUNS=2 bash analysis/crashprobe30.sh` meaningful as a recovery gate.
