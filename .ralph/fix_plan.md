...
PARKED** — 0005 was parked 2026-08-19 by the same directive — so the next M0134
task to select is **M0134-0006 (`select_having.sql`)**. Note 0006 is one of the
four "possible regression, verify" cases: re-run
`scripts/pg-regress-runner.sh select_having` at HEAD FIRST and, if it already
passes, flip the CSV row to `pass` / `pass_required=yes` with a "stale — already
fixed" note instead of implementing anything. M0132 and M0133 are COMPLETE,
...
- [ ] **M0134-0006 — select_having.sql** — regress-sql `failed`: make the case match PG 18.3 (normalise against `./postgres/`). Re-verify at HEAD first (possible regression); if it already passes, flip the CSV row to `pass` with a "stale — already fixed" note instead of implementing.
- [ ] **M0134-0007 — select_implicit.sql** — regress-sql `failed`: make the case match PG 18.3 (normalise against `./postgres/`). Re-verify at HEAD first (possible regression); if it already passes, flip the CSV row to `pass` with a "stale — already fixed" note instead of implementing.
