...
- [ ] **M0131-S30 — crash recovery loses committed work (MEASURED + CONFIRMED 2026-08-11; re-scoped from "measure" to "fix" — see `docs/design/0131-0020`)** (est ~4 loops, split into three defects). The re-measurement S30 demanded has been RUN: proven kill (pid from `postmaster.pid`, `/proc/<pid>/cmdline` verified before the signal and `/proc` verified after, run ABORTS if either check fails) and index-independent assertions (`count(*) = count(distinct aid) = 500000, min=1, max=500000` pins the aid set exactly, no anti-join needed). **3/3 runs FAIL, with three DISTINCT defects** — the loss is real and larger than S30 reported. Probe + gate: `RUNS=3 bash analysis/crashprobe30.sh` must print `OVERALL: PASS`.
    ...
    - [ ] **S30.1 — replay mistakes a segment's unusable tail for end-of-WAL** (the row-loss defect; 4416 of 500000 rows lost in one run). Replay stopped at `lsn=134217721` = `0x7FFFFF9`, seven bytes short of the 128 MiB segment mark, with `reason="invalid record header" detail="padding bytes nonzero (0xff 0x07)"`, and **discarded the entire WAL tail**. When the space left in a page/segment cannot hold a record header, PG continues at the next page after its header instead of parsing the leftover bytes; a RECYCLED segment makes those bytes non-zero, which is what trips this. Same family as `M0131-S19`'s page-header work (commit 15e73de3).
