...
- [ ] **M0131-S18 — pg_control writer durability + full checkpoint-struct coverage + live TLI** (est ~2 loops). `UpdateControlFile` (`internal/control/pgcontrol.go:213-237`) uses `os.WriteFile` — `O_TRUNC` + write with **no fsync** — where upstream's `update_controlfile` (`postgres/src/common/controldata_utils.c:189`) opens `O_RDWR`, writes all 8192 bytes and fsyncs. A SIGKILL inside that window leaves a **zero-length pg_control** and PG PANICs `could not read file "global/pg_control": read 0 of 296`. Subtasks: S18.1 rewrite as `O_RDWR` + `WriteAt` + `Sync`; S18.2 extend `ControlFileData` with the never-decoded `checkPointCopy` fields `nextMulti`(76) `nextMultiOffset`(80) `oldestXid`(84) `oldestXidDB`(88) `oldestMulti`(92) `oldestMultiDB`(96) `oldestCommitTsXid`(112) `newestCommitTsXid`(116) `oldestActiveXid`(120) — decode AND encode (today they survive only by read-modify-write); S18.3 thread the LIVE TLI into `Checkpointer.Config` and `UpdateControlCheckpoint` — both hardcode `1` (`checkpointer.go:757-758`, `initdb/pgcontrol.go:154-155`), so the next checkpoint after M0130-S8.5's `finalizePromotion` stomps the TLI back to 1 while segments are named for TLI 2 and PG then PANICs `could not locate a valid checkpoint record`; thread `full_page_writes` the same way (it reaches the buffer pool at `cmd/goopg/main.go:719-721` but not pg_control); S18.4 stop hardcoding `nextMulti=1`/`oldestMulti=1`/commitTs in `encodeCheckPointStruct` (`internal/wal/recovery.go:783-794`). Gate: golden-byte round-trip against the real `pg_controldata` in `postgres/local_install/bin`. Design: `docs/design/0131-0013-pgcontrol-runtime-state-and-durability.md`.
