...
- [ ] **M0131-S32.2 — the SeqScan-driven UPDATE arm still loses concurrent updates in the SCAN phase** (filed 2026-08-12 loop #145; wrong-answer class; successor to S32.1). Gate: `NOIDX=1 ROWS=1 CLIENTS=8 N=200 bash analysis/concurrent-hotrow.sh` — lands 1063 of 1600. The write-phase classes are CLOSED here: `count(*)` is correct (no duplicate live versions) and the probe reports `epq_self_modified=0` AND `epq_chain_notfound=0`, so the statement is not skipping rows it found — it is finding NO row and reporting `UPDATE 0`. **Start here:** instrument the already-declared `s321ScanNoRow` counter in the seq-scan update path to confirm, then compare `mvcc.TupleVisible` with upstream `HeapTupleSatisfiesUpdate` (`postgres/src/backend/access/heap/heapam_visibility.c`) — the suspicion is that the old version is treated as dead the moment its xmax COMMITS, regardless of whether that commit is visible to our statement snapshot, while the successor's xmin is too new, so the row momentarily has no visible version at all. This is the last table diverging in the S32.1 gate: `pgbench_branches` (5 rows) is planned as a SeqScan, hence `sum(bbalance) = -54588` vs `sum(delta) = -68881`. Remove `internal/executor/s321_probe.go` and the `GOOPG_S321_NOWAIT` kill switch when this closes.
