...
- [ ] **M0131-S32.3 — the last 0.25% of `pgbench_branches` still diverges under the 4-table BEGIN/COMMIT workload** (filed 2026-08-12 loop #146; wrong-answer class; successor to S32.2). Gate: `RUNS=1 LOADSEC=30 bash analysis/atomicity-nocrash-control.sh` — `sum(bbalance)=-966698` vs `sum(delta)=-969135` (2437 increments, 0.25%), while `sum(abalance)` and `sum(tbalance)` are exact and the isolated single-table repro `analysis/concurrent-hotrow.sh` is exact in every configuration. So the residual needs something the minimal repro lacks: an explicit multi-statement transaction holding the row across three other tables' writes (longer xmax lifetimes, so more EPQ laps per statement) — first check whether the loop is hitting `epqRetryLimit`/`maxEPQRetriesRC` and raising 40001 that pgbench retries or swallows, and whether `pgbench_history`'s INSERT ordering matters. **Start here:** extend `analysis/concurrent-hotrow.sh` with a TXN mode that also writes a second table inside the same BEGIN/COMMIT (closing the gap between the two harnesses), then re-run with `GOOPG_S321_PROBE=1` and read `epq_chain_pending_wait` / `scan_epq_pending_wait` against the retry ceiling. Remove `internal/executor/s321_probe.go` and the `GOOPG_S321_NOWAIT` / `GOOPG_S322_NOWAIT` kill switches when this closes.
  - (original filing text, kept for the measurements it records; superseded by the two entries above) M0131-S32.1 (filed 2026-08-12 loop #144; wrong-answer class; the concurrent half left over after S32's chain-walk fix). Gate: `RUNS=1 LOADSEC=30 bash analysis/atomicity-nocrash-control.sh`, extended this loop to assert `sum(tbalance)` and `sum(bbalance)` against `sum(delta)` (the arm that exposed S32 and the sensitive one — 50 tellers / 5 branches means the same row is updated thousands of times, whereas 500000 accounts almost never collide). With the S32 fix in place, scale 5 / 16 clients / 30 s still fails identically LIVE and after a clean restart: `sum(delta) = -31482`, `sum(abalance) = -31482` exact, but `sum(tbalance) = -14938` (under-applied) and `sum(bbalance) = -309543` (~10x OVER-applied). Both directions wrong ⇒ NOT the S32 truncation. **Start here:** over-application points at the duplicate-index-entry path — a non-HOT update leaves the old index entry live, so a repeatedly-updated row accumulates several live entries for one key, and the guard for that (AI-20260810-011258-006, `internal/executor/operators_storage.go:4244`) dedupes only WITHIN one statement's `pending` set. Under-application on tellers is the opposite sign and may be the plain lost-update/EPQ arm. Measure the two tables separately before assuming one cause. Note the script's header records `OVERALL: FAIL` as the KNOWN state, so read the per-table lines, not the verdict.
