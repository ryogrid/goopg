...
- [ ] **M0131-S30 — crash recovery loses committed work (MEASURED + CONFIRMED 2026-08-11; re-scoped from "measure" to "fix" — see `docs/design/0131-0020`)** (est ~4 loops, split into three defects). The re-measurement S30 demanded has been RUN: proven kill (pid from `postmaster.pid`, `/proc/<pid>/cmdline` verified before the signal and `/proc` verified after, run ABORTS if either check fails) and index-independent assertions (`count(*) = count(distinct aid) = 500000, min=1, max=500000` pins the aid set exactly, no anti-join needed). **3/3 runs FAIL, with three DISTINCT defects** — the loss is real and larger than S30 reported. Probe + gate: `RUNS=3 bash analysis/crashprobe30.sh` must print `OVERALL: PASS`.
    ...
    - [ ] **S30.3 — the non-HOT heap-update replay arm is wrong** (independent of any WAL-framing issue): one run REFUSED TO START with `wal replay: replay record 826236 lsn[154662577,154662656]: xlog heap-update add new tuple: storage: not enough free space in page` — replay's page state has diverged from what execution saw. S30's retained hypothesis (`updateOp.Next()`'s fallback emits `HeapDelete` at `operators_storage.go:5270` and `HeapInsert` at `:5300` as two separate records where PG writes one atomic `XLOG_HEAP_UPDATE`; fix = route through `EncodeHeapUpdatePG` + a matching replay arm) is now backed by a replay error inside that arm. **But the two-record split alone cannot lose a committed row** (both records precede the commit record), so the fix must also explain the run where every row survived yet `sum(abalance) != sum(history.delta)` — torn transactions. Do not stop at making the record atomic.
