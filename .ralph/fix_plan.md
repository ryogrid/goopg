...
- [ ] **M0134-0008 — select_parallel.sql** — **PARKED 2026-08-19: structurally
      ...
      **Landed this loop (net progress, kept):** `scripts/pg-regress-runner.sh`
      ...
      after (clean stashed-baseline re-run). **CSV corrected 2026-09-01:** the
      row had been left `not-tried` despite being run live and shown to
      genuinely diverge — a bookkeeping gap, not a stale status (flagged by
      M0134-0189's loop as a discrepancy to resolve). Flipped to `failed` /
      `pass_required=no` with a rationale citing this task; `make
      regen-testport` run in the same commit.
...
- [ ] **M0134-0010 — predicate.sql** — **PARKED 2026-08-19.** Sized at HEAD
      ...
      (d) `Sort`/`Materialize` emission parity. Three ledger rows appended
      2026-08-19. **CSV corrected 2026-09-01:** the row had been left
      `not-tried` despite being run live and shown to genuinely diverge — a
      bookkeeping gap, not a stale status (flagged by M0134-0189's loop as a
      discrepancy to resolve). Flipped to `failed` / `pass_required=no` with a
      rationale citing this task; `make regen-testport` run in the same
      commit.
