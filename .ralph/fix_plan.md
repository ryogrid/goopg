...
- [ ] **M0134-0005 — constraints.sql** — regress-sql `failed`: make the case match PG 18.3 (normalise against `./postgres/`). On pass, flip the CSV row to `pass` / `pass_required=yes` in the same commit.
  ...
  - [ ] **M0134-0005c — index build appears to scan non-live tuples (EXPOSED by 0005b, caps the rest of Bucket 4)** — `ALTER TABLE unique_tbl ADD CONSTRAINT unique_tbl_i_key UNIQUE (i) DEFERRABLE INITIALLY DEFERRED` fails `could not create unique index` / `DETAIL: Key (i)=(1) is duplicated` while the immediately preceding `SELECT * FROM unique_tbl` matches PG **byte-for-byte** — no MVCC-correct read can see that duplicate. **Hypothesis, NOT yet confirmed:** the eager validate-then-build scan counts dead row versions; before 0005b the `UPDATE i=i+1` failed, so the table carried no dead versions at this point in the file, and now it succeeds and leaves the old versions behind. Exposed by, not caused by, 0005b — and it is now the cap on the rest of Bucket 4 exactly as `unique_tbl_i_key`'s absence was before (the downstream "9 rows vs expected 5 / no error" hunks are a **cascade** of the constraint never existing, not a data-integrity regression). Resume: `internal/executor/operators_ddl.go:11969` / `:12016` — check tuple-liveness filtering against PG's MVCC-aware build scan (`postgres/src/backend/catalog/index.c:index_build` → `IndexBuildHeapScan`, `HeapTupleSatisfiesVacuum`). **Confirm the hypothesis before briefing an implementer.** Design: §10.4; ledgered 2026-08-18.
