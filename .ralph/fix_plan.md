...
- [ ] **M0134-0005 — constraints.sql** — regress-sql `failed`: make the case match PG 18.3 (normalise against `./postgres/`). On pass, flip the CSV row to `pass` / `pass_required=yes` in the same commit.
  - Progress: Buckets 1, 2, 3, 6 LANDED plus the `reg*[]` cast (2026-08-18); measurement 1496 → **1411** lines / **33** hunks. Full bucket map, per-bucket root causes and the ranked next slice live in `docs/design/0134-0005-constraints-sql-divergence.md` (§8 = next slice). Read that doc, not this line, before selecting the next slice.
