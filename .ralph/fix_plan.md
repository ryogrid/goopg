...
- [ ] **M0134-0001 — aggregates.sql** — regress-sql `failed`: make the case match PG 18.3 (normalise against `./postgres/`). On pass, flip the CSV row to `pass` / `pass_required=yes` in the same commit.
  - Progress (see `docs/design/0134-0001-p2-explain-format.md`): S1/S2/S4/S5/S6/S7/S10a landed; **S3 (class 7a) landed 2026-08-15** — join labels now PG-interpolated (commit `aa71b24d`), aggregates diff 764→746. class 10 blocked by the varno deferral (ledger 615). Remaining: S8 GroupAggregate + multi-rel GROUP BY (cost-model, largest), scalar-subquery nesting, inheritance MergeAppend (dead-end).
