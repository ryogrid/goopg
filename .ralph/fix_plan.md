...
- [ ] **M0131-S30 — crash recovery heap-row loss (RE-SCOPED 2026-08-11 — original diagnosis REFUTED, see S31 and `docs/design/0131-0019`)** (est ~2 loops). **Do not act on the text below without reading the refutation first.** The confirmation step this item demanded was run: S30's evidence came from an anti-join that goopg executes as an `Index Scan using pgbench_accounts_pkey`, so it measured index REACHABILITY, not row presence; and the run that produced it **never actually killed the server** (`pgrep -f "bin/goopg start -D $DIR"` does not match the cgroup-wrapped command line). With no crash at all the same shape gives `count(*)`=500000, `count(distinct aid)`=500000 and **672 "missing" aids** — so the "~154 DUPLICATED" inference is unsupported and the missing-aid figure is contaminated by S31. What REMAINS open and genuinely unmeasured is the raw `count(*)` loss S30 reported (500000 -> 499949): **re-measure it with a kill that actually fires** (match on the data-dir path, or read the pid from `postmaster.pid`) and with a heap-only anti-join that cannot use an index (`WHERE aid+0 = g`), THEN decide whether any WAL-atomicity work is needed. Original (unconfirmed) hypothesis retained for reference only: `updateOp.Next()`'s non-HOT fallback emits `HeapDelete` + `HeapInsert` as two SEPARATE WAL records (ledger M0118-0129, re-confirmed by M0130-S7.2) where PG's `heap_update` writes ONE `XLOG_HEAP_UPDATE`, so a crash between them leaves both versions or neither — a second candidate is FPI coverage of pages first touched between redo and the crash. If a real `count(*)` loss survives the re-measurement above, the fix is to route the fallback through the atomic `EncodeHeapUpdatePG` record `updateHeapRowCanonicalPG` already uses, plus the matching replay arm. Gate: a repro whose kill is PROVEN to have fired, with count+sum identical and zero missing rows by a heap-only anti-join, three consecutive runs. Design: rides `docs/design/0130-0007-wal-record-fidelity-xlprev-atomic-update.md` (its S7.2 audit named this exact gap).
