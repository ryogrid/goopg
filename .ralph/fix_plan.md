...
- [ ] **M0131-S30 — crash recovery loses AND duplicates heap rows (non-atomic non-HOT update)** (est ~2 loops; **wrong-answer class — found 2026-08-11 while gating S20, PRE-EXISTING**). Repro: `goopg init`, start capped, `pgbench -i -s 5` + `pgbench -T 20 -c 4 -j 2`, snapshot `select count(*), sum(abalance) from pgbench_accounts`, `kill -9 $(head -1 $PGDATA/postmaster.pid)`, restart, re-snapshot. Observed 500000 -> 499949 rows, and an anti-join against `generate_series(1,500000)` found **218 missing `aid`s against only 64 net missing rows — ~154 DUPLICATED**. Reproduced identically on a worktree build of `15e73de3` (500000 -> 499936), so it is not an S20 regression. Root cause almost certainly the already-ledgered non-atomic path: `updateOp.Next()`'s non-HOT fallback emits `HeapDelete` + `HeapInsert` as two SEPARATE WAL records (ledger M0118-0129, re-confirmed by M0130-S7.2) where PG's `heap_update` writes ONE `XLOG_HEAP_UPDATE`, so a crash between them leaves both versions or neither. First step is to CONFIRM that diagnosis (dump the WAL tail with `pg_waldump` around a duplicated `aid`) before changing anything — a second candidate is FPI coverage of pages first touched between redo and the crash. Fix: route the fallback through the atomic `EncodeHeapUpdatePG` record `updateHeapRowCanonicalPG` already uses, plus the matching replay arm. Gate: the repro above with count+sum identical and zero missing/duplicate `aid`s, three consecutive runs. Design: rides `docs/design/0130-0007-wal-record-fidelity-xlprev-atomic-update.md` (its S7.2 audit named this exact gap).
