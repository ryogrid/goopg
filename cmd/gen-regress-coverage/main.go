...
func main() {
	...
	excluded := 0
	for _, c := range cases {
		if _, ok := excludedCases[c.Name]; ok {
			excluded++
		}
	}
	...
	fmt.Fprintf(f, "Total discovered SQL cases: **%d** (%d excluded, %d defer/port target)\n\n",
		len(cases), excluded, len(cases)-excluded)
	...
	for _, c := range cases {
		...
		status := "defer"
		if excl, ok := excludedCases[c.Name]; ok {
			status = "excluded"
			rationale = excl
