...
// original name/schema/sType/sFunc/finalFunc/combineFunc/initCond/
// finalFuncModify/argTypes/OID/strict-flag/variadic-flag. `schema` was
// added to close slice 405 resume point (a) — pronamespace now survives a
// restart instead of always resolving to public.
func TestEncodeDecodeCreateAggregateRoundTrip(t *testing.T) {
	cases := []struct {
		name, schema, sType, sFunc, finalFunc, combineFunc, initCond, finalFuncModify string
		argTypes                                                                      []string
		oid                                                                           uint32
		sFuncStrict, variadic                                                         bool
	}{
		{"newavg", "public", "_avgstate", "avg_transfn", "avg_finalfn", "", "", "", []string{"int4"}, 16384, true, false},
		{"myvariadicagg", "public", "int8", "int8pl", "", "int8pl", "0", "read_write", []string{"int8"}, 16385, false, true},
		{"zeroarg", "myschema", "int8", "myinc", "", "", "", "", nil, 16386, false, false},
		{"日本語集約", "日本語スキーマ", "text", "concat_state", "concat_final", "", "", "", []string{"text", "text"}, 4294967295, true, false}, // multi-byte UTF-8, max OID
	}
	for _, c := range cases {
		raw := EncodeCreateAggregate(c.name, c.schema, c.sType, c.sFunc, c.finalFunc, c.combineFunc, c.initCond, c.finalFuncModify, c.argTypes, c.oid, c.sFuncStrict, c.variadic)
		if raw[0] != RecordKindCreateAggregate {
			t.Errorf("%q: kind byte = %d, want %d", c.name, raw[0], RecordKindCreateAggregate)
			continue
		}
		gotName, gotSchema, gotSType, gotSFunc, gotFinalFunc, gotCombineFunc, gotInitCond, gotFinalFuncModify, gotArgTypes, gotOID, gotStrict, gotVariadic, err := DecodeCreateAggregate(raw)
		if err != nil {
			t.Errorf("%q: decode err: %v", c.name, err)
			continue
		}
		if gotName != c.name || gotSchema != c.schema || gotSType != c.sType || gotSFunc != c.sFunc || gotFinalFunc != c.finalFunc ||
			...
			t.Errorf("%q: decoded (%q, %q, %q, %q, %q, %q, %q, %q, %d, %v, %v)",
				c.name, gotName, gotSchema, gotSType, gotSFunc, gotFinalFunc, gotCombineFunc, gotInitCond, gotFinalFuncModify, gotOID, gotStrict, gotVariadic)
		}
...
func TestDecodeCreateAggregateRejectsTruncatedPayload(t *testing.T) {
	full := EncodeCreateAggregate("agg", "public", "int4", "sfunc", "", "", "", "", []string{"int4"}, 100, false, false)
	for n := 0; n < len(full); n++ {
		if _, _, _, _, _, _, _, _, _, _, _, _, err := DecodeCreateAggregate(full[:n]); err == nil && n < len(full) {
