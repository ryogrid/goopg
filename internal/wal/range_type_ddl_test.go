...
// must return the original name/subtypeName/multirangeName/OID/
// multirangeOID/opclassOID.
func TestEncodeDecodeCreateRangeTypeRoundTrip(t *testing.T) {
	cases := []struct {
		...
		{"myrange", "int4", "mymultirange", 16384, 16385, 1978},
		{"tsrange", "timestamp with time zone", "tsmultirange", 16386, 16387, 3127},
		{"日本語レンジ", "text", "日本語マルチレンジ", 4294967295, 4294967294, 3125}, // multi-byte UTF-8, max OID
	}
	for _, c := range cases {
		raw := EncodeCreateRangeType(c.name, c.subtypeName, c.multirangeName, c.oid, c.multirangeOID, c.opclassOID)
		if raw[0] != RecordKindCreateRangeType {
			t.Errorf("%q: kind byte = %d, want %d", c.name, raw[0], RecordKindCreateRangeType)
			continue
		}
		gotName, gotSubtype, gotMR, gotOID, gotMROID, gotOpclass, err := DecodeCreateRangeType(raw)
		if err != nil {
			t.Errorf("%q: decode err: %v", c.name, err)
			continue
		}
		if gotName != c.name || gotSubtype != c.subtypeName || gotMR != c.multirangeName ||
			gotOID != c.oid || gotMROID != c.multirangeOID || gotOpclass != c.opclassOID {
			t.Errorf("%q: decoded (%q, %q, %q, %d, %d, %d), want (%q, %q, %q, %d, %d, %d)",
				c.name, gotName, gotSubtype, gotMR, gotOID, gotMROID, gotOpclass,
				c.name, c.subtypeName, c.multirangeName, c.oid, c.multirangeOID, c.opclassOID)
		}
	}
}
...
func TestDecodeRangeTypeRejectsWrongKindAndTruncatedPayload(t *testing.T) {
	...
	if _, _, _, _, _, _, err := DecodeCreateRangeType(bogus); err == nil {
		t.Error("DecodeCreateRangeType: expected error on wrong kind")
	}
	if _, err := DecodeDropRangeType(EncodeCreateRangeType("x", "int4", "xmr", 1, 2, 1978)); err == nil {
	...
	truncCases := []struct {
		...
		{"CreateRangeType", []byte{RecordKindCreateRangeType, 0, 0, 0, 0}, func(p []byte) error {
			_, _, _, _, _, _, err := DecodeCreateRangeType(p)
			return err
		}},
		{"CreateRangeTypeTruncatedString", append(append([]byte{RecordKindCreateRangeType}, make([]byte, 12)...), 5, 0), func(p []byte) error {
			// header (13 bytes) + a 2-byte length prefix declaring 5 bytes of
