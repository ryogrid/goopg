...
func btreeLeafPageWith(t *testing.T, items ...[]byte) storage.Page {
	...
	btree.WritePGOpaque(page, btree.PGBTPageOpaque{Prev: btree.PNone, Next: btree.PNone, Flags: btree.BTPLeaf})
...
func TestApplyRecordReplaysPGBtreeInsertPost(t *testing.T) {
	...
	base := btree.PGBTItemRaw([]byte("dup"), tid(7, 10))
	oposting := btree.PGBTPostingRaw(base, []storage.ItemPointer{
	...
	// on the primary.
	orignewitem := btree.PGBTItemRaw([]byte("dup"), tid(7, 25))
	...
	if got := btree.BTreeTupleGetNPosting(gotPosting); got != 4 {
		t.Fatalf("rewritten posting holds %d TIDs, want 4", got)
	}
	wantTIDs := []storage.ItemPointer{tid(7, 10), tid(7, 20), tid(7, 25), tid(7, 30)}
	off := btree.BTreeTupleGetPostingOffset(gotPosting)
	for i, want := range wantTIDs {
		got := btree.PGItemPointerAt(gotPosting[off+i*btree.SizeOfItemPointerData:])
		if got != want {
	...
	if got := btree.PGIndexTupleTID(gotNew); got != tid(7, 40) {
...
func TestApplyRecordReplaysPGBtreeInsertPostFromImage(t *testing.T) {
	...
	base := btree.PGBTItemRaw([]byte("dup"), tid(7, 10))
	before := btreeLeafPageWith(t, btree.PGBTPostingRaw(base, []storage.ItemPointer{tid(7, 10), tid(7, 20)}))
	...
	after := btreeLeafPageWith(t,
		btree.PGBTPostingRaw(base, []storage.ItemPointer{tid(7, 10), tid(7, 15)}),
		btree.PGBTItemRaw([]byte("dup"), tid(7, 20)),
	)
	...
	if got := btree.PGIndexTupleTID(gotNew); got != tid(7, 20) {
...
func TestApplyRecordRefusesPGBtreeInsertPostBadPostingOff(t *testing.T) {
	...
	btreeSeedRel(t, mgr, rel, btreeLeafPageWith(t,
		btree.PGBTPostingRaw(base, []storage.ItemPointer{tid(7, 10), tid(7, 20)})))
	...
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			framed := buildBtreeInsertPostPG(t, rel, 0, 2, tc.postingoff,
				btree.PGBTItemRaw([]byte("dup"), tid(7, 15)))
...
func TestApplyRecordRefusesPGBtreeInsertPostOnNonPosting(t *testing.T) {
	...
	plain := btree.PGBTItemRaw([]byte("dup"), tid(7, 10))
	btreeSeedRel(t, mgr, rel, btreeLeafPageWith(t, plain))

	framed := buildBtreeInsertPostPG(t, rel, 0, 2, 1, btree.PGBTItemRaw([]byte("dup"), tid(7, 15)))
...
func TestApplyRecordRefusesPGBtreeInsertPostAtFirstOffset(t *testing.T) {
	...
	framed := buildBtreeInsertPostPG(t, rel, 0, 1, 1, btree.PGBTItemRaw([]byte("dup"), tid(7, 15)))
