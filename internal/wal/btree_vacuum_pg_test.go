...
func vacuumTestPage(t *testing.T, keys []string, garbage bool) storage.Page {
	...
	flags := uint16(btree.BTPLeaf)
	if garbage {
		flags |= btree.BTPHasGarbage
	}
	btree.WritePGOpaque(p, btree.PGBTPageOpaque{Prev: 5, Next: 7, Level: 0, Flags: flags})
	// P_HIKEY: the page is not rightmost, so its first line pointer is the high
	// key and data starts at P_FIRSTKEY (2).
	if _, err := storage.PageAddItemRaw(p, btree.PGBTPivotRaw([]byte("hikey"), btree.PNone)); err != nil {
		t.Fatal(err)
	}
	for _, k := range keys {
		...
		if _, err := storage.PageAddItemRaw(p, btree.PGBTPivotRaw([]byte(k), storage.BlockNumber(100+int(k[0])))); err != nil {
			t.Fatal(err)
		}
	}
	return p
...
func TestReplayBtreeVacuumPGReproducesPage(t *testing.T) {
	...
	if err := btree.CheckVacuumDelete(pre, got, []uint16{3, 5}); err != nil {
		t.Fatalf("replayed page is not the deletion result: %v", err)
	}
	if op := btree.ReadPGOpaque(got); op.Flags&btree.BTPHasGarbage != 0 {
