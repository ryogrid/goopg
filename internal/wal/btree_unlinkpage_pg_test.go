...
func unlinkTestPage(t *testing.T, prev, next storage.BlockNumber, level uint32, flags uint16) storage.Page {
	...
	btree.WritePGOpaque(page, btree.PGBTPageOpaque{Prev: prev, Next: next, Level: level, Flags: flags})
	return page
}
...
func unlinkTestHalfDeadLeaf(t *testing.T, prev, next, topparent storage.BlockNumber) storage.Page {
	t.Helper()
	page := unlinkTestPage(t, prev, next, 0, btree.BTPHalfDead|btree.BTPLeaf)
	if _, err := storage.PageAddItemRaw(page, btree.PGBTPivotRaw(nil, topparent)); err != nil {
		t.Fatal(err)
	}
	return page
}
...
func unlinkTestMetaPage(t *testing.T, root storage.BlockNumber, level uint32) storage.Page {
	...
	btree.WritePGMetaPage(page, btree.PGBTMetaPage{
		Magic: btree.BTreeMagicPG, Version: btree.BTreeVersionPG,
...
func TestEncodeBtreeUnlinkPagePGIsContentParity(t *testing.T) {
	...
	framed, err := EncodeBtreeUnlinkPagePG(BtreeUnlinkPagePGRequest{
		Rel:        rel,
		TargetBlk:  targetBlk,
		TargetPage: unlinkTestPage(t, prev, next, 0, btree.BTPHalfDead|btree.BTPLeaf),
		SafeXid:    safexid,
		LeafBlk:    targetBlk,
...
func TestEncodeBtreeUnlinkPagePGInternalTargetLogsLeafAndMeta(t *testing.T) {
	...
	framed, err := EncodeBtreeUnlinkPagePG(BtreeUnlinkPagePGRequest{
		Rel:        rel,
		TargetBlk:  targetBlk,
		TargetPage: unlinkTestPage(t, btree.PNone, 6, 2, 0),
		SafeXid:    99,
		LeafBlk:    leafBlk,
	...
	le := binary.LittleEndian
	md := dec.XLog.MainData
	if got := storage.BlockNumber(le.Uint32(md[0:4])); got != btree.PNone {
...
func TestEncodeBtreeUnlinkPagePGRefusesUndescribableRecords(t *testing.T) {
	...
	leafTarget := unlinkTestPage(t, 4, 6, 0, btree.BTPHalfDead|btree.BTPLeaf)
	...
	for _, tc := range []struct {
		...
		{"rightmost target", BtreeUnlinkPagePGRequest{
			Rel: rel, TargetBlk: 5, TargetPage: unlinkTestPage(t, 4, btree.PNone, 0, btree.BTPLeaf),
		...
		{"leaf is not half-dead", BtreeUnlinkPagePGRequest{
			Rel: rel, TargetBlk: 5, TargetPage: internalTarget,
			LeafBlk: 11, LeafPage: unlinkTestPage(t, 10, 12, 0, btree.BTPLeaf),
			MetaBlk: storage.InvalidBlockNumber,
		}},
...
func TestApplyRecordReplaysPGBtreeUnlinkPage(t *testing.T) {
	...
	for _, page := range []storage.Page{
		...
		unlinkTestPage(t, btree.PNone, targetBlk, 2, 0),
		unlinkTestPage(t, leftBlk, rightBlk, 2, 0),
		unlinkTestPage(t, targetBlk, btree.PNone, 2, 0),
	...
	if op := btree.ReadPGOpaque(read(leftBlk)); op.Next != rightBlk {
		t.Errorf("left sibling btpo_next = %d, want %d", op.Next, rightBlk)
	}
	if op := btree.ReadPGOpaque(read(rightBlk)); op.Prev != leftBlk {
		t.Errorf("right sibling btpo_prev = %d, want %d", op.Prev, leftBlk)
	}

	gotTarget := read(targetBlk)
	op := btree.ReadPGOpaque(gotTarget)
	if op.Flags != btree.BTPDeleted|btree.BTPHasFullXID {
		t.Errorf("target flags = %#x, want BTP_DELETED|BTP_HAS_FULLXID (%#x)", op.Flags, btree.BTPDeleted|btree.BTPHasFullXID)
	}
	...
	if got := h.Lower(); got != storage.SizeOfPageHeaderData+btree.SizeOfBTDeletedPageData {
		t.Errorf("deleted target pd_lower = %d, want %d", got, storage.SizeOfPageHeaderData+btree.SizeOfBTDeletedPageData)
	}
	if h.Upper() != h.Special() {
		t.Errorf("deleted target pd_upper = %d, want pd_special %d", h.Upper(), h.Special())
	}
	if got, ok := btree.PGDeletedPageSafeXid(gotTarget); !ok || got != safexid {
		t.Errorf("target safexid = %#x (ok=%v), want %#x", got, ok, safexid)
	}

	gotLeaf := read(leafBlk)
	if op := btree.ReadPGOpaque(gotLeaf); op.Flags != btree.BTPHalfDead|btree.BTPLeaf || op.Prev != 7 || op.Next != 8 {
	...
	if got := btree.BTreeTupleGetDownLink(hikey); got != nextChild {
