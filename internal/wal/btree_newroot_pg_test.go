...
func newRootTestPages(t *testing.T, level uint32, rootBlk, leftBlk, rightBlk storage.BlockNumber) (root, meta storage.Page) {
	...
	if err := btree.InitPGBTPage(root); err != nil {
		t.Fatal(err)
	}
	flags := btree.BTPRoot
	if level == 0 {
	...
	btree.WritePGOpaque(root, btree.PGBTPageOpaque{Prev: btree.PNone, Next: btree.PNone, Level: level, Flags: flags})
	if level > 0 {
		for _, raw := range [][]byte{
			btree.PGBTPivotRaw(nil, leftBlk),
			btree.PGBTPivotRaw([]byte("separator-key"), rightBlk),
		} {
	...
	if err := btree.InitPGMetaPage(meta, rootBlk, level, true); err != nil {
		t.Fatal(err)
	}
	return root, meta
}
...
func TestEncodeBtreeNewRootPGIsContentParity(t *testing.T) {
	...
	framed, err := EncodeBtreeNewRootPG(rel, rootBlk, root, leftBlk, btree.MetaBlock, meta)
	if err != nil {
		t.Fatal(err)
	}
	...
	if b := byID[2]; !b.WillInit || b.Block != btree.MetaBlock || len(b.Data) != sizeOfXLogBtreeMetadata {
		t.Errorf("block 2 = blk %d willinit=%v %d data bytes, want %d/true/%d", b.Block, b.WillInit, len(b.Data), btree.MetaBlock, sizeOfXLogBtreeMetadata)
	}
	...
	wantItems, err := btree.PGRestorePageData(root)
	if err != nil {
		t.Fatal(err)
	}
	...
	if md.Version != btree.BTreeVersionPG {
		t.Errorf("xl_btree_metadata.version = %d, want %d", md.Version, btree.BTreeVersionPG)
	}
}
...
func TestEncodeBtreeNewRootPGRejectsMissingChild(t *testing.T) {
	...
	if _, err := EncodeBtreeNewRootPG(rel, 9, root, storage.InvalidBlockNumber, btree.MetaBlock, meta); err == nil {
...
func TestApplyRecordReplaysPGBtreeNewRoot(t *testing.T) {
	...
	if op := btree.ReadPGOpaque(got); !op.IsRoot() || op.IsLeaf() || op.Level != 1 {
	...
	if lower := storage.MustHeader(gotMeta).Lower(); int(lower) != storage.SizeOfPageHeaderData+btree.SizeOfBTMetaPageDataPG {
		t.Errorf("replayed metapage pd_lower = %d, want %d", lower, storage.SizeOfPageHeaderData+btree.SizeOfBTMetaPageDataPG)
	}
	...
	if btree.ReadPGOpaque(gotChild).Flags&btree.BTPIncompleteSplit != 0 {
...
func TestApplyRecordReplaysPGBtreeNewRootLeaf(t *testing.T) {
	...
	if err := btree.InitPGBTPage(blank); err != nil {
		t.Fatal(err)
	}
	...
	framed, err := EncodeBtreeNewRootPG(rel, rootBlk, root, storage.InvalidBlockNumber, btree.MetaBlock, meta)
	if err != nil {
		t.Fatal(err)
	}
	rec, _, err := encodeRecordXLog(framed, 0)
	...
	if op := btree.ReadPGOpaque(got); !op.IsRoot() || !op.IsLeaf() || op.Level != 0 {
