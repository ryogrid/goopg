...
func splitLeftTrio(t *testing.T, spliceAt, leftCount int, leftBlk, rightBlk, sibBlk storage.BlockNumber) (pre, left, right, sib storage.Page, newItem []byte) {
	t.Helper()
	newItem = btree.PGBTPivotRaw([]byte("newitem"), 77)
	preItems := make([][]byte, 4)
	for i := range preItems {
		preItems[i] = btree.PGBTPivotRaw([]byte{'p', byte('0' + i)}, storage.BlockNumber(50+i))
	}
	...
	inheritedHK := btree.PGBTPivotRaw([]byte("inherited-hikey"), btree.PNone)
	newHK := btree.PGBTPivotRaw([]byte("new-separator"), btree.PNone)
	build := func(op btree.PGBTPageOpaque, hk []byte, items [][]byte) storage.Page {
	...
	pre = build(btree.PGBTPageOpaque{Prev: btree.PNone, Next: sibBlk, Flags: btree.BTPLeaf}, inheritedHK, preItems)
	left = build(btree.PGBTPageOpaque{Prev: btree.PNone, Next: rightBlk, Flags: btree.BTPLeaf | btree.BTPIncompleteSplit}, newHK, merged[:leftCount])
	right = build(btree.PGBTPageOpaque{Prev: leftBlk, Next: sibBlk, Flags: btree.BTPLeaf}, inheritedHK, merged[leftCount:])
	sib = make(storage.Page, storage.BlockSize)
	if err := btree.InitPGBTPage(sib); err != nil {
		t.Fatal(err)
	}
	btree.WritePGOpaque(sib, btree.PGBTPageOpaque{Prev: rightBlk, Next: btree.PNone, Flags: btree.BTPLeaf})
...
func TestEncodeBtreeSplitPGLeftHalfIsIncremental(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			gotNew, gotHK, err := btree.ParseSplitLeftBlockData(b0.Data, tc.wantInfo == xlogBtreeSplitL)
			if err != nil {
				t.Fatalf("block 0 data: %v", err)
			}
			wantHK, _, err := btree.PGHighKeyRaw(left)
			if err != nil {
				t.Fatal(err)
			}
...
func TestEncodeBtreeSplitPGFallsBackToImage(t *testing.T) {
	...
	copy(mutated, left)
	if err := storage.PageReplaceItemRaw(mutated, 3, btree.PGBTPivotRaw([]byte("mergedposting"), 42)); err != nil {
		t.Fatal(err)
	}
