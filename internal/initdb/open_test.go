...
// cycle. The catalog is stored in the pg_class/pg_attribute heap;
...
func TestRuntimeSaveAndReloadCatalog(t *testing.T) {
	...
	// First session: create table via DDL (writes to heap), call SaveCatalog, close.
	rt1, err := Open(OpenOptions{DataDir: dir, PoolSlots: 2})
	if err != nil {
		t.Fatal(err)
	}
	runDDL(t, rt1, "CREATE TABLE pgbench_accounts (aid int4 NOT NULL, bid int4, abalance int4, filler text)")
...
// TestOpenSystemCatalogNotDuplicated verifies that pg_type (a system
