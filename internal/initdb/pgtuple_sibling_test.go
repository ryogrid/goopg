...
func TestPGIndexTupleMatchesBootstrapEncoders(t *testing.T) {
	...
	oid := nbtree.PGIndexAttr{Len: 4, ByVal: true, AlignBy: 4, Storage: 'p'}
	int2 := nbtree.PGIndexAttr{Len: 2, ByVal: true, AlignBy: 2, Storage: 'p'}
	// NameData is a fixed 64-byte struct with attalign 'c'.
	name := nbtree.PGIndexAttr{Len: 64, ByVal: false, AlignBy: 1, Storage: 'p'}
	...
	cases := []struct {
		...
		},
		{
			...
			[]nbtree.PGIndexAttr{name, oid, oid, oid},
		...
		},
		{
			...
			[]nbtree.PGIndexAttr{oid, oid, oid, int2},
	...
	for _, tc := range cases {
		t.Run(tc.what, func(t *testing.T) {
			...
			got, err := nbtree.FormPGIndexTuple(tc.attrs, tc.vals, isnull, tid)
			if err != nil {
			...
			if nbtree.PGIndexTupleSize(got) != len(tc.want) {
				t.Errorf("t_info size = %d, want %d", nbtree.PGIndexTupleSize(got), len(tc.want))
			}
			if nbtree.PGIndexTupleTID(got) != tid {
				t.Errorf("t_tid = %v, want %v", nbtree.PGIndexTupleTID(got), tid)
			}
			// Round-trip: every stored datum must come back byte-identical.
			vals, nulls, err := nbtree.DeformPGIndexTuple(got, tc.attrs, len(tc.attrs))
			if err != nil {
...
func TestPGBTPivotRawMatchesBootstrapMinusInfinityDownlink(t *testing.T) {
	for _, child := range []uint32{1, 2, 0x1234, 0x00010002} {
		...
		got := nbtree.PGBTPivotRaw(nil, storage.BlockNumber(child))
