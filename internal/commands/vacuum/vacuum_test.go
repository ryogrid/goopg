...
func TestVacuumKeepsLiveBelowOldestXmin(t *testing.T) {
	...
	holder, _ := mvccMgr.Begin(transam.IsolationRepeatableRead)
	defer mvccMgr.Rollback(holder)

	// A later tx commits a delete (xmax=delTx.XID).
	delTx, _ := mvccMgr.Begin(transam.IsolationReadCommitted)
...
func TestVacuumReclaimsFreeSpace(t *testing.T) {
	...
	creator, _ := mvccMgr.Begin(transam.IsolationReadCommitted)
	xidC, _ := mvccMgr.AssignXID(creator)
	creator.XID = xidC
	mvccMgr.Commit(creator)
	deleter, _ := mvccMgr.Begin(transam.IsolationReadCommitted)
