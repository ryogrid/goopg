...
//
// The table-level half of timeouts.spec (rdtbl + locktbl) additionally
// requires plain SELECT to take a transaction-scoped ACCESS SHARE heavyweight
// lock so LOCK TABLE conflicts with it; goopg deliberately keeps ordinary
// scans off the heavyweight lock manager for hot-path reasons, so that half is
// deferred (see docs/design/0118-0017-lock-timeout.md). M0118-0009.
