...
func runFailoverGoopgToPG(t *testing.T, repo, pgBasebackupBin, psqlBin string, mode physicalFailoverMode) {
	...
	// M0106: copy relcache init files from primary to standby.
	// These are not included in the tar by emitBaseBackupTar
	// (exclusion or Walk timing issue). PG backends PANIC without them.
...
// M0106: these files are generated by goopg init but not included in
// the pg_basebackup tar (tar Walk timing or client-side filtering).
func copyInitFiles(t *testing.T, primaryDir, standbyDir string) {
	t.Helper()
	for _, rel := range []string{
		"global/pg_internal.init",
		// Also try base/1 (goopg default DB) since base/5 may not exist
		...
		// Also copy local init to base/5/ for postgres database.
