...
func TestLogicalReceiverReconnectRespectsCtxDuringBackoff(t *testing.T) {
	...
	defer subPool.Close()
	subCat := catalog.NewInMemory()
	apply := executor.NewApplyWorker(subCat, subPool, mvcc.NewManager())
	defer apply.SafeRollback()
...
func runFakePublisherSession(
	...
	if err := fw.WriteReadyForQuery(protocol.TxStatusIdle); err != nil {
	...
	if f.Type != protocol.MsgQuery {
		return fmt.Errorf("expected Query (%c), got %c", protocol.MsgQuery, f.Type)
	}
	...
	emit := func(fn func(po *wal.PgOutput) error) error {
		var buf bytes.Buffer
		po := wal.NewPgOutput(snap, &buf)
		if err := fn(po); err != nil {
			return err
		}
		payload := protocol.EncodeWALData(commitLSN, commitLSN, time.Now().UTC(), buf.Bytes())
	...
	if err := emit(func(po *wal.PgOutput) error { return po.Begin(xid, commitLSN) }); err != nil {
		return fmt.Errorf("emit B: %w", err)
	}
	if err := emit(func(po *wal.PgOutput) error {
		return po.Change(wal.Change{Kind: wal.ChangeInsert, Rel: rel, NewTuple: tup})
	}); err != nil {
		return fmt.Errorf("emit I: %w", err)
	}
	if err := emit(func(po *wal.PgOutput) error { return po.Commit(xid, commitLSN) }); err != nil {
