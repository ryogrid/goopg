...
func notifyPayload(f protocol.Frame) (channel, payload string, ok bool) {
	if f.Type != protocol.MsgNotificationResponse || len(f.Payload) < 5 {
		return "", "", false
	}
...
func hasNotification(frames []protocol.Frame, channel, payload string) bool {
...
func drainSimple(t *testing.T, conn net.Conn, r *protocol.FrameReader) []protocol.Frame {
	t.Helper()
