...
func TestExtendedParseBindDescribeExecuteSyncSelectOne(t *testing.T) {
	...
	writeFrontendFrame(t, conn, protocol.MsgParse, parsePayload("", "SELECT 1", nil))
...
func TestExtendedBindUnknownStatementRequiresSync(t *testing.T) {
	...
	writeFrontendFrame(t, conn, protocol.MsgBind, bindPayload("", "missing", nil, nil, nil))
	writeFrontendFrame(t, conn, protocol.MsgParse, parsePayload("", "SELECT 1", nil)) // ignored until Sync
	writeFrontendFrame(t, conn, protocol.MsgSync, nil)
	...
	if frames[0].Type != protocol.MsgErrorResponse || frames[1].Type != protocol.MsgReadyForQuery {
		t.Fatalf("types=%q,%q want E,Z", frames[0].Type, frames[1].Type)
	}
	got := parseErrorFields(t, frames[0].Payload)
	if got[protocol.FieldSQLState] != string(sqlstate.InvalidSQLStatementName) {
		t.Fatalf("sqlstate=%q want %q", got[protocol.FieldSQLState], sqlstate.InvalidSQLStatementName)
	}
	...
	frames = readUntilReady(t, conn)
	want := []byte{protocol.MsgParseComplete, protocol.MsgBindComplete, protocol.MsgDataRow, protocol.MsgCommandComplete, protocol.MsgReadyForQuery}
	if len(frames) != len(want) {
		t.Fatalf("frames=%d want=%d", len(frames), len(want))
	}
	for i, w := range want {
...
func TestExtendedDescribeStatement(t *testing.T) {
	...
	writeFrontendFrame(t, conn, protocol.MsgParse, parsePayload("s1", "SELECT 1", nil))
	writeFrontendFrame(t, conn, protocol.MsgDescribe, describePayload('S', "s1"))
	writeFrontendFrame(t, conn, protocol.MsgSync, nil)

	frames := readUntilReady(t, conn)
	want := []byte{protocol.MsgParseComplete, protocol.MsgParameterDesc, protocol.MsgRowDescription, protocol.MsgReadyForQuery}
	if len(frames) != len(want) {
		t.Fatalf("frames=%d want=%d", len(frames), len(want))
	}
	for i, w := range want {
...
func TestExtendedBindParameterCountMismatch(t *testing.T) {
	...
	writeFrontendFrame(t, conn, protocol.MsgParse, parsePayload("p1", "SELECT $1", nil))
	writeFrontendFrame(t, conn, protocol.MsgBind, bindPayload("", "p1", nil, nil, nil))
	writeFrontendFrame(t, conn, protocol.MsgSync, nil)

	frames := readUntilReady(t, conn)
	want := []byte{protocol.MsgParseComplete, protocol.MsgErrorResponse, protocol.MsgReadyForQuery}
	if len(frames) != len(want) {
		t.Fatalf("frames=%d want=%d", len(frames), len(want))
	}
	for i, w := range want {
	...
	if got[protocol.FieldSQLState] != string(sqlstate.ProtocolViolation) {
		t.Fatalf("sqlstate=%q want %q", got[protocol.FieldSQLState], sqlstate.ProtocolViolation)
	}
}
