...
func TestSCRAMAuthSuccess(t *testing.T) {
	...
	if f.Type != protocol.MsgAuthentication ||
		binary.BigEndian.Uint32(f.Payload[:4]) != protocol.AuthenticationSASL {
	...
	if f.Type != protocol.MsgAuthentication ||
		binary.BigEndian.Uint32(f.Payload[:4]) != protocol.AuthenticationSASLCont {
	...
	if binary.BigEndian.Uint32(f.Payload[:4]) != protocol.AuthenticationSASLFinal {
	...
	if binary.BigEndian.Uint32(f.Payload) != protocol.AuthenticationOK {
