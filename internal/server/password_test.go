...
func writePasswordMessage(t *testing.T, conn net.Conn, password string) {
	...
	hdr := make([]byte, 5)
	hdr[0] = protocol.MsgPasswordMessage
...
func TestPasswordAuthSuccess(t *testing.T) {
	...
	if f.Type != protocol.MsgAuthentication ||
		binary.BigEndian.Uint32(f.Payload) != protocol.AuthenticationCleartextPasswd {
	...
	if f.Type != protocol.MsgAuthentication ||
		binary.BigEndian.Uint32(f.Payload) != protocol.AuthenticationOK {
...
func TestPasswordAuthWrongPasswordClosesWithFATAL(t *testing.T) {
	...
	if got[protocol.FieldSQLState] != string(sqlstate.InvalidAuthorizationSpecification) {
		t.Errorf("SQLSTATE = %q, want 28000", got[protocol.FieldSQLState])
	}
}
...
func TestMD5AuthSuccess(t *testing.T) {
	...
	if f.Type != protocol.MsgAuthentication ||
		binary.BigEndian.Uint32(f.Payload[:4]) != protocol.AuthenticationMD5Passwd {
...
func TestPasswordAuthUnknownUser(t *testing.T) {
	...
	got := decodeFields(t, f.Payload)
	if got[protocol.FieldSQLState] != string(sqlstate.InvalidAuthorizationSpecification) {
		t.Errorf("SQLSTATE = %q, want 28000 (no leak between unknown-user and wrong-password)", got[protocol.FieldSQLState])
	}
}
