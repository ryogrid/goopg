...
func TestPlanErrorPosition(t *testing.T) {
	fields := planErrorHintFields(&planner.PlanError{Code: "42803", Message: "x", Pos: 5})
	pos := ""
	...
	fields = planErrorHintFields(&planner.PlanError{Code: "42803", Message: "x"})
	for _, f := range fields {
		if f.Code == protocol.FieldPosition {
...
func TestNewPlannerExtendedQueryError(t *testing.T) {
	eq := newPlannerExtendedQueryError(&planner.PlanError{
		Code:    "42803",
	...
	if eq.Code != sqlstate.Code("42803") {
	...
	if eq.Code != sqlstate.FeatureNotSupported {
		t.Errorf("fallback Code = %q, want %q", eq.Code, sqlstate.FeatureNotSupported)
	}
	if eq.Message != "boom" {
