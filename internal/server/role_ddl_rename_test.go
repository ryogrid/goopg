...
func TestAlterRoleRenameSuccess(t *testing.T) {
	s := newTestRoleServer()
	if handled, err := s.tryHandleRoleDDL("CREATE ROLE foo LOGIN PASSWORD 'secret'"); !handled || err != nil {
	...
	handled, err := s.tryHandleRoleDDL("ALTER ROLE foo RENAME TO bar")
	if !handled || err != nil {
...
func TestAlterRoleRenameErrors(t *testing.T) {
	s := newTestRoleServer()
	if handled, err := s.tryHandleRoleDDL("CREATE ROLE foo LOGIN"); !handled || err != nil {
		t.Fatalf("CREATE ROLE foo: handled=%v err=%v", handled, err)
	}
	if handled, err := s.tryHandleRoleDDL("CREATE ROLE existing LOGIN"); !handled || err != nil {
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			handled, err := s.tryHandleRoleDDL(tc.sql)
			if !handled {
