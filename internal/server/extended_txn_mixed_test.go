...
// ReadyForQuery.
func mixedExtendedInsert(t *testing.T, conn net.Conn, r *protocol.FrameReader, name, id, label string) []protocol.Frame {
	t.Helper()
	writeFrontendFrame(t, conn, protocol.MsgParse, parsePayload(name, "INSERT INTO items VALUES ($1, $2)", nil))
	writeFrontendFrame(t, conn, protocol.MsgBind, bindPayload("", name, nil, []bindParam{{value: id}, {value: label}}, nil))
...
func mixedExtendedNullInsert(t *testing.T, conn net.Conn, r *protocol.FrameReader, name string) []protocol.Frame {
	t.Helper()
	writeFrontendFrame(t, conn, protocol.MsgParse, parsePayload(name, "INSERT INTO items VALUES ($1, $2)", nil))
	writeFrontendFrame(t, conn, protocol.MsgBind, bindPayload("", name, nil, []bindParam{{isNull: true}, {value: "x"}}, nil))
	writeFrontendFrame(t, conn, protocol.MsgExecute, executePayload("", 0))
	writeFrontendFrame(t, conn, protocol.MsgSync, nil)
	return drainToReady(t, r)
}
...
func TestM0132S8_StatusByteCoherentAcrossSwitch(t *testing.T) {
	t.Run("simple-begin-extended-dml-simple-commit", func(t *testing.T) {
		...
		if st := readyStatus(t, simpleStmt(t, c1, r1, "BEGIN")); st != byte(protocol.TxStatusInTransaction) {
			t.Fatalf("status after simple BEGIN = %q, want 'T'", st)
		}
		if st := readyStatus(t, mixedExtendedInsert(t, c1, r1, "s_ins", "1", "one")); st != byte(protocol.TxStatusInTransaction) {
			t.Errorf("status after in-block extended INSERT = %q, want 'T'", st)
		}
		if st := readyStatus(t, simpleStmt(t, c1, r1, "COMMIT")); st != byte(protocol.TxStatusIdle) {
	...
	t.Run("extended-begin-simple-dml-extended-commit", func(t *testing.T) {
		...
		if st := readyStatus(t, extendedStmt(t, c1, r1, "s_begin", "BEGIN")); st != byte(protocol.TxStatusInTransaction) {
			t.Fatalf("status after extended BEGIN = %q, want 'T'", st)
		}
		if st := readyStatus(t, simpleStmt(t, c1, r1, "INSERT INTO items VALUES (2, 'two')")); st != byte(protocol.TxStatusInTransaction) {
			t.Errorf("status after in-block simple INSERT = %q, want 'T'", st)
		}
		if st := readyStatus(t, extendedStmt(t, c1, r1, "s_commit", "COMMIT")); st != byte(protocol.TxStatusIdle) {
