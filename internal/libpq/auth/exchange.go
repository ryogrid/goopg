...
// than panicking.
func Exchange(d Decision, r *libpq.FrameReader, w *libpq.FrameWriter, store UserStore, user string) error {
	switch d.Method {
	case MethodTrust:
...
func runCleartext(d Decision, r *libpq.FrameReader, w *libpq.FrameWriter, store UserStore, user string) error {
	if store == nil {
		return ErrMethodUnsupported{Method: d.Method}
	}
...
func runMD5(d Decision, r *libpq.FrameReader, w *libpq.FrameWriter, store UserStore, user string) error {
	if store == nil {
		return ErrMethodUnsupported{Method: d.Method}
	}
	var salt [4]byte
...
func runSCRAM(d Decision, r *libpq.FrameReader, w *libpq.FrameWriter, store UserStore, user string) error {
	if store == nil {
		return ErrMethodUnsupported{Method: d.Method}
	}
...
// handles it).
func readSASLInitial(r *libpq.FrameReader) (string, []byte, error) {
	f, err := r.ReadFrame()
	if err != nil {
		return "", nil, ErrAuthExchange{Err: err}
	}
	if f.Type != libpq.MsgPasswordMessage {
...
func readSASLResponse(r *libpq.FrameReader) ([]byte, error) {
	f, err := r.ReadFrame()
	if err != nil {
		return nil, ErrAuthExchange{Err: err}
	}
...
func readPasswordMessage(r *libpq.FrameReader) (string, error) {
	f, err := r.ReadFrame()
	if err != nil {
