...
func TestM0132S13_PreparedPlanReuseReadsFreshData(t *testing.T) {
	...
	writeFrontendFrame(t, conn, libpq.MsgParse, parsePayload("q", "SELECT * FROM items", nil))
	...
	exec := func(wantRows int) {
		t.Helper()
		writeFrontendFrame(t, conn, libpq.MsgBind, bindPayload("", "q", nil, nil, nil))
...
func TestM0132S13_DescribeCachedAcrossExecutions(t *testing.T) {
	...
	writeFrontendFrame(t, conn, libpq.MsgParse, parsePayload("q", "SELECT id, label FROM items", nil))
	writeFrontendFrame(t, conn, libpq.MsgSync, nil)
	if f := drainToReady(t, r); hasError(f) {
		t.Fatalf("Parse errored: %+v", f)
	}
	...
	describe := func() int {
		t.Helper()
		writeFrontendFrame(t, conn, libpq.MsgDescribe, describePayload('S', "q"))
		writeFrontendFrame(t, conn, libpq.MsgSync, nil)
		frames := drainToReady(t, r)
		var nfields int
		for _, f := range frames {
			...
			if f.Type == libpq.MsgRowDescription {
...
func TestM0132S13_ParameterizedExecuteReuse(t *testing.T) {
	...
	for i := 1; i <= 3; i++ {
		writeFrontendFrame(t, conn, libpq.MsgBind, bindPayload("", "ins", nil, []bindParam{{value: strconv.Itoa(i)}, {value: "r" + strconv.Itoa(i)}}, nil))
		writeFrontendFrame(t, conn, libpq.MsgExecute, executePayload("", 0))
		writeFrontendFrame(t, conn, libpq.MsgSync, nil)
		if f := drainToReady(t, r); hasError(f) {
