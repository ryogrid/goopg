...
func copyCsvIn(t *testing.T, addr, sql, payload string) (frames string, tag string) {
	...
	writeFrontendFrame(t, conn, libpq.MsgCopyData, []byte(payload))
	...
	for _, f := range readUntilReady(t, conn) {
		...
		case libpq.MsgErrorResponse:
			tag = errorFieldValue(f.Payload, libpq.FieldMessage)
		}
	}
	return b.String(), tag
}
...
func TestCopyFromCsvErrors(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			...
			if !strings.Contains(frames, string(libpq.MsgErrorResponse)) {
