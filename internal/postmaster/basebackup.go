...
func (s *Server) replyBaseBackup(ctx context.Context, w *libpq.FrameWriter, args string) error {
	...
	if s.cfg.Checkpointer != nil {
		if err := s.cfg.Checkpointer.CheckpointNow(); err != nil {
			return s.writeQueryError(w, errcodes.InternalError,
	...
	if err != nil {
		return s.writeStreamingError(w, errcodes.InternalError,
...
func appendWALSegments(tw *tar.Writer, dataDir string, tli uint32, segSize int64, startSeg, endSeg uint64) error {
	...
	for _, de := range des {
		...
		name := de.Name()
		if _, segno, ok := xlog.ParseXLogFileName(name, segSize); ok {
	...
	if len(segs) == 0 || segs[0].segno != startSeg {
		return fmt.Errorf("could not find WAL file %q", xlog.XLogFileName(tli, startSeg, segSize))
	}
	for i := 1; i < len(segs); i++ {
		if segs[i].segno != segs[i-1].segno+1 {
			return fmt.Errorf("could not find WAL file %q", xlog.XLogFileName(tli, segs[i-1].segno+1, segSize))
		}
	}
	if segs[len(segs)-1].segno != endSeg {
		return fmt.Errorf("could not find WAL file %q", xlog.XLogFileName(tli, endSeg, segSize))
	}
...
func buildBackupLabel(label string, startLSN, ckptLSN uint64, tli uint32, segSize int64) []byte {
	...
	walFile := xlog.XLogFileName(tli, segno, segSize)
	var b bytes.Buffer
