...
func pubRespond(t *testing.T, rows []string) []byte {
	...
	if err := w.WriteReadyForQuery(libpq.TxStatusIdle); err != nil {
		t.Fatal(err)
	}
...
func TestRunTableSyncHappyPath(t *testing.T) {
	...
	rows, err := RunTableSync(TableSyncConfig{
		...
		From:    from,
		Reader:  libpq.NewFrameReader(bytes.NewReader(pubBytes)),
		Writer:  libpq.NewFrameWriter(&subOut),
	})
	if err != nil {
	...
	subFR := libpq.NewFrameReader(bytes.NewReader(subOut.Bytes()))
...
func TestRunTableSyncMultiRowFrame(t *testing.T) {
	...
	w.WriteReadyForQuery(libpq.TxStatusIdle)
	w.Flush()
	...
	rows, err := RunTableSync(TableSyncConfig{
		...
		Relname: "t",
		From:    from,
		Reader:  libpq.NewFrameReader(bytes.NewReader(pubOut.Bytes())),
		Writer:  libpq.NewFrameWriter(&subOut),
	})
	if err != nil {
		t.Fatalf("RunTableSync: %v", err)
...
func TestRunTableSyncAdvancesIToDOnCopyOutResponse(t *testing.T) {
	...
	w := libpq.NewFrameWriter(&pubOut)
	w.WriteCopyOutResponse(0, nil)
	w.WriteErrorResponse([]libpq.ErrorField{
		{Code: 'C', Value: "XX000"},
