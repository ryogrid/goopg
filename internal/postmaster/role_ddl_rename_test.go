...
func TestAlterRoleRenameErrors(t *testing.T) {
	...
	cases := []struct {
		name     string
		sql      string
		wantCode errcodes.Code
	}{
		{"role does not exist", "ALTER ROLE nosuchrole RENAME TO whatever", errcodes.UndefinedObject},
		{"new name already exists", "ALTER ROLE foo RENAME TO existing", errcodes.DuplicateObject},
		{"reserved pg_ prefix", "ALTER ROLE foo RENAME TO pg_reserved", errcodes.ReservedName},
		{"bootstrap superuser", "ALTER ROLE postgres RENAME TO newname", errcodes.FeatureNotSupported},
	}
	for _, tc := range cases {
