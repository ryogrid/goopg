...
func commitCopyTx(mgr *transam.Manager, tx transam.Transaction, asyncCommit bool) error {
	if asyncCommit {
...
func (s *Server) handleQueryOrCopy(ctx context.Context, r *libpq.FrameReader, w *libpq.FrameWriter, sess *misc.SessionRegistry, payload []byte, connTx *connTxState, prepStmts *preparedStatements) (*copyInState, error) {
	q, err := extractCString(payload)
	if err != nil {
		if err := s.writeQueryError(w, errcodes.ProtocolViolation,
	...
	default:
		if err := s.writeQueryError(w, errcodes.FeatureNotSupported,
...
func (s *Server) dispatchCopyViaExecutor(ctx context.Context, w *libpq.FrameWriter, sql string, connTx *connTxState, sess *misc.SessionRegistry) (*copyInState, error) {
	...
	if len(stmts) != 1 {
		if err := s.writeQueryError(w, errcodes.SyntaxError, "expected exactly one COPY statement"); err != nil {
			return nil, err
		}
		return nil, nil
	}
	node, err := optimizer.Plan(stmts[0], s.cfg.Catalog)
	if err != nil {
	...
	if !ok {
		...
		if err := s.writeQueryError(w, errcodes.FeatureNotSupported, "expected COPY statement"); err != nil {
			return nil, err
		}
		return nil, nil
	}
	...
	if connTx != nil && !connTx.InExplicit() {
		tx, err = s.cfg.TxnMgr.Begin(transam.IsolationReadCommitted, connTx.ProcNum)
	} else {
		tx, err = s.cfg.TxnMgr.Begin(transam.IsolationReadCommitted)
	}
	if err != nil {
		if err := s.writeQueryError(w, errcodes.SystemError, err.Error()); err != nil {
			return nil, err
		}
		return nil, nil
	}
	...
	_ = s.cfg.TxnMgr.Rollback(tx)
	if err := s.writeQueryError(w, errcodes.FeatureNotSupported, "unknown COPY direction"); err != nil {
		return nil, err
	}
	return nil, nil
}
...
func (s *Server) runInlineCopy(r *libpq.FrameReader, w *libpq.FrameWriter, ectx *executor.Context, cs *parser.CopyStmt) error {
	node, perr := optimizer.Plan(cs, s.cfg.Catalog)
	if perr != nil {
	...
	plan, ok := node.(*optimizer.Copy)
	if !ok {
		return s.writeQueryError(w, errcodes.FeatureNotSupported, "expected COPY statement")
	}
	...
	return s.writeQueryError(w, errcodes.FeatureNotSupported, "unknown COPY direction")
}
...
func (s *Server) runInlineCopyFromStdin(r *libpq.FrameReader, w *libpq.FrameWriter, ectx *executor.Context, plan *optimizer.Copy) error {
	from, err := executor.NewCopyFromExecutor(ectx, plan)
	if err != nil {
		return s.writeQueryError(w, execErrCode(err), execErrMsg(err))
	}
	...
	for {
		...
		case libpq.MsgCopyDone:
			...
			return w.WriteCommandComplete(fmt.Sprintf("COPY %d", from.RowsInserted()))
		case libpq.MsgCopyFail:
			msg, merr := extractCString(f.Payload)
			if merr != nil || msg == "" {
				msg = "COPY from stdin aborted by frontend"
			}
			return s.writeQueryError(w, errcodes.QueryCanceled, msg)
		case libpq.MsgFlush:
...
func (s *Server) runCopyToStream(w *libpq.FrameWriter, ctx *executor.Context, plan *optimizer.Copy) (count int64, errorSent bool, err error) {
...
func (s *Server) handleCopyToStdoutQuery(w *libpq.FrameWriter, matchable string) (*copyInState, error) {
...
func (s *Server) handleCopyInFrame(w *libpq.FrameWriter, st *copyInState, f libpq.Frame) (bool, error) {
	switch f.Type {
	case libpq.MsgCopyData:
		if st.fromExec != nil {
			var err error
			...
			if err != nil {
				...
				st.fromExec = nil
				if werr := w.WriteErrorResponse([]libpq.ErrorField{
					{Code: libpq.FieldSeverity, Value: "ERROR"},
					{Code: libpq.FieldSeverityNonLocal, Value: "ERROR"},
					{Code: libpq.FieldSQLState, Value: execErrCodeStr(err)},
					{Code: libpq.FieldMessage, Value: execErrMsg(err)},
					{Code: libpq.FieldRoutine, Value: "postmaster.handleCopyInFrame"},
				}); werr != nil {
					return true, werr
				}
	...
	case libpq.MsgCopyFail:
		...
		if err := w.WriteErrorResponse([]libpq.ErrorField{
			{Code: libpq.FieldSeverity, Value: "ERROR"},
			{Code: libpq.FieldSeverityNonLocal, Value: "ERROR"},
			{Code: libpq.FieldSQLState, Value: string(errcodes.QueryCanceled)},
			{Code: libpq.FieldMessage, Value: msg},
			{Code: libpq.FieldRoutine, Value: "postmaster.handleCopyInFrame"},
		}); err != nil {
			return true, err
	...
	default:
		...
		if err := w.WriteErrorResponse([]libpq.ErrorField{
			{Code: libpq.FieldSeverity, Value: "ERROR"},
			{Code: libpq.FieldSeverityNonLocal, Value: "ERROR"},
			{Code: libpq.FieldSQLState, Value: string(errcodes.ProtocolViolation)},
			{Code: libpq.FieldMessage, Value: fmt.Sprintf("unexpected message %q in COPY FROM STDIN", f.Type)},
			{Code: libpq.FieldRoutine, Value: "postmaster.handleCopyInFrame"},
		}); err != nil {
			return true, err
		}
...
func syntaxErrorCode(err error) errcodes.Code {
	var se *parser.SyntaxError
	if errors.As(err, &se) && se.Code != "" {
		return errcodes.Code(se.Code)
	}
	return errcodes.SyntaxError
}
...
func syntaxErrorMsg(err error) (msg string, extra []libpq.ErrorField) {
	var se *parser.SyntaxError
	if errors.As(err, &se) {
		msg = se.Error()
...
// *PlanError.
func planErrorFields(err error) (errcodes.Code, string) {
	var pe *optimizer.PlanError
	if errors.As(err, &pe) {
		return errcodes.Code(pe.Code), pe.Message
	}
	return errcodes.FeatureNotSupported, err.Error()
}
...
func planErrorHintFields(err error) []libpq.ErrorField {
	var pe *optimizer.PlanError
	if !errors.As(err, &pe) {
		return nil
	}
	...
	if pe.Detail != "" {
		fields = append(fields, libpq.ErrorField{Code: libpq.FieldDetail, Value: pe.Detail})
	}
	if pe.Hint != "" {
		fields = append(fields, libpq.ErrorField{Code: libpq.FieldHint, Value: pe.Hint})
	}
	// PlanError.Pos is 0-based (0 = unset); wire protocol FieldPosition is 1-based.
	if pe.Pos > 0 {
		fields = append(fields, libpq.ErrorField{Code: libpq.FieldPosition, Value: strconv.Itoa(pe.Pos + 1)})
	}
	return fields
}

func execErrCode(err error) errcodes.Code {
	var ee *executor.ExecError
	if errors.As(err, &ee) && ee.Code != "" {
		return errcodes.Code(ee.Code)
	}
...
func execErrDetailFields(err error) []libpq.ErrorField {
	var ee *executor.ExecError
	if !errors.As(err, &ee) {
		return nil
	}
	...
	if ee.Hint != "" {
		fields = append(fields, libpq.ErrorField{Code: libpq.FieldHint, Value: ee.Hint})
	}
	if ee.Context != "" {
		fields = append(fields, libpq.ErrorField{Code: libpq.FieldWhere, Value: ee.Context})
	}
...
func newPlannerExtendedQueryError(err error) *extendedQueryError {
	...
	return &extendedQueryError{Code: errcodes.FeatureNotSupported, Message: err.Error()}
}
