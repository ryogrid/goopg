...
func databaseDDLErrorSQLState(err error) errcodes.Code {
...
func (s *Server) resolveCreateDatabaseTemplate(templateName string) (srcOid uint32, tables []*catalog.Table, sequences []executor.SeqInfo, views []*catalog.Table, matViews []*catalog.Table, err error) {
	...
	if !ok {
		return 0, nil, nil, nil, nil, &databaseDDLError{
			code: errcodes.UndefinedDatabase,
...
func (s *Server) walLogRelmapForNewDatabase(oid uint32) {
	...
	payload, err := xlog.EncodeRelmapUpdatePG(oid, pgDefaultTablespace, image)
	if err != nil {
		return
	}
...
func (s *Server) walLogRelfileFPIs(dbOid, relOid uint32) {
	...
	for off := 0; off+storage.BlockSize <= len(data); off += storage.BlockSize {
		...
		rec, encErr := xlog.EncodePageImagePG(rel, blk, page)
		if encErr != nil {
			return
		}
...
func (s *Server) copyTemplateTables(srcOid, newOid uint32, tables []*catalog.Table) error {
	...
	for _, srcTbl := range tables {
		...
		// B4.6 Stage 2: PRESERVE the template's relation OID in the copy (real
		// PG semantics — CREATE DATABASE copies relations keeping their OIDs;
		// only the database OID is new). The new table shares srcTbl.OID but
		// lives in newOid's namespace (base/<newOid>/<srcOID>), so the same
		// relOid now legitimately appears in two databases — the catalog
		// namespace layer and the (dbOid,relOid)-keyed storage isolate them.
		newTbl := &catalog.Table{
			Schema:  srcTbl.Schema,
			Name:    srcTbl.Name,
			Columns: cols,
			OID:     srcTbl.OID,
			DBOid:   newOid,
		}
		if err := im.TryRegisterUserTable(newTbl, newOid); err != nil {
...
func (s *Server) tryHandleDatabaseDDL(sql string, liveDBName string, actingRole string, resolveCurrent currentGUCResolver) (bool, string, error) {
	...
	if name == "" {
		return true, "", &databaseDDLError{code: errcodes.SyntaxError, msg: "missing database name"}
	}
	...
	case databaseDDLCreate:
		...
		if rawEnc, ok := extractEncodingRawFromSQL(sql); ok {
			...
			if dbEncodingID < 0 {
				return true, "", &databaseDDLError{
					code: errcodes.InvalidParameterValue,
		...
		if err != nil {
			if errors.Is(err, catalog.ErrDatabaseExists) {
				...
				return true, "", &databaseDDLError{
					code: errcodes.DuplicateDatabase,
		...
		if s.cfg.WAL != nil {
			if rec, encErr := xlog.EncodeDbaseCreateWalLogPG(oid, catalog.DefaultTablespaceOID, 0); encErr == nil {
	...
	case databaseDDLDrop:
		...
		if s.cfg.WAL != nil {
			...
			rec, encErr := xlog.EncodeDbaseDropPG(droppedOid, []uint32{catalog.DefaultTablespaceOID}, 0)
			if encErr == nil {
...
func (s *Server) handleDatabaseDDLBypass(sql, liveDBName, actingRole string, resolveCurrent currentGUCResolver, w *libpq.FrameWriter) (handled bool, err error) {
	...
	if notice != "" {
		if err := w.WriteNoticeResponse([]libpq.ErrorField{
			...
			{Code: libpq.FieldMessage, Value: notice},
		}); err != nil {
			return true, err
		}
	}
