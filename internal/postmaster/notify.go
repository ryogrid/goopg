...
type notifyHub struct {
	...
	listeners map[string]map[*misc.SessionRegistry]struct{}
...
func newNotifyHub() *notifyHub {
	return &notifyHub{
		listeners: make(map[string]map[*misc.SessionRegistry]struct{}),
		pending:   make(map[*misc.SessionRegistry][]Notification),
	}
}
...
func (h *notifyHub) Listen(sess *misc.SessionRegistry, channel string) {
	if sess == nil {
		return
	}
	h.mu.Lock()
	...
	if set == nil {
		set = make(map[*misc.SessionRegistry]struct{})
...
func (h *notifyHub) Unlisten(sess *misc.SessionRegistry, channel string) {
	if sess == nil {
		return
	}
	h.mu.Lock()
...
func (h *notifyHub) UnlistenAll(sess *misc.SessionRegistry) {
	if sess == nil {
		return
	}
	h.mu.Lock()
...
func (h *notifyHub) DrainPending(sess *misc.SessionRegistry) []Notification {
	if sess == nil {
		return nil
	}
	h.mu.Lock()
...
func (s *Server) execNotifyStmt(w *libpq.FrameWriter, stmt parser.Stmt, connTx *connTxState) (bool, error) {
...
func (s *Server) deliverNotifications(w *libpq.FrameWriter, connTx *connTxState) error {
	if connTx == nil {
		return nil
	}
...
func (h *notifyHub) RemoveSession(sess *misc.SessionRegistry) {
	if sess == nil {
		return
	}
	h.mu.Lock()
