...
func TestLogDurationEmitsCombinedOrBareLine(t *testing.T) {
	...
	*records = nil
	...
	disabledSess := misc.NewSessionRegistry(misc.BuildDefaultRegistry())
	*records = nil
	...
	thresholdSess := misc.NewSessionRegistry(misc.BuildDefaultRegistry())
	if err := sess.Set("log_min_duration_statement", "0", false); err != nil {
		t.Fatalf("Set: %v", err)
	}
...
func TestServerLogLinePrefix(t *testing.T) {
	...
	sess := misc.NewSessionRegistry(reg)
	connTx := &connTxState{DBName: "wordpress", BackendPID: 777}

	if err := reg.Set("log_line_prefix", "[%p %d] ", misc.SourceConfigFile); err != nil {
		t.Fatalf("Set: %v", err)
	}
	*records = nil
	...
	if err := reg.Set("log_line_prefix", "", misc.SourceConfigFile); err != nil {
		t.Fatalf("Set: %v", err)
	}
	*records = nil
	s.logStatement("simple", "SELECT 1", sess, connTx)
