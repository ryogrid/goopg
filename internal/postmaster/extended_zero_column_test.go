...
func TestExtendedZeroColumnSelectEmitsRows(t *testing.T) {
	...
	writeFrontendFrame(t, conn, libpq.MsgParse, parsePayload("z", "SELECT FROM items", nil))
	writeFrontendFrame(t, conn, libpq.MsgBind, bindPayload("", "z", nil, nil, nil))
...
func TestExtendedZeroColumnSelectWithFilter(t *testing.T) {
	...
	writeFrontendFrame(t, conn, libpq.MsgParse, parsePayload("z2", "SELECT FROM items WHERE id >= 2", nil))
	writeFrontendFrame(t, conn, libpq.MsgBind, bindPayload("", "z2", nil, nil, nil))
	writeFrontendFrame(t, conn, libpq.MsgExecute, executePayload("", 0))
	writeFrontendFrame(t, conn, libpq.MsgSync, nil)
