...
func startRetTestServer(t *testing.T, dir string) *retTestServer {
	...
	rt.Checkpointer.SetRetainer(&xlog.SlotAwareRetainer{
		Writer: rt.WAL,
		Slots:  rt.Slots,
	})
	...
	for _, f := range readUntilReady(t, conn) {
		if f.Type == libpq.MsgErrorResponse {
			t.Fatalf("query %q: %s", sql,
				parseErrorFields(t, f.Payload)[libpq.FieldMessage])
		}
	}
}
...
func retSeedRows(t *testing.T, addr string, startID, count int) {
	...
	for len(data) > 0 {
		...
		writeFrontendFrame(t, conn, libpq.MsgCopyData, data[:n])
		data = data[n:]
	}
...
func requireSeg0Missing(t *testing.T, dir string) {
	t.Helper()
	seg0 := filepath.Join(dir, "pg_wal", xlog.XLogFileName(1, 0, xlog.DefaultSegmentSize))
