...
type connTxState struct {
	...
	NotifySession *misc.SessionRegistry
...
func (c *connTxState) ReleasePinnedSnapshotOnFail(mgr *transam.Manager) {
	if mgr == nil {
		return
	}
	c.mu.Lock()
...
func (c *connTxState) AbortInPlaceOnFail(mgr *transam.Manager) {
	if mgr == nil {
		return
	}
	c.mu.Lock()
...
func (c *connTxState) Begin(tx transam.Transaction) {
	c.mu.Lock()
	c.active = true
	c.failed = false
	c.tx = tx
	...
	c.sess.BeginExplicitTransaction(tx, transam.Snapshot{})
	c.mu.Unlock()
}
...
func (c *connTxState) wireStatus(afterError bool) libpq.TransactionStatus {
	if c == nil {
		return libpq.TxStatusIdle
	}
	c.mu.Lock()
	defer c.mu.Unlock()
	...
	if c.failed || afterError {
		return libpq.TxStatusInFailedTransaction
	}
	return libpq.TxStatusInTransaction
}
...
func (c *connTxState) Tx() transam.Transaction {
	c.mu.Lock()
	defer c.mu.Unlock()
	if c.sess != nil {
...
func (c *connTxState) DetachPrepared(gid string, newTx transam.Transaction) *connTxState {
	c.mu.Lock()
	defer c.mu.Unlock()
	p := &connTxState{
		active:                   true,
