...
// next read.
func readUntilReady(t *testing.T, conn net.Conn) []libpq.Frame {
	t.Helper()
	r := libpq.NewFrameReader(conn)
...
func TestSelectOne(t *testing.T) {
	...
	if frames[3].Type != libpq.MsgReadyForQuery || frames[3].Payload[0] != 'I' {
...
func TestSelectOneCaseAndSemicolon(t *testing.T) {
	...
	for _, q := range []string{"select 1", "SELECT 1;", "  select 1 ;  "} {
		...
		frames := readUntilReady(t, conn)
		conn.Close()
		if len(frames) != 4 || frames[0].Type != libpq.MsgRowDescription {
...
func TestUnsupportedQueryProducesErrorResponse(t *testing.T) {
	...
	if got[libpq.FieldSQLState] != string(errcodes.FeatureNotSupported) {
		t.Errorf("SQLSTATE = %q, want %q", got[libpq.FieldSQLState], errcodes.FeatureNotSupported)
	}
	if got[libpq.FieldSeverity] != "ERROR" {
		t.Errorf("severity = %q, want ERROR", got[libpq.FieldSeverity])
	}
}
...
func TestEmptyQueryReturnsEmptyQueryResponse(t *testing.T) {
	...
	if frames[0].Type != libpq.MsgEmptyQueryResponse {
...
func TestConnectionStaysAliveAfterError(t *testing.T) {
	...
	if first[0].Type != libpq.MsgErrorResponse {
		t.Fatalf("first frame = %c, want E", first[0].Type)
	}
	writeQuery(t, conn, "SELECT 1")
	second := readUntilReady(t, conn)
	if second[0].Type != libpq.MsgRowDescription {
