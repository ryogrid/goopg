...
func TestDatabaseDDLErrorSQLState(t *testing.T) {
	...
	if got := databaseDDLErrorSQLState(err); got != errcodes.DuplicateDatabase {
		t.Errorf("databaseDDLErrorSQLState(duplicate create) = %q, want %q", got, errcodes.DuplicateDatabase)
	}
	...
	if got := databaseDDLErrorSQLState(err); got != errcodes.UndefinedDatabase {
		t.Errorf("databaseDDLErrorSQLState(undefined drop) = %q, want %q", got, errcodes.UndefinedDatabase)
	}
	...
	if got := databaseDDLErrorSQLState(err); got != errcodes.UndefinedObject {
		t.Errorf("databaseDDLErrorSQLState(unrecognized GUC) = %q, want %q", got, errcodes.UndefinedObject)
	}

	// A WAL-append-shaped internal error (not a databaseDDLError) still
	// falls back to the generic sqlstate.SystemError.
	if got := databaseDDLErrorSQLState(errUnwrappedForTest); got != errcodes.SystemError {
		t.Errorf("databaseDDLErrorSQLState(plain error) = %q, want %q", got, errcodes.SystemError)
	}
}
...
func TestTryHandleDatabaseDDLDropRequiresOwnership(t *testing.T) {
	...
	if got := databaseDDLErrorSQLState(err); got != errcodes.InsufficientPrivilege {
		t.Errorf("DROP DATABASE aliceonly (as bob): sqlstate = %q, want %q", got, errcodes.InsufficientPrivilege)
	}
...
func TestTryHandleDatabaseDDLDropGuards(t *testing.T) {
	...
	for _, name := range []string{"template0", "template1"} {
		...
		if got := databaseDDLErrorSQLState(err); got != errcodes.WrongObjectType {
			t.Errorf("DROP DATABASE %s: sqlstate = %q, want %q", name, got, errcodes.WrongObjectType)
		}
	...
	if got := databaseDDLErrorSQLState(err); got != errcodes.ObjectInUse {
		t.Errorf("DROP DATABASE postgres (self): sqlstate = %q, want %q", got, errcodes.ObjectInUse)
	}
	...
	if got := databaseDDLErrorSQLState(err); got != errcodes.ObjectInUse {
		t.Errorf("DROP DATABASE tpch (busy): sqlstate = %q, want %q", got, errcodes.ObjectInUse)
	}
...
func TestTryHandleDatabaseDDLCreateTemplateDoesNotExistErrors(t *testing.T) {
	...
	if got := databaseDDLErrorSQLState(err); got != errcodes.UndefinedDatabase {
		t.Errorf("SQLSTATE = %q, want %q", got, errcodes.UndefinedDatabase)
	}
...
func TestTryHandleDatabaseDDLCreateTemplateWithIndexErrors(t *testing.T) {
	...
	if got := databaseDDLErrorSQLState(err); got != errcodes.FeatureNotSupported {
		t.Errorf("SQLSTATE = %q, want %q", got, errcodes.FeatureNotSupported)
	}
	if im.HasDatabase("foo") {
