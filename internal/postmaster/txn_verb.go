...
func noTransactionInProgressNotice() []libpq.ErrorField {
	return []libpq.ErrorField{
		...
		{Code: libpq.FieldMessage, Value: "there is no transaction in progress"},
	}
}
...
func verbWarnFields(out txnVerbOutcome) []libpq.ErrorField {
	if !out.Warn {
		return nil
	}
...
func (s *Server) applyTransactionVerb(ctx *executor.Context, connTx *connTxState, txNode *optimizer.Transaction, autoCommitPtr *bool) txnVerbOutcome {
	switch txNode.Verb {
		...
		if connTx != nil && !connTx.InExplicit() && autoCommitPtr != nil {
			...
			*autoCommitPtr = false
			...
			if txNode.IsolationLevel != "" {
				parsedLvl, perr := transam.ParseIsolationLevel(txNode.IsolationLevel)
				if perr != nil {
					return txnVerbOutcome{Handled: true, Err: &txnVerbError{Code: errcodes.SyntaxError, Msg: perr.Error()}}
				}
				if parsedLvl != ctx.Tx.Isolation {
					...
					if berr != nil {
						return txnVerbOutcome{Handled: true, Err: &txnVerbError{Code: errcodes.SystemError, Msg: berr.Error()}}
					}
					ctx.Tx = newTx
					...
					if parsedLvl == transam.IsolationReadCommitted {
						snap, serr := s.cfg.TxnMgr.SnapshotFor(newTx)
						if serr != nil {
							_ = s.cfg.TxnMgr.Rollback(newTx)
							return txnVerbOutcome{Handled: true, Err: &txnVerbError{Code: errcodes.SystemError, Msg: serr.Error()}}
						}
						ctx.Snap = snap
					}
				}
		...
		if connTx != nil && connTx.InExplicit() {
			...
			if sess := connTx.Session(); sess != nil {
				...
				if deferErr != nil {
					...
					s.endExplicitBlock(ctx, connTx, true)
					code := errcodes.ForeignKeyViolation
			...
			if explicitTx.Isolation == transam.IsolationSerializable && explicitTx.Handle != 0 {
				if ssiErr := s.cfg.TxnMgr.PreCommitCheckForSerializationFailure(explicitTx.Handle); ssiErr != nil {
					...
					var ssiFields []libpq.ErrorField
					if sfe, ok := ssiErr.(*transam.SerializationFailureError); ok {
						if d := sfe.Detail(); d != "" {
							ssiFields = append(ssiFields,
								libpq.ErrorField{Code: libpq.FieldDetail, Value: d},
								libpq.ErrorField{Code: libpq.FieldHint, Value: "The transaction might succeed if retried."})
						}
					}
			...
			if err := ctx.CommitTransaction(explicitTx); err != nil {
				s.endExplicitBlock(ctx, connTx, true)
				return txnVerbOutcome{Handled: true, Err: &txnVerbError{Code: errcodes.SystemError, Msg: err.Error()}}
			}
