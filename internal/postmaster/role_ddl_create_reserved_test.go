...
func TestCreateRoleRejectsReservedPgPrefix(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.sql, func(t *testing.T) {
			...
			if got := roleErrorSQLState(err); got != errcodes.ReservedName {
				t.Errorf("%s: SQLSTATE = %q, want %q (err=%v)", tc.sql, got, errcodes.ReservedName, err)
			}
			...
			if len(fields) != 1 || fields[0].Code != libpq.FieldDetail || fields[0].Value != wantDetail {
