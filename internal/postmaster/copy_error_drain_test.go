...
func TestCopyFromStdinErrorDrainsTrailingFrames(t *testing.T) {
	...
	writeFrontendFrame(t, conn, libpq.MsgCopyData, []byte("notanint\tx\n"))
	...
	writeFrontendFrame(t, conn, libpq.MsgCopyData, []byte("1\ty\n"))
...
func TestCopyFailAfterCopyEndedIsIgnored(t *testing.T) {
	...
	writeQuery(t, conn, "COPY items FROM STDIN")
	writeFrontendFrame(t, conn, libpq.MsgCopyData, []byte("1\tok\n"))
	writeFrontendFrame(t, conn, libpq.MsgCopyDone, nil)
	frames := readUntilReady(t, conn)
	if len(frames) != 3 || frames[1].Type != libpq.MsgCommandComplete {
		t.Fatalf("frames=%q want CopyInResponse/CommandComplete/RFQ", frameTypes(frames))
	}

	writeFrontendFrame(t, conn, libpq.MsgCopyFail, cstring("too late"))
