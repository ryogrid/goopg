...
func halfDeadTestLeaf(t *testing.T, prev, next storage.BlockNumber) storage.Page {
	...
	nbtree.WritePGOpaque(page, nbtree.PGBTPageOpaque{Prev: prev, Next: next, Flags: nbtree.BTPLeaf})
	return page
}
...
func halfDeadTestParent(t *testing.T, children []storage.BlockNumber) storage.Page {
	...
	nbtree.WritePGOpaque(page, nbtree.PGBTPageOpaque{Prev: nbtree.PNone, Next: 77, Level: 1})
	raws := [][]byte{nbtree.PGBTPivotRaw([]byte("high-key"), nbtree.PNone)}
	for i, child := range children {
		key := []byte{byte('a' + i)}
		if i == 0 {
			key = nil // minus infinity
		}
		raws = append(raws, nbtree.PGBTPivotRaw(key, child))
	}
...
func TestApplyRecordReplaysPGBtreeMarkPageHalfDead(t *testing.T) {
	...
	leaf := halfDeadTestLeaf(t, nbtree.PNone, 7)
	...
	op := nbtree.ReadPGOpaque(gotLeaf)
	if op.Flags != nbtree.BTPHalfDead|nbtree.BTPLeaf {
		t.Errorf("replayed leaf flags = %#x, want BTP_HALF_DEAD|BTP_LEAF (%#x)", op.Flags, nbtree.BTPHalfDead|nbtree.BTPLeaf)
	}
	if op.Prev != nbtree.PNone || op.Next != 7 || op.Level != 0 {
	...
	if len(hikey) != nbtree.SizeOfIndexTupleData {
		t.Errorf("dummy high key is %d bytes, want %d", len(hikey), nbtree.SizeOfIndexTupleData)
	}
	if got := nbtree.BTreeTupleGetDownLink(hikey); got != storage.InvalidBlockNumber {
	...
	if got := nbtree.BTreeTupleGetDownLink(first); got != 7 {
		t.Errorf("parent offset 2 downlink = %d, want 7 (the right neighbour's child)", got)
	}
	if len(first) != nbtree.SizeOfIndexTupleData {
	...
	if got := nbtree.BTreeTupleGetDownLink(last); got != 8 {
