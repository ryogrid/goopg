...
func TestTimelineHistoryRoundTrip(t *testing.T) {
	...
	if err := xlog.WriteHistory(dir, 4, want); err != nil {
		t.Fatalf("WriteHistory: %v", err)
	}

	got, err := xlog.ReadHistory(dir, 4)
	if err != nil {
...
func TestTimelineHistoryFileName(t *testing.T) {
	...
	for _, c := range cases {
		if got := xlog.TimelineHistoryFileName(c.tli); got != c.want {
...
func TestReadHistoryMissingFile(t *testing.T) {
	dir := t.TempDir()
	got, err := xlog.ReadHistory(dir, 1)
	if err != nil {
...
func TestWriteHistoryFormat(t *testing.T) {
	dir := t.TempDir()
	entries := []xlog.TimelineHistoryEntry{
		{TLI: 1, SwitchLSN: 0x0000000118000000, Reason: "no recovery target specified"},
	}
	if err := xlog.WriteHistory(dir, 2, entries); err != nil {
		t.Fatalf("WriteHistory: %v", err)
	}
...
func TestReadHistoryToleratesCommentAndBlankLines(t *testing.T) {
	...
	got, err := xlog.ReadHistory(dir, 3)
	if err != nil {
		t.Fatalf("ReadHistory: %v", err)
	}
	want := []xlog.TimelineHistoryEntry{
