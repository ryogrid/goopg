...
func pgoDecodePhysicalValue(t catalog.Type, data []byte, regOut func(string, uint32) string) ([]byte, int, error) {
	...
	if t.IsArray {
		...
		text, err := array.RenderTextStyled(t.Name, payload, array.OutputStyle{RegOut: regOut})
		if err != nil {
			return nil, 0, err
		}
	...
	case "interval":
		...
		return []byte(datetime.FormatInterval(months, days, micros)), 16, nil
	case "uuid":
	...
	case "numeric", "decimal":
		...
		s, err := nodes.NumericTextFromStoredPayload(payload)
		if err != nil {
			return nil, 0, err
		}
		return []byte(s), n, nil
