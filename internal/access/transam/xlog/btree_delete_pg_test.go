...
func buildBtreeDeletePayload(deleted []uint16, updates []nbtree.PostingUpdate) []byte {
	var data []byte
	put := func(v uint16) {
		var b [2]byte
...
func buildBtreeDeletePG(t *testing.T, rel storage.RelFileNode, blk storage.BlockNumber,
	deleted []uint16, updates []nbtree.PostingUpdate,
) []byte {
	t.Helper()
...
func TestApplyRecordReplaysPGBtreeDelete(t *testing.T) {
	...
	page := btreeLeafPageWith(t,
		...
		nbtree.PGBTItemRaw([]byte("bbb"), tid(7, 20)),
		nbtree.PGBTItemRaw([]byte("ccc"), tid(7, 30)),
		nbtree.PGBTItemRaw([]byte("ddd"), tid(7, 40)),
		nbtree.PGBTItemRaw([]byte("eee"), tid(7, 50)),
	)
	op := nbtree.ReadPGOpaque(page)
	op.Flags |= nbtree.BTPHasGarbage
	nbtree.WritePGOpaque(page, op)
	...
	if got := nbtree.ReadPGOpaque(leaf); got.Flags&nbtree.BTPHasGarbage != 0 {
...
func TestApplyRecordReplaysPGBtreeDeleteWithPostingUpdates(t *testing.T) {
	...
	btreeSeedRel(t, mgr, rel, btreeLeafPageWith(t,
		nbtree.PGBTItemRaw([]byte("aaa"), tid(7, 10)),
		nbtree.PGBTItemRaw([]byte("aab"), tid(7, 20)),
		posting,
	...
	framed := buildBtreeDeletePG(t, rel, 0, []uint16{2},
		[]nbtree.PostingUpdate{{Offset: 3, DeleteTIDs: []uint16{1, 3}}})
	...
	if !nbtree.BTreeTupleIsPosting(rewritten) {
...
func TestApplyRecordReplaysPGBtreeDeleteCollapsesPostingToPlainTuple(t *testing.T) {
	...
	base := nbtree.PGBTItemRaw([]byte("bbb"), tid(8, 10))
	btreeSeedRel(t, mgr, rel, btreeLeafPageWith(t,
		nbtree.PGBTPostingRaw(base, []storage.ItemPointer{tid(8, 10), tid(8, 20)}),
		nbtree.PGBTItemRaw([]byte("ccc"), tid(9, 10)),
	))

	framed := buildBtreeDeletePG(t, rel, 0, nil,
		[]nbtree.PostingUpdate{{Offset: 1, DeleteTIDs: []uint16{0}}})
	...
	if nbtree.BTreeTupleIsPosting(got) {
		t.Fatal("the single survivor is still flagged as a posting list")
	}
	if ptr := nbtree.PGIndexTupleTID(got); ptr != tid(8, 20) {
	...
	if next, err := storage.PageGetItemRaw(leaf, 2); err != nil {
		t.Fatal(err)
	} else if ptr := nbtree.PGIndexTupleTID(next); ptr != tid(9, 10) {
...
func TestApplyRecordReplaysPGBtreeVacuumWithPostingUpdates(t *testing.T) {
	...
	btreeSeedRel(t, mgr, rel, btreeLeafPageWith(t,
		nbtree.PGBTItemRaw([]byte("aaa"), tid(7, 10)),
		nbtree.PGBTPostingRaw(base, []storage.ItemPointer{tid(8, 10), tid(8, 20), tid(8, 30)}),
	))

	framed := buildBtreeDeleteOpPG(t, xlogBtreeVacuum, rel, 0, []uint16{1},
		[]nbtree.PostingUpdate{{Offset: 2, DeleteTIDs: []uint16{1}}})
...
func TestApplyRecordReplaysPGBtreeDeleteFromImage(t *testing.T) {
	...
	after := btreeLeafPageWith(t, nbtree.PGBTItemRaw([]byte("bbb"), tid(7, 20)))
...
func TestApplyRecordRefusesPGBtreeDeleteMalformed(t *testing.T) {
	...
	t.Run("update-deletes-every-tid", func(t *testing.T) {
		...
		refuse(t, buildBtreeDeletePG(t, rel, 0, nil,
			[]nbtree.PostingUpdate{{Offset: 2, DeleteTIDs: []uint16{0, 1}}}),
			"leaving none")
	})
	...
	t.Run("tid-offset-past-the-posting", func(t *testing.T) {
		seed()
		refuse(t, buildBtreeDeletePG(t, rel, 0, nil,
			[]nbtree.PostingUpdate{{Offset: 2, DeleteTIDs: []uint16{5}}}),
