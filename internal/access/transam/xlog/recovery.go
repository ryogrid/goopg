...
func replayDecodedXLogBtreeInsert(mgr *storage.Manager, r Record, xlog *XLogDecodedRecord, isleaf, ismeta, posting bool) error {
	...
	if !isleaf {
		...
		} else if err := replayExistingXLogBlock(mgr, child, endLSN, nbtree.ReplayClearIncompleteSplit); err != nil {
	...
	if block.HasImage && block.ImageApply {
		...
		apply := func(page storage.Page) error {
			return nbtree.ApplyInsertRecordAt(page, block.Data, offnum)
		}
		if posting {
			apply = func(page storage.Page) error {
				return nbtree.ApplyInsertPostingRecordAt(page, block.Data, offnum)
			}
		}
...
func replayDecodedXLogBtreeDedup(mgr *storage.Manager, r Record, xlog *XLogDecodedRecord) error {
	...
	if want := nintervals * nbtree.SizeOfDedupInterval; len(block.Data) < want {
		return fmt.Errorf("wal: xlog btree-dedup: block 0 data len %d for %d intervals (want >= %d)",
			len(block.Data), nintervals, want)
	}
	intervals := make([]nbtree.DedupInterval, nintervals)
	for i := range intervals {
		off := i * nbtree.SizeOfDedupInterval
		intervals[i] = nbtree.DedupInterval{
	...
	if err := replayExistingXLogBlock(mgr, block, endLSN, func(page storage.Page) error {
		return nbtree.ReplayDedupPage(page, intervals)
	}); err != nil {
...
func replayDecodedXLogBtreeRestoreMeta(mgr *storage.Manager, xlog *XLogDecodedRecord, endLSN storage.LSN, blockID byte, what string) error {
	...
	if err := replayInitedXLogBlock(mgr, meta, endLSN, func(page storage.Page) error {
		return nbtree.ReplayRestoreMetaPage(page, md)
	}); err != nil {
...
func replayDecodedXLogBtreeNewRoot(mgr *storage.Manager, r Record, xlog *XLogDecodedRecord) error {
	...
	if root.HasImage && root.ImageApply {
		...
		var items [][]byte
		if level > 0 {
			parsed, err := nbtree.PGParseRestorePageData(root.Data)
			if err != nil {
		...
		if err := replayInitedXLogBlock(mgr, root, endLSN, func(page storage.Page) error {
			return nbtree.ReplayNewRootPage(page, level, items)
		}); err != nil {
...
func replayDecodedXLogBtreeMarkPageHalfDead(mgr *storage.Manager, r Record, xlog *XLogDecodedRecord) error {
	...
	if parent.HasImage && parent.ImageApply {
		...
		return nbtree.ReplayHalfDeadParent(page, poffset)
	}); err != nil {
	...
	if err := replayInitedXLogBlock(mgr, leaf, endLSN, func(page storage.Page) error {
		return nbtree.ReplayMarkHalfDeadLeaf(page, leftblk, rightblk, topparent)
	}); err != nil {
...
func replayDecodedXLogBtreeUnlinkPage(mgr *storage.Manager, r Record, xlog *XLogDecodedRecord, isMeta bool) error {
	...
	if leftsib != nbtree.PNone {
		...
		if left.HasImage && left.ImageApply {
			...
			return nbtree.ReplayUnlinkLeftSibling(page, rightsib)
		}); err != nil {
	...
	if target.HasImage && target.ImageApply {
		...
		return nbtree.ReplayUnlinkTargetPage(page, leftsib, rightsib, level, safexid)
	}); err != nil {
	...
	if right.HasImage && right.ImageApply {
		...
		return nbtree.ReplayUnlinkRightSibling(page, leftsib)
	}); err != nil {
	...
	if leaf, ok := xlogBlockRefByID(xlog, 3); ok {
		if leaf.HasImage && leaf.ImageApply {
			...
			// levels.
			return nbtree.ReplayMarkHalfDeadLeaf(page, leafleftsib, leafrightsib, leaftopparent)
		}); err != nil {
...
func replayDecodedXLogBtreeSplit(mgr *storage.Manager, r Record, xlog *XLogDecodedRecord, newItemOnLeft bool) error {
	...
	if right.HasImage && right.ImageApply {
		...
		items, err := nbtree.PGParseRestorePageData(right.Data)
		if err != nil {
		...
		if err := replayInitedXLogBlock(mgr, right, endLSN, func(page storage.Page) error {
			return nbtree.ReplaySplitRightPage(page, level, left.Block, sibBlk, items)
		}); err != nil {
	...
	if left.HasImage && left.ImageApply {
		...
		newItem, highKey, perr := nbtree.ParseSplitLeftBlockData(left.Data, newItemOnLeft)
		if perr != nil {
			return fmt.Errorf("wal: xlog btree-split left data: %w", perr)
		}
		desc := nbtree.SplitLeftDescription{
		...
		if err := replayExistingXLogBlock(mgr, left, endLSN, func(page storage.Page) error {
			return nbtree.ReplaySplitLeftPage(page, level, right.Block, desc)
		}); err != nil {
	...
	if hasSib {
		if sib.HasImage && sib.ImageApply {
			...
			return nbtree.ReplaySplitSiblingPrev(page, right.Block)
		}); err != nil {
...
func replayDecodedXLogBtreeDelete(mgr *storage.Manager, r Record, xlog *XLogDecodedRecord) error {
	...
	if err := replayExistingXLogBlock(mgr, block, endLSN, func(page storage.Page) error {
		return nbtree.ReplayDeletePage(page, deleted, updates)
	}); err != nil {
...
func decodeXLogBtreeDeletePayload(data []byte, ndeleted, nupdated int) ([]uint16, []nbtree.PostingUpdate, error) {
	...
	updates := make([]nbtree.PostingUpdate, nupdated)
	for i := range updates {
		off := 2 * (ndeleted + i)
	...
	pos := want
	for i := range updates {
		if len(data)-pos < nbtree.SizeOfBtreeUpdate {
...
func replayBtreeVacuum(mgr *storage.Manager, r Record) error {
	...
	if err := nbtree.ReplayVacuumPage(page, p.KeptItems, p.OpaqueFlags); err != nil {
...
func replayBtreeUnlinkPage(mgr *storage.Manager, r Record) error {
	...
	if p.HasLeftSib {
		if err := apply(p.LeftSibBlk, func(page storage.Page) error {
			return nbtree.ReplaySetSiblingNext(page, p.LeftSibNewNext)
		}); err != nil {
	...
	if p.HasRightSib {
		if err := apply(p.RightSibBlk, func(page storage.Page) error {
			return nbtree.ReplaySetSiblingPrev(page, p.RightSibNewPrev)
		}); err != nil {
			return fmt.Errorf("wal: btree-unlink right sib: %w", err)
		}
	}
	if err := apply(p.LeafBlk, func(page storage.Page) error {
		return nbtree.ReplaySetOpaqueFlags(page, p.LeafFlagsAfter)
	}); err != nil {
	...
	if p.HasParent {
		...
		if err := apply(p.ParentBlk, func(page storage.Page) error {
			return nbtree.ReplayParentRetargetByChild(page, p.LeafBlk)
		}); err != nil {
...
func replayBtreeNewRoot(mgr *storage.Manager, r Record) error {
	...
	if storage.MustHeader(page).LSN() < endLSN {
		if err := nbtree.ReplayNewRootPage(page, p.Level, p.Items); err != nil {
	...
	if err := mgr.ReadBlock(p.Rel, nbtree.MetaBlock, metaPage); err != nil {
		return err
	}
	...
	if err := nbtree.ReplayMetaSetRoot(metaPage, p.RootBlk, p.Level); err != nil {
		return fmt.Errorf("wal: btree-newroot meta apply: %w", err)
	}
	storage.MustHeader(metaPage).SetLSN(endLSN)
	return mgr.WriteBlock(p.Rel, nbtree.MetaBlock, metaPage)
}
...
func replayBtreeMarkPageHalfDead(mgr *storage.Manager, r Record) error {
	...
	if err := nbtree.ReplaySetOpaqueFlags(page, p.FlagsAfter); err != nil {
...
func replayBtreeInsert(mgr *storage.Manager, r Record) error {
	...
	if err := nbtree.ApplyInsertRecordAt(page, item, offnum); err != nil {
