...
func splitTestPages(t *testing.T, level uint32, leftBlk, rightBlk, sibBlk storage.BlockNumber, nright int) (left, right, sib storage.Page) {
	...
	if err := nbtree.InitPGBTPage(right); err != nil {
		t.Fatal(err)
	}
	nbtree.WritePGOpaque(right, nbtree.PGBTPageOpaque{Prev: leftBlk, Next: pgSib, Level: level, Flags: flags})
	...
	if pgSib != nbtree.PNone {
		if _, err := storage.PageAddItemRaw(right, nbtree.PGBTPivotRaw([]byte("inherited-hikey"), nbtree.PNone)); err != nil {
			t.Fatal(err)
		}
	}
	for i := range nright {
		key := []byte{byte('r'), byte('0' + i)}
		if _, err := storage.PageAddItemRaw(right, nbtree.PGBTPivotRaw(key, storage.BlockNumber(100+i))); err != nil {
			t.Fatal(err)
		}
	}

	left = make(storage.Page, storage.BlockSize)
	if err := nbtree.InitPGBTPage(left); err != nil {
		t.Fatal(err)
	}
	nbtree.WritePGOpaque(left, nbtree.PGBTPageOpaque{
		Prev: nbtree.PNone, Next: rightBlk, Level: level,
		Flags: flags | nbtree.BTPIncompleteSplit,
	})
	for _, raw := range [][]byte{
		nbtree.PGBTPivotRaw([]byte("new-separator"), nbtree.PNone), // P_HIKEY
		nbtree.PGBTPivotRaw([]byte("l0"), 10),
		nbtree.PGBTPivotRaw([]byte("l1"), 11),
	} {
	...
	if sibBlk != storage.InvalidBlockNumber {
		...
		nbtree.WritePGOpaque(sib, nbtree.PGBTPageOpaque{Prev: rightBlk, Next: nbtree.PNone, Level: level, Flags: flags})
	}
...
func TestEncodeBtreeSplitPGIsContentParity(t *testing.T) {
	...
	wantData, err := nbtree.PGRestorePageData(right)
	if err != nil {
		t.Fatal(err)
	}
...
func TestEncodeBtreeSplitPGRejectsUnredoableRightOpaque(t *testing.T) {
	...
	for _, tc := range []struct {
		name   string
		mutate func(op *nbtree.PGBTPageOpaque)
	}{
		{"garbage hint", func(op *nbtree.PGBTPageOpaque) { op.Flags |= nbtree.BTPHasGarbage }},
		{"wrong back-link", func(op *nbtree.PGBTPageOpaque) { op.Prev = 77 }},
		{"wrong forward link", func(op *nbtree.PGBTPageOpaque) { op.Next = 78 }},
	} {
		t.Run(tc.name, func(t *testing.T) {
			left, right, sib := splitTestPages(t, 0, leftBlk, rightBlk, sibBlk, 2)
			op := nbtree.ReadPGOpaque(right)
			tc.mutate(&op)
			nbtree.WritePGOpaque(right, op)
...
func TestApplyRecordReplaysPGBtreeSplit(t *testing.T) {
	...
	if err := nbtree.InitPGBTPage(preLeft); err != nil {
		t.Fatal(err)
	}
	nbtree.WritePGOpaque(preLeft, nbtree.PGBTPageOpaque{Prev: nbtree.PNone, Next: sibBlk, Flags: nbtree.BTPLeaf})
	...
	if got, want := nbtree.ReadPGOpaque(gotRight), nbtree.ReadPGOpaque(right); got != want {
	...
	if op := nbtree.ReadPGOpaque(gotSib); op.Prev != rightBlk {
...
func TestApplyRecordReplaysPGBtreeSplitChildClear(t *testing.T) {
	...
	pre := func(op nbtree.PGBTPageOpaque) storage.Page {
		p := make(storage.Page, storage.BlockSize)
	...
	for _, page := range []storage.Page{
		pre(nbtree.PGBTPageOpaque{}),
		pre(nbtree.PGBTPageOpaque{Prev: nbtree.PNone, Next: sibBlk, Level: 1}),
		pre(nbtree.PGBTPageOpaque{Prev: leftBlk, Next: nbtree.PNone, Level: 1}),
		pre(nbtree.PGBTPageOpaque{Prev: nbtree.PNone, Next: nbtree.PNone, Flags: nbtree.BTPLeaf | nbtree.BTPIncompleteSplit}),
	} {
		if _, err := mgr.Extend(rel, page); err != nil {
			t.Fatal(err)
		}
	...
	if op := nbtree.ReadPGOpaque(gotChild); op.IncompleteSplit() {
