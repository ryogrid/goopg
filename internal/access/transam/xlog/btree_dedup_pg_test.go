...
func dedupPageTIDs(t *testing.T, page storage.Page, firstOff uint16) [][]storage.ItemPointer {
	...
	for off := firstOff; off <= uint16(count); off++ {
		...
		if !nbtree.BTreeTupleIsPosting(raw) {
			out = append(out, []storage.ItemPointer{nbtree.PGIndexTupleTID(raw)})
			continue
		}
		n := int(nbtree.BTreeTupleGetNPosting(raw))
		base := nbtree.BTreeTupleGetPostingOffset(raw)
		tids := make([]storage.ItemPointer, n)
		for i := range tids {
			tids[i] = nbtree.PGItemPointerAt(raw[base+i*nbtree.SizeOfItemPointerData:])
		}
		out = append(out, tids)
	}
	return out
...
func TestApplyRecordReplaysPGBtreeDedup(t *testing.T) {
	...
	btreeSeedRel(t, mgr, rel, btreeLeafPageWith(t,
		nbtree.PGBTItemRaw([]byte("aaa"), tid(7, 10)),
		nbtree.PGBTItemRaw([]byte("aaa"), tid(7, 20)),
		nbtree.PGBTItemRaw([]byte("aaa"), tid(7, 30)),
		nbtree.PGBTItemRaw([]byte("bbb"), tid(8, 10)),
		nbtree.PGBTItemRaw([]byte("bbb"), tid(8, 20)),
	))

	framed := buildBtreeDedupPG(t, rel, leafBlk, []nbtree.DedupInterval{
		{BaseOff: 1, NItems: 3},
		{BaseOff: 4, NItems: 2},
	})
	...
	if !nbtree.BTreeTupleIsPosting(first) {
...
func TestApplyRecordReplaysPGBtreeDedupKeepsHighKeyAndMergesPostings(t *testing.T) {
	...
	highKey := nbtree.PGBTItemRaw([]byte("zzz"), tid(0, 0))
	base := nbtree.PGBTItemRaw([]byte("aaa"), tid(7, 10))
	existing := nbtree.PGBTPostingRaw(base, []storage.ItemPointer{tid(7, 10), tid(7, 20)})
	...
	nbtree.WritePGOpaque(page, nbtree.PGBTPageOpaque{
		Prev: nbtree.PNone, Next: 9, Flags: nbtree.BTPLeaf | nbtree.BTPHasGarbage,
	})
	for _, raw := range [][]byte{
		highKey,
		existing,
		nbtree.PGBTItemRaw([]byte("aaa"), tid(7, 30)),
		nbtree.PGBTItemRaw([]byte("ccc"), tid(9, 10)),
	} {
	...
	framed := buildBtreeDedupPG(t, rel, leafBlk, []nbtree.DedupInterval{{BaseOff: 2, NItems: 2}})
	...
	if op := nbtree.ReadPGOpaque(leaf); op.Flags&nbtree.BTPHasGarbage != 0 {
...
func TestApplyRecordReplaysPGBtreeDedupFromImage(t *testing.T) {
	...
	after := btreeLeafPageWith(t, nbtree.PGBTPostingRaw(base, []storage.ItemPointer{tid(7, 10), tid(7, 20)}))
...
func TestApplyRecordRefusesPGBtreeDedupBadIntervals(t *testing.T) {
	...
	t.Run("unmatched-baseoff", func(t *testing.T) {
		seed()
		framed := buildBtreeDedupPG(t, rel, 0, []nbtree.DedupInterval{{BaseOff: 5, NItems: 2}})
	...
	t.Run("truncated-interval-array", func(t *testing.T) {
		...
		data := make([]byte, nbtree.SizeOfDedupInterval)
	...
	t.Run("single-item-interval", func(t *testing.T) {
		seed()
		framed := buildBtreeDedupPG(t, rel, 0, []nbtree.DedupInterval{{BaseOff: 1, NItems: 1}})
