...
func btreeInternalPage(t *testing.T, level uint32, downlinks ...storage.BlockNumber) storage.Page {
	...
	nbtree.WritePGOpaque(page, nbtree.PGBTPageOpaque{Prev: nbtree.PNone, Next: nbtree.PNone, Level: level})
	for i, blk := range downlinks {
		var key []byte
		if i > 0 { // slot 1 is the minus-infinity downlink: no key
			key = []byte("k")
		}
		if _, err := storage.PageAddItemRaw(page, nbtree.PGBTPivotRaw(key, blk)); err != nil {
			t.Fatal(err)
		}
	}
	return page
...
func buildBtreeInsertPG(t *testing.T, info uint8, rel storage.RelFileNode, blk storage.BlockNumber,
	offnum uint16, item []byte, childBlk storage.BlockNumber, md *nbtree.PGBTMetaPage,
) []byte {
	t.Helper()
	...
	if md != nil {
		refs = append(refs, BlockRef{
			ID: 2, Rel: rel, Block: nbtree.MetaBlock, SameRel: true,
...
func TestApplyRecordReplaysPGBtreeInsertUpper(t *testing.T) {
	...
	if err := nbtree.InitPGMetaPage(meta, rootBlk, 1, true); err != nil {
		t.Fatal(err)
	}
	...
	newItem := nbtree.PGBTPivotRaw([]byte("sep"), storage.BlockNumber(3))
	...
	if nbtree.ReadPGOpaque(child).Flags&nbtree.BTPIncompleteSplit != 0 {
...
func TestApplyRecordRefusesPGBtreeInsertUpperWithoutChild(t *testing.T) {
	...
	framed := buildBtreeInsertPG(t, xlogBtreeInsertUpper, rel, 0, 2,
		nbtree.PGBTPivotRaw([]byte("sep"), 3), storage.InvalidBlockNumber, nil)
...
func TestApplyRecordReplaysPGBtreeInsertMeta(t *testing.T) {
	...
	if err := nbtree.InitPGMetaPage(stale, childBlk, 0, true); err != nil {
		t.Fatal(err)
	}
	btreeSeedRel(t, mgr, rel, stale, btreeLeafPage(t, true), btreeInternalPage(t, 1, childBlk))

	md := nbtree.PGBTMetaPage{
		Version: nbtree.BTreeVersionPG, Root: rootBlk, Level: 1,
	...
	framed := buildBtreeInsertPG(t, xlogBtreeInsertMeta, rel, rootBlk, 2,
		nbtree.PGBTPivotRaw([]byte("sep"), 3), childBlk, &md)
...
func TestApplyRecordPGBtreeInsertMetaRestoresMetaBehindABlock0Image(t *testing.T) {
	...
	body, err := assembleXLogRecord(mainData, []BlockRef{
		...
		{ID: 2, Rel: rel, Block: nbtree.MetaBlock, SameRel: true, Data: encodeXLogBtreeMetadata(md), WillInit: true},
	})
	if err != nil {
		t.Fatal(err)
	}
	...
	if m := nbtree.ReadPGMetaPage(gotMeta); m.Root != rootBlk || m.Level != 1 {
...
func TestApplyRecordReplaysPGBtreeMetaCleanup(t *testing.T) {
	...
	meta := make(storage.Page, storage.BlockSize)
	if err := nbtree.InitPGMetaPage(meta, rootBlk, 0, true); err != nil {
		t.Fatal(err)
	}
	...
	md := nbtree.PGBTMetaPage{
		Version: nbtree.BTreeVersionPG, Root: rootBlk, Level: 0,
		FastRoot: rootBlk, FastLevel: 0, LastCleanupNumDelpages: 12, AllEqualImage: true,
	}
	body, err := assembleXLogRecord(nil, []BlockRef{
		{ID: 0, Rel: rel, Block: nbtree.MetaBlock, Data: encodeXLogBtreeMetadata(md), WillInit: true},
	})
	if err != nil {
		t.Fatal(err)
	}
	...
	if err := mgr.ReadBlock(rel, nbtree.MetaBlock, got); err != nil {
		t.Fatal(err)
	}
	m := nbtree.ReadPGMetaPage(got)
	if m.Magic != nbtree.BTreeMagicPG || m.Root != rootBlk || m.LastCleanupNumDelpages != 12 {
		t.Errorf("replayed metapage = %+v, want magic + root %d + num_delpages 12", m, rootBlk)
	}
	if !nbtree.ReadPGOpaque(got).IsMeta() {
		t.Errorf("replayed metapage lost BTP_META")
	}
	...
	if err := mgr.ReadBlock(rel, nbtree.MetaBlock, again); err != nil {
		t.Fatal(err)
	}
