...
func TestEncodeCopyTextRowPgbenchShape(t *testing.T) {
	...
	got, err := EncodeCopyTextRow(nil, row, cols, "ISO", "MDY", "UTC", "hex", nil, false)
	if err != nil {
		t.Fatal(err)
	}
	want := []byte("1\t1\t0\t\n")
...
func TestEncodeCopyTextRowDate(t *testing.T) {
	...
	for _, tc := range cases {
		got, err := EncodeCopyTextRow(nil, row, cols, tc.style, tc.order, "UTC", "hex", nil, false)
		if err != nil {
...
func TestEncodeCopyTextRowTimestampTZ(t *testing.T) {
	...
	for _, tc := range cases {
		got, err := EncodeCopyTextRow(nil, row, cols, tc.style, tc.order, tc.zone, "hex", nil, false)
		if err != nil {
	...
	got, err := EncodeCopyTextRow(nil, row, plain, "ISO", "MDY", "Asia/Kolkata", "hex", nil, false)
	if err != nil {
		t.Fatal(err)
	}
...
func TestRoundTripBoolAndTimestamp(t *testing.T) {
	...
	enc, err := EncodeCopyTextRow(nil, row, cols, "ISO", "MDY", "UTC", "hex", nil, false)
	if err != nil {
		t.Fatal(err)
	}
...
func TestEncodeCopyTextRowTimeAndTimeTZ(t *testing.T) {
	...
	for _, tc := range cases {
		...
		for _, style := range []string{"ISO", "SQL", "Postgres", "German"} {
			got, err := EncodeCopyTextRow(nil, tc.row, cols, style, "MDY", "Asia/Tokyo", "hex", nil, false)
			if err != nil {
...
func TestCopyTextTimeRoundTrip(t *testing.T) {
	...
	for _, tc := range cases {
		...
		text, err := datumToCopyText(tc.typ, d, "ISO", "MDY", "Asia/Tokyo", "hex", nil, false)
		if err != nil {
