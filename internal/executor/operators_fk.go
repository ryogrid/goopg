...
//
// childTbl is frequently the storage-less partitioned ROOT: the deferred-FK
// queue (checkFKInsertForConstraints, operators_storage.go) and ALTER TABLE
// callers only ever know the FK owner's root name (FK definitions live only
// on the root), never the leaf a row was actually routed into. A root has
// zero physical blocks, so scanning only childTbl silently finds nothing —
// this is the M0134-0005h COMMIT-tier integrity gap. Mirror the sibling scans
// scanTableForMatch/scanTableForMatchFKWait: scan childTbl itself (a no-op for
// a storage-less root, harmless/needed for a real table) THEN every
// allDescendants(childTbl) partition/inheritance descendant, so a violating
// row that lives only in a leaf partition (or a partition of a partition) is
// still found. Design 0134-0005 §14.6; probe tmp/ralph-handoffs/m0134-0005h-probe.
func fullTableFKCheck(ctx *Context, childTbl *catalog.Table, fk catalog.ForeignKey) error {
	if err := fullTableFKCheckRel(ctx, childTbl, childTbl, fk); err != nil {
		return err
	}
	if im, ok := ctx.Catalog.(*catalog.InMemory); ok {
		for _, child := range allDescendants(im, childTbl, snapDetachEpoch(ctx)) {
			if err := fullTableFKCheckRel(ctx, childTbl, child, fk); err != nil {
				return err
			}
		}
	}
...
// fullTableFKCheckRel scans every row of exactly leafTbl (no descendant
// expansion — that is fullTableFKCheck's job) and verifies each row's FK
// column values exist in the referenced parent table. Column positions are
// resolved from leafTbl.Columns (by name, via fkColValues), never from a
// caller's precomputed index, so a partition whose physical column order
// differs from the root's is still decoded correctly. ownerTbl is the FK's
// declaring table (the partitioned root, when leafTbl is a descendant) and is
// passed through to assertParentExists purely for constraint-name synthesis
// when fk.Name is unset — PG's per-leaf FK clone reuses the parent's
// autogenerated conname (Q5/Q6 of the probe), not one derived from the leaf.
func fullTableFKCheckRel(ctx *Context, ownerTbl, leafTbl *catalog.Table, fk catalog.ForeignKey) error {
	rel := ctx.Catalog.RelFileNode(leafTbl)
	cols := leafTbl.Columns
	...
	for blk := storage.BlockNumber(0); blk < nBlocks; blk++ {
		...
		for slotIdx := uint16(1); slotIdx <= uint16(count); slotIdx++ {
			...
			if err := assertParentExists(ctx, ownerTbl, leafTbl, fk, vals); err != nil {
				s.RUnlock()
				ctx.Pool.Unpin(s)
				return err
			}
