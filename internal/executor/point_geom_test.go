...
func TestPointStrictlyLeftRightOperators(t *testing.T) {
	...
	if got, err := evalBinary(parser.OpBitShiftLeft, NullDatum, NewStringDatum("(1,1)"), 0); err != nil || !got.IsNull() {
		t.Errorf("point << NULL: got=%+v err=%v want NULL", got, err)
	}

	// Integer bit-shift unaffected: 1 << 4 = 16.
	if got, err := evalBinary(parser.OpBitShiftLeft, NewIntDatum(1), NewIntDatum(4), 0); err != nil || got.Kind != KindInt || got.Int != 16 {
		t.Errorf("1 << 4: got=%+v err=%v want int 16", got, err)
	}

	// A non-point string still errors as before (no point hijack).
	if _, err := evalBinary(parser.OpBitShiftLeft, NewStringDatum("abc"), NewStringDatum("def"), 0); err == nil {
