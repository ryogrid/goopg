...
func EncodeCopyCsvRow(dst []byte, row Row, cols []catalog.Column, f copyToFormat, dateStyle, dateOrder, timeZone string, cat catalog.Catalog, qualify bool, visible ...func(schema string) bool) ([]byte, error) {
	for i, c := range cols {
		if i > 0 {
			dst = append(dst, f.delim)
		}
