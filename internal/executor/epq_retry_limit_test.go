...
func TestEPQRetryLimitByIsolation(t *testing.T) {
	if got := epqRetryLimit(transam.IsolationReadCommitted); got != maxEPQRetriesRC {
	...
	for _, iso := range []transam.IsolationLevel{transam.IsolationRepeatableRead, transam.IsolationSerializable} {
