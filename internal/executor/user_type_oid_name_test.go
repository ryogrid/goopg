...
// by format_type's built-in-fallback path and the ::regtype cast, see
// expr.go) across all four user-type kinds plus their auto-generated array/
// multirange forms. Before this helper existed, ::regtype only ever
...
func TestUserTypeNameForOIDAllKinds(t *testing.T) {
	...
	for _, tc := range cases {
		got, ok := userTypeNameForOID(cat, tc.oid)
		if ok != tc.wantOK || got != tc.want {
			t.Errorf("%s: userTypeNameForOID(%d) = (%q, %v), want (%q, %v)", tc.name, tc.oid, got, ok, tc.want, tc.wantOK)
		}
	}
}
