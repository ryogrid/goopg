...
// ---------------------------------------------------------------------------
// The join-GRAPH sibling of the proof above (`graphEndOf`,
// `graphJoinKeyDivisor`, `bestProvableGraphKey`, `coveringGraphEdges`,
// `graphFKParentRaw`) lived here from M0127-P5.6-f-ii until M0127-P6.3.
//
// It existed for ONE consumer: `estimateJoinCost` (bushy.go) priced a
// candidate join DURING subset enumeration, over `joinEdge`s and bitmasks,
// with no `*Join` node to prove a key over — so the same superkey/FK proof had
// to be written a second time in the graph's coordinate space. P6.3 deleted
// that enumerator (08 §4), and with it the only caller. The `*Join`-space
// prover above is the survivor and is now the sole implementation, which
// retires the sibling-paths obligation (rule #2) the pair carried.
//
// Nothing is lost by the deletion: the two deliberate weaknesses of the graph
// version — no `covered` list, so edges left uncovered by a proven key
// contributed no selectivity of their own, and no 04 §3.3 `rowsBound` — were
// consequences of the integer DP's table-wide-maximum pricing, and the search
// that replaced it prices clauses individually through `joinrelsize.go`.

// columnsSubset moved here from `bushy.go` at M0127-P6.3: its last two
// consumers are this file's `*Join`-space superkey proof and `joinrelsize.go`,
// both of which outlived the DP that first needed it (08 §4).
