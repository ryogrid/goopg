...
func TestPartialHashJoinPropagatesUpTheJoinTree(t *testing.T) {
	withParallelOn(t, func() {
		...
		addPartialHashJoinPath(s, probe, mid, third, s.cp, parser.JoinInner, midPP.HashKeys, nil, 0, hashJoinFinalCostInput{})
	...
	withParallelOn(t, func() {
		...
		for _, tc := range []struct {
			...
			t.Run(tc.name, func(t *testing.T) {
				...
				addPartialHashJoinPath(s, joinrel, outer, inner, s.cp, parser.JoinInner, keys, nil, 0, hashJoinFinalCostInput{})
