...
func TestAddPathsToJoinrel_CartesianPairStillGetsAPath(t *testing.T) {
	...
	joinrel := newRelOptInfo(a|b, 5000, 64)
	if err := addPathsToJoinrel(nil, joinrel, outer, inner, nil, defaultCostParams(), nil); err != nil {
		t.Fatalf("addPathsToJoinrel: %v", err)
	}
...
func TestAddPathsToJoinrel_SymmetricPairResolvesToTheFirstOfferedOrder(t *testing.T) {
	...
	for iter := 0; iter < 20; iter++ {
		...
		if err := addPathsToJoinrel(nil, joinrel, rel1, rel2, clauses, cp, nil); err != nil {
			t.Fatalf("addPathsToJoinrel(rel1, rel2, nil): %v", err)
		}
		if err := addPathsToJoinrel(nil, joinrel, rel2, rel1, clauses, cp, nil); err != nil {
			t.Fatalf("addPathsToJoinrel(rel2, rel1, nil): %v", err)
		}
		setCheapest(joinrel)
...
func TestAddPathsToJoinrel_MissingCheapestPathIsLoud(t *testing.T) {
	...
	if err := addPathsToJoinrel(nil, joinrel, good, bare, nil, defaultCostParams(), nil); err == nil {
		t.Fatalf("a nil inner cheapest path must be reported, not skipped")
	}
	if err := addPathsToJoinrel(nil, joinrel, bare, good, nil, defaultCostParams(), nil); err == nil {
