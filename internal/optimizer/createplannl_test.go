...
// column 3. R25 decomposes the fused NLI node into a lateral Join over the
// probe, so the emitted keys are PG nestloop params — OuterColumnRef at
// level 1 over the SAME outer positions the old positional keys used —
// resolved per outer row from the lateral driver's pushed tuple. The direct
// statement of the contract is unchanged: the key expressions must index
...
func TestCreateNestLoopPlanNLIProbeKeyIsInOUTERCoordinates(t *testing.T) {
	...
	if j.Algo != JoinAlgoNestedLoop || !j.Lateral {
		t.Fatalf("Join Algo=%v Lateral=%v, want nested-loop lateral (the decomposed NLI)", j.Algo, j.Lateral)
	}
	is, ok := j.Right.(*IndexScan)
	if !ok {
		t.Fatalf("decomposed Right = %T, want *IndexScan probe", j.Right)
	}
	if len(is.Keys) != 2 || is.Key != nil {
		t.Fatalf("a two-column probe must use Keys, not Key: Key=%v Keys=%v", is.Key, is.Keys)
	}
	outerWidth := len(j.Left.Output())
	for i, k := range is.Keys {
		ocr, isOuter := k.(*OuterColumnRef)
		if !isOuter {
			t.Fatalf("probe key %d = %T, want *OuterColumnRef (PG nestloop param)", i, k)
		}
		if ocr.Level != 1 {
			t.Fatalf("probe key %d level = %d, want 1 (the immediate outer row)", i, ocr.Level)
		}
		if ocr.Index >= outerWidth {
			t.Fatalf("probe key %d = outer col(%d), which is outside the %d-column outer row it is resolved against",
				i, ocr.Index, outerWidth)
		}
	}
	if got := is.Keys[0].(*OuterColumnRef).Index; got != 1 {
		t.Errorf("probe key 0 = outer col(%d), want col(1) (binding 3 = b1, outer position 1)", got)
	}
	if got := is.Keys[1].(*OuterColumnRef).Index; got != 2 {
		t.Errorf("probe key 1 = outer col(%d), want col(2) (binding 4 = b2, outer position 2)", got)
	}
	...
	if len(j.Output()) != 5 || j.Output()[3].Name != "a0" {
		t.Fatalf("schema = %v, want outer ++ inner", j.Output())
	}
}
...
func TestCreateNestLoopPlanNLIResidualIsInMERGEDCoordinates(t *testing.T) {
	...
	if is.Key == nil || len(is.Keys) != 0 {
		t.Fatalf("a one-column probe must use Key, not Keys: Key=%v Keys=%v", is.Key, is.Keys)
	}
	ocr, isOuter := is.Key.(*OuterColumnRef)
	if !isOuter {
		t.Fatalf("probe key = %T, want *OuterColumnRef (PG nestloop param)", is.Key)
	}
	if ocr.Level != 1 {
		t.Fatalf("probe key level = %d, want 1 (the immediate outer row)", ocr.Level)
	}
	if got := ocr.Index; got != 1 {
		t.Errorf("probe key = outer col(%d), want col(1) (binding 3 = b1, outer position 1)", got)
	}
}
...
func TestLeafCondGatesTheNLIInner(t *testing.T) {
	...
	j, isJoin := node.(*Join)
	if !isJoin || j.Algo != JoinAlgoNestedLoop || !j.Lateral {
		t.Fatalf("plan = %T, want lateral *Join (R25 decomposed)", node)
	}
	is, ok := j.Right.(*IndexScan)
	if !ok {
		t.Fatalf("decomposed Right = %T, want *IndexScan probe", j.Right)
	...
	if j.Predicate != nil {
		t.Errorf("the absorbed qual must NOT appear as a join residual; Predicate = %v", j.Predicate)
	}
}
