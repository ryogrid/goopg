...
func (s *Handler) replyTimelineHistory(w *libpq.FrameWriter, args string) error {
	...
	name := xlog.TimelineHistoryFileName(tli)
	path := filepath.Join(walDir, name)
	content, err := os.ReadFile(path)
	if err != nil {
		if !errors.Is(err, fs.ErrNotExist) {
			return s.writeQueryError(w, errcodes.InternalError, err.Error())
		}
...
func (s *Handler) replyCreateReplicationSlot(w *libpq.FrameWriter, args string) error {
	...
	var slotKind xlog.SlotKind
	var plugin string
	switch kind {
	case "PHYSICAL":
		slotKind = xlog.SlotPhysical
...
func (s *Handler) replyStartReplication(ctx context.Context, r *libpq.FrameReader, w *libpq.FrameWriter, raw string, appName, dbName string) error {
	...
	it, err := xlog.NewRecordIterator(s.cfg.WAL, walDir, segSize, args.StartLSN)
	if err != nil {
	...
	var senderHandle *xlog.Sender
	if s.cfg.WalSenders != nil {
		senderHandle = s.cfg.WalSenders.Register(xlog.SenderState{
	...
	defer func() {
		...
		s.cfg.Logger.Info("walsender disconnect",
			"event", xlog.EventWalsenderDisconnect,
			"slot", args.SlotName,
	...
	chunkCh := make(chan xlog.RawChunk, 1)
	recErrCh := make(chan error, 1)
	go func() {
		for {
			chunk, err := it.NextRaw(streamCtx, xlog.XLOGBlockSize)
			if err != nil {
				recErrCh <- err
				return
			}
	...
	for {
		...
		case <-receiveDone:
			...
			return nil
		case chunk := <-chunkCh:
			frame := libpq.EncodeWALData(chunk.StartLSN, chunk.EndLSN, time.Now().UTC(), chunk.Payload)
		...
		case <-keepaliveTimer.C:
			frame := libpq.EncodeKeepalive(s.cfg.WAL.WrittenLSN(), time.Now().UTC(), false)
			if err := w.WriteCopyData(frame); err != nil {
				streamCancel()
				return err
			}
...
func (s *Handler) handleStandbyCopyData(slotName string, payload []byte, senderHandle *xlog.Sender, syncRep *xlog.SyncRep, appName string) error {
	...
	if kind == libpq.ReplMsgStandbyStatus {
		st := parsed.(*libpq.StandbyStatusUpdate)
...
// to fill this in.
func walsenderClientAddr(_ *libpq.FrameReader) string {
	return ""
}
...
func replicationSlotErrCode(err error) errcodes.Code {
	switch err {
	case xlog.ErrSlotExists:
		return errcodes.DuplicateObject
	case xlog.ErrSlotNotFound:
		return errcodes.UndefinedObject
	case xlog.ErrSlotInUse:
		return errcodes.ObjectInUse
	case xlog.ErrSlotInvalidName:
		return errcodes.InvalidParameterValue
	}
