...
func TestLogicalReceiverHandleCopyDataAppliesPgoutputAndAdvancesLSN(t *testing.T) {
	...
	emit := func(t *testing.T, fn func(po *xlog.PgOutput) error) {
		t.Helper()
		...
		walPayload := libpq.EncodeWALData(0, 0xCAFE, time.Now().UTC(), buf.Bytes())
		if err := rec.handleCopyData(walPayload); err != nil {
			t.Fatal(err)
		}
	}

	emit(t, func(po *xlog.PgOutput) error { return po.Begin(42, 0xCAFE) })
	emit(t, func(po *xlog.PgOutput) error {
		return po.Change(xlog.Change{
			Kind:     xlog.ChangeInsert,
			Rel:      pubCat.RelFileNode(pubTbl),
			NewTuple: tuple,
		})
	})
	emit(t, func(po *xlog.PgOutput) error { return po.Commit(42, 0xCAFE) })
