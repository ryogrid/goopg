...
// (Token slices — see internal/Lex and the dispatch splitter)
...
//   - Syntax errors surface as *SyntaxError carrying the ABSOLUTE
...
type lexerState struct {
	...
	spanStart int
	toks []Token // this statement's tokens (absolute Pos)
	i    int            // next raw index into toks

	out []Stmt       // filled by the start production
	err *SyntaxError // first error wins
	...
	intoFor map[*SelectStmt]*intoTarget
...
func (l *lexerState) applyNulls(sb SortBy, marker string) {
	switch marker {
	case "f":
		v := true
		sb.NullsFirst = &v
...
func ParseOneSrc(src string, toks []Token) ([]Stmt, error) {
	...
	if parsePooled(l) != 0 {
		...
		return nil, &SyntaxError{Message: "syntax error", Raw: true}
	}
	return l.out, l.errAsError()
}

func ParseOne(toks []Token, baseOffset int) ([]Stmt, error) {
	_ = baseOffset
...
func fragEndPos(src string, toks []Token) int {
	eof := -1
...
func (l *lexerState) setValueIsDefault() bool {
	...
	for j := i + 1; j < len(l.toks); j++ {
		if l.toks[j].Kind != TokenEOF && l.toks[j].Value != ";" {
			return false
		}
	}
	return true
...
func (l *lexerState) setValueStart() int {
	for i, t := range l.toks {
		...
		if (t.Kind == TokenKeyword || t.Kind == TokenIdent) && strings.EqualFold(t.Value, "to") {
			return i + 1
		}
	}
	return -1
...
func (l *lexerState) setValueAtoms() string {
	...
	for i++; i < len(l.toks); i++ {
		t := l.toks[i]
		switch t.Kind {
		case TokenIntLit, TokenNumericLit, TokenStringLit,
			TokenIdent, TokenQuotedIdent, TokenKeyword:
...
func (l *lexerState) checkFollowsWith() bool {
	...
	p := l.toks[l.i-2]
	return (p.Kind == TokenKeyword || p.Kind == TokenIdent) &&
...
func (l *lexerState) peekCharTypmodLit() (string, bool) {
	...
	if open.Kind != TokenSymbol || open.Value != "(" ||
		num.Kind != TokenIntLit ||
		close_.Kind != TokenSymbol || close_.Value != ")" ||
		str.Kind != TokenStringLit {
		return "", false
	}
	l.i = i + 4
...
func (l *lexerState) mapToken(i int) lexResult {
	...
	case TokenIntLit:
		...
		// error was being dropped, and Atoi SATURATES — so
		// `SELECT 9223372036854775808` silently became MaxInt64 and
		// `99999999999999999999::int8` returned a value instead of raising
...
func (l *lexerState) Error(msg string) {
	...
	// Same STRUCTURED shape as the legacy path (errSyntaxAtCur): Message holds
	// only the offending token's raw source spelling and Error() supplies the
	// "syntax error at or near …" wrapper. Building the whole sentence here
	// with Raw=true produced an identical user-visible string but a different
	// FIELD, which every caller reading err.Message directly could see.
	near := l.lastText
	...
	l.err = &SyntaxError{Message: near, Pos: l.lastPos}
}
...
func (l *lexerState) directArrayAt(pos int) bool {
	for i, tk := range l.toks {
		...
		if tk.Kind != TokenIdent || !strings.EqualFold(tk.Value, "array") {
			return false
		}
		return i+1 < len(l.toks) && l.toks[i+1].Kind == TokenSymbol && l.toks[i+1].Value == "["
	}
	return false
}
