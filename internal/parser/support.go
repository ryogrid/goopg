...
func columnRefFromParts(q qname) Expr {
	parts := q.parts
	if len(parts) > 3 {
		parts = parts[len(parts)-3:]
	}
	return NewColumnRef(q.pos, parts)
}
...
func rangeVarFromName(q qname, alias string) RangeVar {
	schema := ""
	name := ""
	...
	return NewRangeVar(q.pos, schema, name, alias)
}
...
func gateSyntaxError(l *lexerState, msg, hint string) {
	if l.err == nil {
		l.err = &SyntaxError{Message: msg, Hint: hint, Raw: true, Pos: l.lastConsumedPos()}
	}
}
...
func buildJoin(l *lexerState, spec *joinSpec, right RangeVar, q joinQual) JoinExpr {
	j := NewJoinExpr(spec.pos, spec.jt, spec.natural, right, q.on, q.using)
	if spec.jt == JoinCross && spec.natural {
		l.err = &SyntaxError{Message: "NATURAL CROSS JOIN is not supported", Raw: true, Pos: spec.pos}
		return j
	}
	if !spec.natural && spec.jt != JoinCross && q.on == nil && len(q.using) == 0 {
		l.err = &SyntaxError{Message: "expected ON or USING in JOIN", Raw: true, Pos: spec.pos}
	}
	return j
}
...
func derivedRangeVar(l *lexerState, pos int, sub *SelectStmt, alias string, cols []string, lateral bool) RangeVar {
	if alias == "" {
...
func lerr(yylex yyLexer, msg string, pos int) {
	l := yylex.(*lexerState)
	if l.err == nil {
		l.err = &SyntaxError{Message: msg, Raw: true, Pos: pos}
	}
}
...
func syntheticParenSelect(pos int, fe FromExpr) *SelectStmt {
	s := NewSelectStmt(pos)
	s.Targets = []ResTarget{NewResTarget(pos, "", NewStarExpr(pos, "", ""))}
	s.From = flattenFrom(fe)
	s.FromExprs = []FromExpr{fe}
	return s
}

// flattenFrom extracts Base plus every join right side.
func flattenFrom(fe FromExpr) []RangeVar {
	out := []RangeVar{fe.Base}
...
func newTableFuncRef(pos int, name string, args []Expr, ord bool, rows []RowsFromEntry) *TableFuncRef {
	return NewTableFuncRef(pos, name, args, ord, rows)
}
...
func plainGroupItem(e Expr) *groupItem {
	return &groupItem{flat: []Expr{e}, alts: [][]Expr{{e}}}
}
...
// is a unit of one.
func groupingUnits(exprs []Expr) [][]Expr {
	var units [][]Expr
	for _, e := range exprs {
...
func flattenUnits(units [][]Expr) []Expr {
	var out []Expr
	for _, u := range units {
		out = append(out, u...)
	}
...
func cartesianGroupingSets(components [][][]Expr) [][]Expr {
	sets := [][]Expr{{}}
...
func buildGroupClause(pos int, items []*groupItem) *groupClause {
	...
	if construct {
		gc.sets = NewGroupingSetsSpec(pos, cartesianGroupingSets(comps))
	}
	return gc
}
...
func substringSimilar(l yyLexer, pos int, str, pat, esc Expr) Expr {
	...
	if !patOK || !escOK {
		if ls != nil && ls.err == nil {
			ls.err = &SyntaxError{Message: "SUBSTRING(... SIMILAR ... ESCAPE ...) with non-literal operands is not yet supported", Raw: true, Pos: pos}
		}
		return NewNullConst(pos)
	}
	if patNull || escNull {
	...
	folded, err := foldSubstringSimilar(str, patVal, escVal, pos)
	if err != nil {
		if se, ok := err.(*SyntaxError); ok {
			raiseErr(l, se)
		} else if ls != nil && ls.err == nil {
			ls.err = &SyntaxError{Message: err.Error(), Raw: true, Pos: pos}
		}
		return NewNullConst(pos)
	}
	return folded
...
func literalValue(e Expr) (string, bool, bool) {
	switch x := e.(type) {
	case *StringConst:
...
func foldSetOps(base *SelectStmt, pairs []setopPair) *SelectStmt {
	if len(pairs) == 0 {
		return base
	}
	...
	for i := len(pairs) - 1; i >= 0; i-- {
		...
		left.SetOp = NewSetOpClause(pr.op.pos, pr.op.typ, pr.op.all, node)
...
func tableSelect(pos int, q qname) *SelectStmt {
	schema, name := "", ""
	if len(q.parts) == 1 {
		name = q.parts[0]
	} else {
	...
	star := NewStarExpr(pos, "", "")
	rt := NewResTarget(pos, "", star) // legacy leaves ResTarget.pos zero here
	rv := NewRangeVar(q.pos, schema, name, "")
	s := NewSelectStmt(pos)
	s.Targets = []ResTarget{rt}
	fe := NewFromExpr(rv.Pos(), rv, nil)
	s.From = []RangeVar{rv}
	s.FromExprs = []FromExpr{fe}
	return s
}
...
type cteItem struct{ cte *CommonTableExpr }
...
func buildBetween(pos int, left Expr, low, high Expr, negated, symmetric bool) Expr {
	...
	le := func(a, b Expr) Expr {
		return NewBinaryOp(pos, OpLe, a, b)
	}
	and := func(a, b Expr) Expr {
		return NewBinaryOp(pos, OpAnd, a, b)
	}
	or := func(a, b Expr) Expr {
		return NewBinaryOp(pos, OpOr, a, b)
	}
	...
	if negated {
		return NewUnaryOp(pos, OpNot, tree)
	}
	return tree
}
...
func buildSimilarTo(l yyLexer, left, pattern, escape Expr, pos int, negate bool) Expr {
	...
	if !patOK || !escOK {
		return NewSimilarToPattern(pos, left, pattern, escape, negate)
	}
	if patNull || escNull {
		return NewNullConst(pos)
	}
	if err := similarto.ValidateEscape(escVal); err != nil {
		if ls, ok2 := l.(*lexerState); ok2 && ls.err == nil {
			ls.err = &SyntaxError{Raw: true, Code: "22025", Message: "invalid escape string", Hint: "Escape string must be empty or one character."}
		}
		return NewNullConst(pos)
	}
	...
	return NewBinaryOp(pos, op, left, NewTypedStringLit(pos, "text", converted))
}

// binOp maps an operator spelling to its OpCode; unknown spellings raise a hard error.
func binOp(l yyLexer, s string) OpCode {
	if op := ParseBinaryOp(s); op != OpUnknown {
		return op
	}
	if ls, ok := l.(*lexerState); ok && ls.err == nil {
		ls.err = &SyntaxError{Message: fmt.Sprintf("unsupported operator %q", s), Raw: true, Pos: ls.lastConsumedPos()}
	}
	return OpUnknown
}
...
func buildIntervalQualified(pos int, body, hi, lo string, prec int) Expr {
	unit := hi
	if lo != "" {
	...
	return NewIntervalLitQualified(pos, body, unit, hasPrec, prec)
}
...
type castType struct {
	...
	// IntervalQualTypmods), and cast_target cannot know which position
...
func buildIntervalLit(pos int, body string) Expr {
	...
	if val, unit, ok := SplitEmbeddedInterval(body); ok {
		return NewIntervalLitEmbedded(pos, val, unit)
	}
	if m, d, us, ok := ParseIntervalBody(body); ok {
		return NewIntervalLitPre(pos, m, d, us)
	}
	// Neither form decoded: the body is not a valid interval. select.go's
	// tryTypedLiteral returns (nil,false) here, so INTERVAL is re-read as a
	// bare identifier and the literal that follows it has no production —
	// a syntax error AT THE LITERAL. Falling back to a TypedStringLit
	// accepted every malformed body instead (`interval '1days2hours'`,
	// `interval '1h2h'`, 80 executor cases).
	return nil
}
...
func finishFrame(mode FrameMode, ext *partFrameExtent, ex *partFrameExcl) *WindowFrame {
	return &WindowFrame{
		Mode:       mode,
		StartKind:  ext.start,
...
// ColumnDef (grammar carrier pattern).
type colSpec struct {
	checkText string
	fkInfo    *fkInfo
	fkNotValid    bool
	...
	onUp     FKAction
	name     string
	namePos  int // ColumnDef.pos: the column NAME's offset (ddl.go parseColumnDef)
	typePos  int // ColumnType.pos: the type name's offset
	schema   string
	typ      string
	args     []int64
	isArray  bool
...
type tableElem struct {
	...
	check      string
	checkName  string
	fkDef      *TableForeignKeyDef
	namedPk    *TableConstraintDef
	namedUq    *TableConstraintDef
	exclusion  *TableConstraintDef
	like       *ObjectName
	likeOpts   string
	uqIncl     []string
	uqNND      bool
...
type colConstraints struct {
	...
	checkNoInherit bool
	checkNotEnforced bool
	// nnOccur records every literal column-level NOT NULL clause in written
	// order, so resolveColumnNotNull can apply PG's conflict algorithm. A
	// single notNull flag cannot: `CONSTRAINT foo NOT NULL CONSTRAINT bar NOT
	// NULL` is a 42601 upstream and was silently collapsing to one.
	nnOccur []colNotNullOccurrence
	// lastCon / sawEnforced mirror parse_utilcmd.c transformConstraintAttrs'
	// lastprimarycon and saw_enforced: an ENFORCED attribute binds to the
	// constraint element it FOLLOWS, is legal only on CHECK and FOREIGN KEY,
	// and may appear only once per element — the counter resets on each new
	// element, so `CHECK (x) NOT ENFORCED UNIQUE NOT ENFORCED` fails as
	// misplaced rather than as a duplicate.
	lastCon     string
	sawEnforced bool
	storage        string
	fk         *fkInfo
...
func applyIdentityOpts(cd *ColumnDef, o *identityOpts) {
	if o == nil {
		return
	}
...
func dropBehavior(s string) DropBehavior {
	if s == "cascade" {
		return DropCascade
	}
	return DropDefault
...
func objectNameFromQn(q qname) ObjectName {
	var o ObjectName
	o.pos = q.pos
...
// noteEnforcedAttr applies parse_utilcmd.c transformConstraintAttrs' two
// checks for a column-level [NOT] ENFORCED, IN UPSTREAM'S ORDER: misplaced
// first, then duplicate. Both are 42601 with their own wording, so they go
// through raiseErr rather than the yyLexer.Error rewriter. It returns false
// when the attribute is rejected, so the caller leaves the flags alone.
func noteEnforcedAttr(l yyLexer, cc *colConstraints, isNot bool, pos int) bool {
	switch cc.lastCon {
	case "check", "fk":
	default:
		...
		raiseErr(l, &SyntaxError{Pos: pos, Message: msg, Raw: true})
		return false
	}
	if cc.sawEnforced {
		raiseErr(l, &SyntaxError{
			Pos:     pos,
...
// noteConstraintElem records a non-attribute constraint element as the one a
// following ENFORCED attribute binds to, resetting the per-element duplicate
// counter exactly as upstream's `default:` arm does.
func noteConstraintElem(cc *colConstraints, kind string) {
	cc.lastCon = kind
	cc.sawEnforced = false
}
...
func namedTableConstraint(name string, cols []string, isPrimary bool, incl []string, nnd bool, a *constrAttrs) *TableConstraintDef {
	d := NewTableConstraintDef(name, cols, isPrimary)
...
func sessionAuthzStmt(l *lexerState, local bool, atom string) Stmt {
	if l.authzIsDefaultKeyword() {
		return NewSetStmt(0, local, "session_authorization", "", true)
	}
	return NewSetStmt(0, local, "session_authorization", atom, false)
}
...
func (l *lexerState) authzIsDefaultKeyword() bool {
	for i, t := range l.toks {
		...
		n := l.toks[i+1]
		return (n.Kind == TokenKeyword || n.Kind == TokenIdent) &&
...
// idxTail collects CREATE INDEX's order-independent tail clauses.
type idxTail struct {
	nnd        bool
	opts       any
	tablespace string
...
func mergeIdxTail(dst, src *idxTail) *idxTail {
	if src.nnd {
		dst.nnd = true
	}
	if src.opts != nil {
	...
	if src.tablespace != "" {
		dst.tablespace = src.tablespace
	}
	return dst
}

func applyIndexOpts(ix *CreateIndexStmt, v any) {
	o, _ := v.(*indexOpts)
	if o == nil {
		return
	}
...
func newExclusionConstraint(name, method string, elems []excludeElem, incl []string, where Expr, a *constrAttrs) *TableConstraintDef {
	var cols []string
	op := ""
	for _, e := range elems {
		if e.cols != nil {
	...
	d := NewTableConstraintDef(name, cols, false)
	d.IsExclusion = true
	d.ExclusionOp = op
	d.Method = method
	d.IncludeColumns = incl
...
func appendCallArg(acc *callArgs, a callArg) *callArgs {
	...
	if a.variadic {
		...
		elemCast := ""
		expr := a.expr
		if ce, ok := expr.(*CastExpr); ok {
			if _, isArr := ce.Operand.(*ArrayConstructorExpr); isArr {
				elemCast = strings.TrimSuffix(ce.Type.Name, "[]")
				expr = ce.Operand
			}
		}
		if arr, ok := expr.(*ArrayConstructorExpr); ok {
			for _, el := range arr.Elements {
				if elemCast != "" {
					el = NewCastExpr(el.Pos(), el, ObjectName{Name: elemCast}, nil)
				}
...
func callFuncExpr(pos int, name ObjectName, ca *callArgs) *FuncCall {
	if ca == nil {
		return NewFuncCall(pos, name, nil, false)
	}
	fc := NewFuncCall(pos, name, ca.exprs, false)
...
func newIndexElem(e Expr, collation string, opclass opClassRef, desc bool, nullsFirst *bool) indexElem {
	el := indexElem{order: IndexColOrder{Descending: desc, OpClass: opclass.name, Collation: collation}}
	...
	if cr, ok := e.(*ColumnRef); ok && cr.Schema == "" && cr.Table == "" {
		el.name = cr.Column
	} else {
		el.expr = e
	}
...
func arbiterFromExprs(items []Expr) *OnConflictTarget {
	...
	t := NewOnConflictTarget(cols, "", nil)
	t.Exprs = exprs
	return t
}
...
func onConflictColumnTarget(cols []string) *OnConflictTarget {
	t := NewOnConflictTarget(cols, "", nil)
	t.Exprs = make([]Expr, len(cols))
	return t
}
...
func prefixOp(l yyLexer, s string) OpCode {
	if s == "~" {
		return OpBitNot
	}
	if ls, ok := l.(*lexerState); ok && ls.err == nil {
		ls.err = &SyntaxError{Message: fmt.Sprintf("operator does not exist: %s", s), Raw: true, Pos: ls.lastConsumedPos()}
	}
	return OpUnknown
}
...
type partKey struct {
	...
	expr      Expr // expression key; nil for a plain column key
	opClass   string
	collation string
}
...
func newPartKey(e Expr, pos int, collation, opClass string) partKey {
...
func tzZone(l yyLexer, zone Expr) Expr {
	ls, ok := l.(*lexerState)
...
func intoWrap(l yyLexer, s Stmt) Stmt {
	ls, ok := l.(*lexerState)
...
func checkStrayInto(l yyLexer, sel *SelectStmt, msg string, noPos bool) bool {
	ls, ok := l.(*lexerState)
...
func strayIntoLeft(ls *lexerState) *SyntaxError {
...
func parenGroup(pos int, inner *SelectStmt, t *parenTail) *SelectStmt {
	inner.Parenthesized = true
	grp := NewSelectStmt(pos)
	grp.SetOpOperand = inner
	if t.op != nil {
		grp.SetOp = NewSetOpClause(t.op.pos, t.op.typ, t.op.all, t.right)
...
func quantifiedAny(l yyLexer, pos int, left Expr, op OpCode, sub *SelectStmt, list []Expr, listPos int) Expr {
	anyOp := op
	notEq := false
	switch op {
	case OpEq:
...
func unwrapAnyArray(l yyLexer, list []Expr, listPos int) []Expr {
	if len(list) != 1 {
		return list
	}
	...
	if ac, ok := e.(*ArrayConstructorExpr); ok && len(ac.Elements) > 0 {
		return ac.Elements
	}
	return list
}
...
// applyNotNullOccurrences runs PG's column-level NOT NULL conflict algorithm
// (parse_utilcmd.c transformColumnDefinition, via ddl.go's
// resolveColumnNotNull — the SAME function, not a second copy) and records
// its 42601 if the column's clauses conflict. It must run AFTER Primary /
// IdentityColumn / the type name are set, because the NO INHERIT prohibition
// depends on all three.
func applyNotNullOccurrences(l yyLexer, cd *ColumnDef, occ []colNotNullOccurrence) {
	if len(occ) == 0 {
		return
	}
	if err := resolveColumnNotNull(cd, occ); err != nil {
		if se, ok := err.(*SyntaxError); ok {
			raiseErr(l, se)
		}
	}
...
func copyColConstraints(cd *ColumnDef, collation, compression, nnName, uqName, checkName string, nnNoInherit, checkNoInherit bool, checkNotEnforced bool, storage string) {
...
func applyPartOfElems(poc *PartitionOfClause, elems []*partOfElem) {
	...
	for _, e := range elems {
		...
		case e.hasCheck:
			if e.checkName != "" {
				poc.CheckConstraints = append(poc.CheckConstraints, PartitionCheckConstraint{Name: e.checkName, Expr: e.checkExpr})
			}
		default:
			if e.notNull {
			...
			if e.hasDef {
				poc.ColDefaults = append(poc.ColDefaults, PartitionColDefault{ColName: e.col, Expr: e.def})
			}
			if e.hasGen {
				poc.ColGeneratedExprs = append(poc.ColGeneratedExprs, PartitionColGenerated{ColName: e.col, Expr: e.genExpr})
			}
		}
	}
}
...
// bitStringConst is decodeBitStringLit (expr.go) — the SAME function, not a
// second copy. The copy that stood here silently accepted `b'2'` and `x'Z'`,
// decoding the bad digit as zero, because it had no validation arm: an
// invalid digit is a 22P02, and only expr.go's version raises it.
// checkCopyEndpointDir mirrors copy.go's endpoint/direction pairing: STDIN
// belongs to COPY FROM and STDOUT to COPY TO. Upstream's grammar allows
// either word in either direction (gram.y copy_file_name maps both to a NULL
// filename), but goopg has always rejected the mismatch, and this is a
// package move, not the place to relax a shipping diagnostic.
// lastTokPos is the byte offset of the token most recently returned by the
// lexer — where a "something is missing here" error should point.
// seqRelOp marks an ALTER SEQUENCE clause that is really a RELATION
// operation, so buildAlterSequence can switch statement types.
type seqRelOp struct {
	kind string // "rename" | "owner" | "schema"
	name string
}

func altSeqRelOp(kind, name string) any { return &seqRelOp{kind: kind, name: name} }

// buildAlterSequence picks the statement type: an AlterTableStmt when the
// clause list carries a relation operation (RENAME / OWNER TO / SET SCHEMA),
// an AlterSequenceStmt otherwise. ddl.go makes the same split.
func buildAlterSequence(pos int, name ObjectName, ifExists bool, opts []any) Stmt {
	var rel []*seqRelOp
	var seq []any
	for _, o := range opts {
		if r, ok := o.(*seqRelOp); ok {
			rel = append(rel, r)
			continue
		}
		seq = append(seq, o)
	...
	if len(rel) == 0 {
		st := NewAlterSequenceStmt(pos, name, ifExists)
		applyAlterSeqOpts(st, seq)
		return st
	}
	st := NewAlterTableStmt(pos, name)
	st.IfExists = ifExists
	st.TagOverride = "ALTER SEQUENCE"
	for _, r := range rel {
		switch r.kind {
		case "rename":
			a := NewATAction(AlterTableRenameTable)
			a.NewName = r.name
...
// atActionAt re-anchors an already-built AlterTableAction. at_constraint
// reduces before the ADD that introduces it is visible, so its own first
// keyword is the wrong anchor for the position ddl.go records.
func atActionAt(v any, pos int) *AlterTableAction {
	a, _ := v.(*AlterTableAction)
	if a != nil {
		a.pos = pos
	}
	return a
...
func checkCopyEndpointDir(l yyLexer, dir CopyDirection, ep *copyEndpoint, pos int) {
	switch {
	case ep.kind == CopyEndpointStdin && dir != CopyFrom:
		raiseErr(l, &SyntaxError{Pos: pos, Message: "STDIN is only valid with COPY FROM"})
	case ep.kind == CopyEndpointStdout && dir != CopyTo:
		raiseErr(l, &SyntaxError{Pos: pos, Message: "STDOUT is only valid with COPY TO"})
	}
}

func bitStringConst(l yyLexer, pos int, v string) Expr {
	e, err := decodeBitStringLit(Token{Kind: TokenBitStringLit, Value: v, Pos: pos})
	if err != nil {
		if se, ok := err.(*SyntaxError); ok {
			raiseErr(l, se)
		}
		return NewStringConst(pos, "")
	}
	return e
}
...
// applyCtWith fills CreateTableStmt.With from a ct_tail WITH list AND derives
// WithOIDS from it, the way ddl.go does at each of its four CREATE TABLE
// arms. It exists because the two grammar arms (with and without a column
// list) each carried their own copy of the loop, and only the copies had the
// oids derivation missing — the recurring sibling-paths bug.
func applyCtWith(ct *CreateTableStmt, kvs []string) {
	for _, kv := range kvs {
		parts := splitKV(kv)
		if len(parts) == 0 {
			continue
		...
		if len(parts) == 1 {
			// A boolean flag written without a value — `WITH (oids)` — is
			// stored as "true" (ddl.go parseWithOptions), not dropped. The
			// same str_pair_list also serves RESET (n_distinct), where a bare
			// name is a NAME and must stay one, so the defaulting belongs
			// here rather than in the grammar rule.
			ct.With[parts[0]] = "true"
			continue
		}
		ct.With[parts[0]] = parts[1]
	}
	if v, ok := ct.With["oids"]; ok {
		ct.WithOIDS = !strings.EqualFold(v, "false") && v != "0"
	}
}
...
func viewReloptions(cv *CreateViewStmt, kvs []string) {
	for _, kv := range kvs {
		...
		case "check_option":
			...
			// of `WITH [LOCAL|CASCADED] CHECK OPTION`. LOCAL is the only value
			// that is not cascaded: ddl.go:2612 defaults everything else —
			// absent, empty, misspelt, or the "true" a bare flag now carries —
			// to cascaded rather than raising, matching reloptions.c's enum
			// handling. Taking the value verbatim let `WITH (check_option)`
			// store "true".
			opt := "cascaded"
			if hasVal && strings.EqualFold(strings.Trim(val, "'"), "local") {
				opt = "local"
			}
			cv.CheckOption = opt
...
func insertTarget(q qname, alias string, cols []string) (RangeVar, []string) {
...
func multiSetRHS(l yyLexer, e Expr, pos int) Expr {
	switch e.(type) {
	case *RowExpr, *SubqueryExpr:
		return e
	}
	if ls, ok := l.(*lexerState); ok {
		for _, tk := range ls.toks {
...
func applyExplainOpts(l yyLexer, pos int, opts []*explainOpt) ExplainOptions {
	var o ExplainOptions
	fail := func(msg string) {
		if ls, ok := l.(*lexerState); ok && ls.err == nil {
			ls.err = &SyntaxError{Message: msg, Raw: true, Pos: pos}
		}
	}
	for _, e := range opts {
		...
		if name == "format" {
			...
			case "json":
				o.Format = ExplainFormatJSON
			case "xml":
				o.Format = ExplainFormatXML
			case "yaml":
				o.Format = ExplainFormatYAML
			default:
...
type atConstrTail struct {
	...
	notValid                      bool
	notValidPos                   int // byte offset of the NOT, for the ALTER CONSTRAINT rejection
	enforced, hasEnforceability   bool
	noInherit, hasInheritability  bool
}

// mergeATTailAt is mergeATTail with the attribute's source position, needed
// only by NOT VALID: ALTER CONSTRAINT rejects it with a caret under the NOT.
func mergeATTailAt(t *atConstrTail, w string, pos int) *atConstrTail {
	t = mergeATTail(t, w)
	if w == "not_valid" {
		t.notValidPos = pos
	}
...
func applyATCheckTail(a *AlterTableAction, t *atConstrTail) *AlterTableAction {
	applyATTail(a, t)
	if t != nil {
...
func applyAlterConstraint(a *AlterTableAction, t *atConstrTail) *AlterTableAction {
	if t == nil {
		return a
	}
...
func applyFnAttrs(st *CreateFunctionStmt, a *fnAttrs) *CreateFunctionStmt {
...
func applyProcAttrs(st *CreateProcedureStmt, a *fnAttrs) *CreateProcedureStmt {
...
func tableArgs(base []FunctionArg, cols []FunctionArg) []FunctionArg {
	for _, c := range cols {
		c.Mode, c.ModeExplicit = FuncArgOut, true
		base = append(base, c)
	}
	return base
}
...
// floatNormalized applies gram.y opt_float's precision fold AND its two
// out-of-range errors. Those are ereports from inside a production — 22023,
// not 42601 — so they cannot surface through yyLexer.Error, which rewrites
// everything into "syntax error at or near". raiseErr carries the code.
func floatNormalized(l yyLexer, name string, args []int64) (string, []int64) {
	n, a, err := normalizeFloatTypeName(name, args, 0)
	if ls, ok := l.(*lexerState); ok && err != nil {
		if se, ok2 := err.(*SyntaxError); ok2 {
			se.Pos = ls.lastConsumedPos()
		}
	}
	if err != nil {
		if se, ok := err.(*SyntaxError); ok {
...
// raiseErr records a fully-formed SyntaxError — message, Raw, Code and Hint
// together — as the parse's first error. l.raise cannot: it has no Code
// parameter, so an ereport-from-a-production error routed through it would
// silently become a 42601.
func raiseErr(l yyLexer, se *SyntaxError) {
	ls, ok := l.(*lexerState)
	if !ok || ls.err != nil {
		return
	}
...
func colTypeOf(l yyLexer, tw *typeWithArgs, pos int) ColumnType {
	...
	if tw.ct.schema == "" && !tw.quoted {
		...
		name, args = floatNormalized(l, name, args)
	}
	elem, isArray := trimArrayTail(name)
	if !tw.quoted && len(args) == 0 &&
		(strings.EqualFold(elem, "char") || strings.EqualFold(elem, "character")) {
		// gram.y CharacterWithoutLength -> bpchar with typmod 1. ddl.go:5195
		// stamps it AFTER the array tail is split off, so `character[]` gets
		// it too — the copy in typmodsFor only ever saw the scalar cast form
		// and only knew the "char" spelling, which is why a `character[]`
		// argument came out as "char"[] (OID 1002) instead of character[]
...
func fnDropBehavior(s string) DropBehavior {
	if s == "cascade" {
		return DropCascade
	}
	return DropDefault
...
func dropProcNames(extras []DropFunctionItem) []ObjectName {
	if len(extras) == 0 {
		return nil
	}
	out := make([]ObjectName, 0, len(extras))
...
func appendNamedCallArg(c *namedCallArgs, a callArg) *namedCallArgs {
	if c == nil {
		c = &namedCallArgs{exprs: []Expr{}, argNames: []string{}}
	}
...
func joinBodyTokens(l yyLexer, from int) string {
	...
	for _, t := range st.toks {
		...
		parts = append(parts, TokenBodySQL(t))
	}
	return strings.Join(parts, " ")
}
...
func isDefaultKeywordAt(l yyLexer, pos int) bool {
	...
	for _, t := range st.toks {
		if t.Pos == pos {
			return t.Kind == TokenKeyword && t.Keyword == KwDefault
		}
	}
	return false
}
...
func implicitCharLen(l yyLexer, pos int, ct castType) []int64 {
	if st, ok := l.(*lexerState); ok {
		for _, t := range st.toks {
			if t.Pos == pos && t.Kind == TokenQuotedIdent {
...
func colTypmodArgs(tw *typeWithArgs, n int64) []int64 {
	if strings.EqualFold(tw.ct.name, "interval") && tw.ct.schema == "" {
		if _, col, ok := IntervalQualTypmods("", "", int(n)); ok {
			return []int64{col}
		}
	}
	return []int64{n}
...
func tzSetStmt(local bool, v string) Stmt {
	if strings.EqualFold(v, "default") {
		return NewSetStmt(0, local, "timezone", "", true)
	}
	return NewSetStmt(0, local, "timezone", v, false)
}
...
func fetchStmt(pos int, v any, isMove bool) Stmt {
	if isMove {
		return NewCompatNoopStmt(pos, "MOVE")
	}
	f, _ := v.(*fetchSpec)
	if f == nil {
		f = &fetchSpec{count: 1, forward: true}
	}
	return NewFetchStmt(pos, f.name, f.count, f.forward)
}
...
func vacuumNamedOpt(name, val string) func(*VacuumStmt) {
	switch strings.ToLower(name) {
	...
	case "verbose":
		return func(v *VacuumStmt) { v.Verbose = true }
	case "analyze", "analyse":
		return func(v *VacuumStmt) { v.Analyze = true }
	case "full":
		return func(v *VacuumStmt) { v.Full = true }
	case "freeze":
		return func(v *VacuumStmt) { v.Freeze = true }
	case "truncate":
		return func(v *VacuumStmt) { v.NoTruncate = isFalseWord(val) }
	case "parallel":
		n := reloptInt(val)
		return func(v *VacuumStmt) { v.ParallelWorkers = n }
	case "disable_page_skipping":
		return func(v *VacuumStmt) { v.DisablePageSkipping = true }
	case "skip_database_stats":
		return func(v *VacuumStmt) { v.SkipDatabaseStats = true }
	case "only_database_stats":
		return func(v *VacuumStmt) { v.OnlyDatabaseStats = true }
	case "skip_locked":
		return func(v *VacuumStmt) { v.SkipLocked = true }
	case "index_cleanup":
	...
	case "process_main":
		return func(v *VacuumStmt) { v.NoProcessMain = isFalseWord(val) }
	case "process_toast":
		return func(v *VacuumStmt) { v.NoProcessToast = isFalseWord(val) }
	case "buffer_usage_limit":
		return func(v *VacuumStmt) { v.BufferUsageLimit = val }
	}
	return func(v *VacuumStmt) {}
}
...
func vacuumAt(pos int, v any) *VacuumStmt {
	src, _ := v.(*VacuumStmt)
	if src == nil {
		return NewVacuumStmt(pos)
	}
	return NewVacuumStmtFrom(pos, src)
}
...
func applyMergeAction(c *MergeWhenClause, v any) {
	a, _ := v.(*mergeAction)
	if a == nil {
		return
	}
...
func mergeNotMatchedBy(word string) *MergeWhenClause {
	switch strings.ToLower(word) {
	case "source":
		return NewMergeWhenClause(0, false, true, false)
	case "target":
		return NewMergeWhenClause(0, false, false, true)
	}
	return NewMergeWhenClause(0, false, false, false)
}
...
func applyRangeOpts(st *CreateTypeStmt, v any) {
	opts, _ := v.([]any)
...
type domainOp func(*CreateDomainStmt)

func domainNotNull(v bool) domainOp {
	return func(d *CreateDomainStmt) { d.NotNull = v }
}

func domainDefault(e Expr) domainOp {
	return func(d *CreateDomainStmt) { d.Default = e }
}

func domainNoop() domainOp { return func(*CreateDomainStmt) {} }
...
func domainCheck(l yyLexer, name string, openParen int) domainOp {
	...
	return func(d *CreateDomainStmt) {
		d.Checks = append(d.Checks, NewDomainCheckClause(name, expr, vals))
	}
}
...
func domainCheckInValues(st *lexerState, openParen int) ([]string, bool) {
	...
	if t := next(); t.Kind != TokenSymbol || t.Value != "(" {
		return nil, false
	}
	t := next()
	...
	for {
		...
		case TokenKeyword:
			if t.Keyword != KwTrue && t.Keyword != KwFalse {
				return nil, false
			}
			vals = append(vals, t.Value)
		default:
...
func applyDomainConstraints(st *CreateDomainStmt, v any) {
	ops, _ := v.([]any)
	for _, o := range ops {
...
type seqOp func(*CreateSequenceStmt)

func seqDataType(name string) seqOp {
	n := lowerIdent(name)
	return func(s *CreateSequenceStmt) { s.DataType = n }
}

func seqInt(which string, n int64) seqOp {
	v := n
	return func(s *CreateSequenceStmt) {
		switch which {
		case "increment":
			s.Increment = &v
		case "minvalue":
...
func seqCycle() seqOp { return func(s *CreateSequenceStmt) { s.Cycle = true } }
...
// earlier CYCLE.
func seqNoop() seqOp { return func(*CreateSequenceStmt) {} }

func seqOwnedBy(owner string) seqOp {
	o := owner
	return func(s *CreateSequenceStmt) { s.OwnedBy = o }
}
...
func applySeqOpts(st *CreateSequenceStmt, v any) {
	ops, _ := v.([]any)
	for _, o := range ops {
		if f, ok := o.(seqOp); ok {
			f(st)
	...
	for i, t := range st.toks {
		if t.Pos == pos {
			return i+1 < len(st.toks) &&
				st.toks[i+1].Kind == TokenSymbol && st.toks[i+1].Value == ","
		}
	}
	return false
}
...
func trigTiming(word string) int {
	switch strings.ToLower(word) {
	case "before":
		return int(TriggerBefore)
	case "after":
		return int(TriggerAfter)
	case "instead":
		return int(TriggerInsteadOf)
	}
	return 0
}
...
func buildTrigger(pos int, name string, table ObjectName, isConstraint bool,
	timing int, events any, deferAny any, refs any,
	eachAny any, when Expr, fn ObjectName, args []string) Stmt {
	st := NewCreateTriggerStmt(pos, name, table, isConstraint)
	st.Timing = TriggerTiming(timing)
...
func commentAt(pos int, cs *CommentOnStmt) Stmt {
	out := NewCommentOnStmt(pos, cs.ObjKind, cs.ObjName, cs.SubName)
...
func commentColumn(q qname) *CommentOnStmt {
	parts := q.parts
	if len(parts) < 2 {
		return NewCommentOnStmt(0, "column", ObjectName{}, strings.Join(parts, "."))
	}
	col := parts[len(parts)-1]
	rest := parts[:len(parts)-1]
	name := ObjectName{Name: rest[len(rest)-1]}
	if len(rest) > 1 {
		name.Schema = rest[len(rest)-2]
	}
	return NewCommentOnStmt(0, "column", name, col)
}

// commentConstraint — `ON DOMAIN` switches the kind, which is the only place
// the two constraint flavours differ.
func commentConstraint(name string, onDomain bool, table ObjectName) *CommentOnStmt {
	kind := "constraint"
	if onDomain {
		kind = "domain constraint"
	}
	return NewCommentOnStmt(0, kind, table, name)
}

func commentCast(src, dst string) *CommentOnStmt {
	cs := NewCommentOnStmt(0, "cast", ObjectName{}, "")
	cs.CastSource, cs.CastTarget = src, dst
	return cs
}

// alterFnOp is one ALTER FUNCTION action, applied in written order.
type alterFnOp func(*AlterFunctionStmt)

func alterFnVolatile(v string) alterFnOp {
	s := v
	return func(a *AlterFunctionStmt) { a.Volatile = &s }
}

func alterFnStrict(v bool) alterFnOp {
	b := v
	return func(a *AlterFunctionStmt) { a.Strict = &b }
}

func alterFnLeakproof(v bool) alterFnOp {
	b := v
	return func(a *AlterFunctionStmt) { a.Leakproof = &b }
}

func alterFnSecurity(v bool) alterFnOp {
	b := v
	return func(a *AlterFunctionStmt) { a.SecurityDefiner = &b }
}

func alterFnNoop() alterFnOp { return func(*AlterFunctionStmt) {} }

func alterFnOwner(name string) alterFnOp {
	n := name
	return func(a *AlterFunctionStmt) { a.NewOwner = n }
}
...
func alterFnRename(name string) alterFnOp {
	n := name
	return func(a *AlterFunctionStmt) { a.RenameTo = n }
}

func alterFnSchema(name string) alterFnOp {
	n := name
	return func(a *AlterFunctionStmt) { a.NewSchema = n }
}

// alterFnConfig records a SET/RESET clause. keep is false for the two forms
// legacy drops (`FROM CURRENT`, `TO DEFAULT`).
func alterFnConfig(op FunctionConfigOp, keep bool) alterFnOp {
	if !keep {
		return alterFnNoop()
	}
	o := op
	return func(a *AlterFunctionStmt) { a.ConfigOps = append(a.ConfigOps, o) }
}

func applyAlterFnActions(st *AlterFunctionStmt, v any) {
...
func dropWithArgs(pos int, kind string, ifExists bool, q qname, args []string, behavior DropBehavior) Stmt {
	st := NewDropCompatStmt(pos, kind, ifExists, []ObjectName{objectNameFromQn(q)}, behavior)
	SetDropCompatExtras(st, args, "", nil, "", "")
	return st
}
...
func buildView(l yyLexer, orReplace bool, modifier any, q qname, cols []string, with []string, sel Stmt, check any) Stmt {
	...
	cv := NewCreateViewStmt(name, cols, s)
...
// entry plus a qualified star. Sharing RewriteIndirectionStarTargets
...
func rewriteIndirectionStars(l yyLexer, s *SelectStmt) error {
	if s == nil {
		return nil
	}
...
type extOp func(*CreateExtensionStmt)

func extSchema(name string) extOp {
	n := name
	return func(e *CreateExtensionStmt) { e.Schema = n }
}

func extVersion(v string) extOp {
	s := v
	return func(e *CreateExtensionStmt) { e.Version = s }
}

func extCascade() extOp { return func(e *CreateExtensionStmt) { e.Cascade = true } }

func applyExtOpts(st *CreateExtensionStmt, v any) {
	for _, raw := range asAnySlice(v) {
...
type altSeqOp func(*AlterSequenceStmt)

func altSeqDataType(name string) altSeqOp {
	n := lowerIdent(name)
	return func(s *AlterSequenceStmt) { s.DataType = n }
}

func altSeqInt(which string, n int64) altSeqOp {
	v := n
	return func(s *AlterSequenceStmt) {
		switch which {
		case "increment":
			s.Increment = &v
		case "minvalue":
...
func altSeqRestart() altSeqOp { return func(s *AlterSequenceStmt) { s.Restart = true } }
...
func altSeqLogged(v string) altSeqOp {
	s := v
	return func(a *AlterSequenceStmt) { a.SetLogged = s }
}
...
func applyAlterSeqOpts(st *AlterSequenceStmt, v any) {
	for _, raw := range asAnySlice(v) {
...
func altTypeRenameValue(old, new_ string) alterTypeOp {
	o, n := old, new_
	return func(a *AlterTypeStmt) { a.RenameOldValue, a.RenameNewValue = o, n }
}

func altTypeRenameTo(name string) alterTypeOp {
	n := name
	return func(a *AlterTypeStmt) { a.RenameTo = n }
}

func altTypeOwner(name string) alterTypeOp {
	n := name
	return func(a *AlterTypeStmt) { a.NewOwner = n }
}
...
type alterDomainOp func(name string) *AlterDomainStmt

func altDomAction(action string) alterDomainOp {
	a := action
	return func(name string) *AlterDomainStmt {
		return NewAlterDomainStmt(0, name, a)
	}
}

func altDomDefault(e Expr) alterDomainOp {
	return func(name string) *AlterDomainStmt {
		st := NewAlterDomainStmt(0, name, "setdefault")
		st.DefaultExpr = e
		return st
	}
}
...
func altDomAddCheck(l yyLexer, cname string, openParen int) alterDomainOp {
	expr, vals := domainCheckParts(l, openParen)
	return func(name string) *AlterDomainStmt {
		st := NewAlterDomainStmt(0, name, "addconstraint")
...
func altDomDropConstraint(ifExists bool, cname string) alterDomainOp {
	ie, c := ifExists, cname
	return func(name string) *AlterDomainStmt {
		st := NewAlterDomainStmt(0, name, "dropconstraint")
...
func altDomRenameConstraint(old, new_ string) alterDomainOp {
	o, n := old, new_
	return func(name string) *AlterDomainStmt {
		st := NewAlterDomainStmt(0, name, "renameconstraint")
...
func altDomRenameTo(newName string) alterDomainOp {
	n := newName
	return func(name string) *AlterDomainStmt {
		st := NewAlterDomainStmt(0, name, "rename")
		st.NewName = n
		return st
	}
}
...
func altDomOwner(owner string) alterDomainOp {
	o := owner
	return func(name string) *AlterDomainStmt {
		st := NewAlterDomainStmt(0, name, "owner")
		st.NewOwner = o
		return st
	}
}
...
func alterDomainAt(pos int, st *AlterDomainStmt) Stmt {
	out := NewAlterDomainStmt(pos, st.Name, st.Action)
...
func altTypeAttrCmds(v any) alterTypeOp {
	...
	return func(a *AlterTypeStmt) {
		for _, raw := range cmds {
			if c, ok := raw.(AlterTypeAttrCmd); ok {
				a.AttrCmds = append(a.AttrCmds, c)
			}
		}
		MirrorFirstAttrCmd(a)
	}
}
...
func copyEndpointWord(l yyLexer, word string, pos int) *copyEndpoint {
	switch strings.ToLower(word) {
	case "stdin":
		return &copyEndpoint{kind: CopyEndpointStdin}
	case "stdout":
		return &copyEndpoint{kind: CopyEndpointStdout}
	}
	if st, ok := l.(*lexerState); ok {
...
func copyEndpointProgram(l yyLexer, word, name string, pos int) *copyEndpoint {
	if strings.EqualFold(word, "program") {
		return &copyEndpoint{kind: CopyEndpointProgram, name: name}
	}
	if st, ok := l.(*lexerState); ok {
		st.raise("expected STDIN, STDOUT, PROGRAM, or filename (got "+word+")", false, pos)
	}
...
func strPairMap(kvs []string) map[string]string {
	...
	for _, kv := range kvs {
		...
		// A bare flag is `name = true` (ddl.go parseWithOptions), the same
		// convention applyCtWith uses; an empty string is not what legacy
		// stores and would compare unequal.
...
func altViewOwner(owner string) alterViewOp {
	o := owner
	return func(st *AlterTableStmt) { st.OwnerTo = o }
}

func altViewSetSchema(name string) alterViewOp {
	n := name
	return func(st *AlterTableStmt) { st.SetSchema = n }
}
...
func altTypeRenameAttr(old, new_ string) alterTypeOp {
	o, n := old, new_
	return func(a *AlterTypeStmt) { a.RenameAttrOld, a.RenameAttrNew = o, n }
}
...
func applyOfElements(ct *CreateTableStmt, v any) {
	elems, _ := v.([]*tableElem)
	for _, e := range elems {
		if e == nil {
			continue
		...
		if e.check != "" {
			if e.checkName != "" {
				ct.TableNamedChecks = append(ct.TableNamedChecks,
					PartitionCheckConstraint{Name: e.checkName, Expr: e.check, NoInherit: e.checkNoInh, NotEnforced: e.checkNotEnf})
			} else {
...
func isQuotedIdentAt(l yyLexer, pos int) bool {
	...
	for _, t := range st.toks {
		if t.Pos == pos {
			return t.Kind == TokenQuotedIdent
		}
	}
	return false
}
...
func roleSetStmt(local bool, v string, isDefault bool) Stmt {
	if isDefault {
		return NewSetStmt(0, local, "role", "", true)
	}
	return NewSetStmt(0, local, "role", v, false)
}
...
func partBoundValues(l yyLexer, exprs []Expr) []Expr {
	for i, e := range exprs {
		cr, ok := e.(*ColumnRef)
		if !ok || cr.Schema != "" || cr.Table != "" {
			continue
		...
		exprs[i] = NewPartitionRangeBoundKeyword(cr.Pos(), isMax)
	}
	return exprs
}
...
func substringCall(l yyLexer, pos int, args []Expr) Expr {
	if len(args) == 3 {
