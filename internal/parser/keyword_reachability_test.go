...
var notYetPortedKeywords = map[string]string{
	...
	"SYSTEM_USER": "DELIBERATE AND PERMANENT: IsNoParenFuncName does not list it, so legacy treats it as a bare identifier. Adding it to sql_value_func_name would CREATE a parity diff, not fix one (see grammar/pg_grammar.y func_expr_common_subexpr).",

	// type_func_name_keyword
	"OVERLAPS": "row OVERLAPS row (:15162) — uq_cols's WITHOUT OVERLAPS does not count, see blindRules",
	// TABLESAMPLE was removed 2026-08-29 (M0134-0175): tablesample_clause now
	// consumes it. This test caught the stale entry on its own.
}
