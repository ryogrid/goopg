...
func fragmentRouted(frag []Token) bool {
...
func withFollowerRouted(toks []Token) bool {
	...
	for i := 1; i < len(toks); i++ { // skip WITH
		...
		if depth > 0 || tok.Kind != TokenKeyword {
			continue
		}
...
func explainInnerRouted(frag []Token) bool {
	i := 1
	for i < len(frag) {
		tk := frag[i]
		...
		if tk.Kind == TokenKeyword || tk.Kind == TokenIdent {
...
func secondKeywordRouted(frag []Token, key string) bool {
...
func commentRouted(frag []Token) bool {
...
func alterDomainRouted(frag []Token) bool {
	...
	for _, tok := range frag[2:] {
		...
		seen++
		if seen == 2 {
			act = strings.ToLower(tok.Value)
			if !alterDomainActions[act] {
				return false
			}
			continue
		}
		if seen == 3 && act == "set" {
			// SET has three spellings and the grammar covers only two.
			// `SET SCHEMA` is a relation operation ddl.go answers with a
			// CompatNoopStmt from a skip-to-semicolon scan, so routing it
			// turned a working statement into a 42601.
...
func copyRouted(frag []Token) bool {
	depth := 0
	for _, tok := range frag[1:] {
		if tok.Kind == TokenSymbol {
			switch tok.Value {
		...
		if depth > 0 && (tok.Kind == TokenKeyword || tok.Kind == TokenIdent) &&
...
func alterIndexRouted(frag []Token) bool {
...
func alterViewRouted(frag []Token) bool {
...
func alterMatviewRouted(frag []Token) bool {
...
func createRoutineRouted(frag []Token) bool {
	depth := 0
...
func alterTableActionsRouted(frag []Token) bool {
...
func actionWords(frag []Token) []string {
	var w []string
	for _, t := range frag {
		switch t.Kind {
		case TokenKeyword, TokenIdent:
			w = append(w, strings.ToLower(t.Value))
		case TokenSymbol, TokenOperator:
			w = append(w, t.Value)
		default:
...
func fragmentBase(parent, frag []Token) int {
...
// routeExpr is the expression counterpart of routeBatch: it
...
func routeExpr(src string, toks []Token) (Expr, bool, error) {
	if len(toks) == 0 {
		return nil, false, nil
	}
	_ = src
	prep := make([]Token, 0, len(toks)+2)
	prep = append(prep, Token{Kind: TokenKeyword, Value: "select", Pos: toks[0].Pos - 7})
	...
	if !ok || len(sel.Targets) != 1 || sel.From != nil || sel.Where != nil ||
		...
		return nil, true, &SyntaxError{Message: "unexpected trailing tokens after expression"}
	}
