...
// not yet ported (INSERT arrives in P3): *SyntaxError, PG-style
...
func TestParseOneGarbageErrors(t *testing.T) {
	...
	toks, err := Lex("GRANT SELECT ON t TO r")
	if err != nil {
		t.Fatal(err)
	}
	...
	// The "(byte N)" suffix is SyntaxError.Error()'s own wrapper, shared with
	// the legacy path since P7.1 unified the struct's shape; callers that
	// render a caret read Message and Pos, not this string.
	if want := `syntax error at or near "grant" (byte 0)`; se.Error() != want {
