// External test package: internal/testutil/tpch imports internal/parser, so
// an in-package test file would be an import cycle. Everything this test uses
// (Lex, ParseOne) is exported.
package parser_test
...
func TestTPCHGrammarCoverage(t *testing.T) {
	...
	for i := 1; i <= 22; i++ {
		...
		for _, part := range strings.Split(qs, "\\;") {
			...
			if _, err := parser.ParseOne(toks, 0); err != nil {
