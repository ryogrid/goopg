...
func FormatDateElem(days int32, st OutputStyle) string {
	...
	return misc.FormatDate(t, st.Style, st.Order)
}
...
func FormatTimestampElem(micros int64, st OutputStyle) string {
	...
	return misc.FormatTimestamp(timestampMicrosToTime(micros), st.Style, st.Order)
}
...
func FormatTimestampTZElem(micros int64, st OutputStyle) string {
	...
	return misc.FormatTimestampTZ(timestampMicrosToTime(micros), st.Style, st.Order, st.Zone)
}
...
func DecodeElemStyled(elemName string, data []byte, varlena bool, size int, st OutputStyle) (string, int, error) {
	...
	if varlena {
		...
		if lower == "numeric" || lower == "decimal" {
			...
			text, err := nodes.NumericTextFromStoredPayload(data[4:sz])
			if err != nil {
				return "", 0, err
			}
			return text, sz, nil
	...
	case "interval":
		...
		return QuoteTextElem(datetime.FormatInterval(months, days, micros)), 16, nil
	case "int2", "smallint":
	...
	case "time":
		return QuoteTextElem(datetime.FormatTime(int64(binary.LittleEndian.Uint64(data[:8])))), 8, nil
	case "timestamp":
	...
	case "timetz":
		...
		return QuoteTextElem(datetime.FormatTimeTZ(micros, zone)), 12, nil
