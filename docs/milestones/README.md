...
| 0091 | Select-only TPS regression recovery (activity goroutineID closure-capture + btree.RangeScan zero-copy) | partial — 350.89 → 510.52 TPS (1.45×); residual cloneRow allocation chain identified, filed as M0092 | `0091-select-only-tps-regression-recovery.md` |
| 0092 | Lazy row emission in indexScanOp + projectOp (eliminate the cloneRow → acquireRow → rowPool.New chain that dominates 34 % of allocs in the post-M0091 profile) | planned | `0092-lazy-row-emission-in-scan-and-project.md` |
