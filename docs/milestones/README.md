...
| 0091 | Select-only TPS regression recovery (GC ~70 % of CPU + activity.goroutineID `runtime.Stack` calls + btree.RangeScan per-slot byte copies — 17× regression vs post-M0026 baseline) | planned | `0091-select-only-tps-regression-recovery.md` |
