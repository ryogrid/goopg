...
| 0134 | regress-sql `failed`/`not-tried` test-case digestion (filed 2026-08-15, priority next after M-NIGHTLY; consumes all 189 `regress-sql` inventory cases with `status = failed`/`not-tried`, one case per task, `failed` first) | planned | `0134-regress-sql-failed-not-tried-digestion.md` |
