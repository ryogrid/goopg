...
| 0093 | Skip WAL emission for read-only commits (PG-parity lazy-XID equivalent via `wroteWAL` flag) — eliminates the per-query `XactCommit` WAL record + fsync that goopg currently emits unconditionally; target TPS ≥ 1,000 on pgbench-S | planned | `0093-read-only-commit-skip-wal-emission.md` |
