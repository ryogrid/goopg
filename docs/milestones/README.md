...
| 0129 | Q74 fix (M0128-P0.1 second half) + M0128 verdict follow-ups + residual-ledger burn-down (filed 2026-08-08 per user directive; **top-priority milestone**, ahead of M-NIGHTLY — fixes the live default-ON ~7× Q74 regression with attribution already complete (resume: `addPathsToJoinrel`, joinpaths.go:139); implements the measured-GO cooperative parallel hash build (the announced-but-unfiled P2.1a producer/consumer split + P2.1b concurrent build); burns down the bitmap design's 8 deferral rows including BitmapAnd/Or (`choose_bitmap_and` port) and a recorded selectivity-region survival proof; re-enables the resjunk-ctid column path with intermediate-node propagation; carries the sort-spill ctid side-channel through spills (root-0038); gives `DELETE … USING` real EvalPlanQual; promotes the command counter to statement granularity with per-tuple `cmin`/`cmax` (fence-map retirement); lands the four `reduce_outer_joins` residuals (strictness catalog, ON-clause propagation, LEFT→ANTI, RIGHT→LEFT flip incl. the parser representation work); wires `ExecError.Pos` to the `FieldPosition` wire field with a regress re-baseline; and re-runs the clause-6 adjudication for Q2/Q8/Q17/Q18/Q22. 10 tasks (S1–S10) with inline subtasks; no deferral without a ledger-recorded strong reason; all design docs completed within the milestone) | planned | `0129-q74-fix-and-m0128-followups.md` |
