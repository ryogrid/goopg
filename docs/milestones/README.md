...
| 0132 | Explicit transactions across the extended query protocol (filed 2026-08-12; the extended path commits one transaction per `Execute` and ignores `BEGIN`/`COMMIT`/`ROLLBACK`, so **`ROLLBACK` does not roll back** — 11 slices S1–S11, with S2+S3+S4 bound to land together) | planned | `0132-extended-protocol-explicit-transactions.md` |
