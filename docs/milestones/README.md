...
Selection order across milestones is decided solely by the `## Current
Priority` banner in `.ralph/fix_plan.md` (per `.ralph/PROMPT.md`).
Per-milestone "Priority placement" notes in this directory are filing-time
context only — none of them asserts live rank.
...
2. Write the design docs listed under "Required Design Docs" first, with status `draft`.
   (**Overridden for M0137–M0145** — in that group the design doc is written when
...
| 0085 | pg_commit_ts (optional commit-timestamps subsystem; `track_commit_timestamp` GUC) | planned | `0085-commit-timestamps-pg-commit-ts.md` |
...
| 0134 | regress-sql `failed`/`not-tried` test-case digestion (filed 2026-08-15; consumes all 189 `regress-sql` inventory cases with `status = failed`/`not-tried`, one case per task, `failed` first) | **exhausted 2026-09-01** (all 189 cases sized; none currently selectable) | `0134-regress-sql-failed-not-tried-digestion.md` |
| 0137 | Parity measurement harness and instrument repair (filed 2026-09-14; **first** of the plan-parity group M0137–M0145 — promotes the parity capture pipeline into `scripts/` and fixes the K18 `$$` trap, machine-stamps every capture, reconciles the TPC-DS reference, re-baselines `make plan-gate`, builds the per-query/per-mechanism index over the 126-round corpus, and retires the flag/ledger debt. Every other milestone in the group is judged by instruments this one repairs) | planned | `0137-parity-measurement-harness-and-instrument-repair.md` |
| 0138 | PG-faithful ANALYZE statistics (filed 2026-09-14; implements the owner's **Question 2 = yes** — goopg reproduces PG's estimates including its errors, because otherwise identical plan generation is impossible. Ports PG's `BlockSampler` two-stage block sampling, its PRNG and the signed `stadistinct` convention; **overturns R79's "keep the superior statistics" verdict** and unblocks M0142) | planned | `0138-pg-faithful-analyze-statistics.md` |
| 0139 | Executor-side narrowing / projection pushdown (filed 2026-09-14; implements the owner's **Question 1 = (a)**. Narrows what the executor *produces* at scan leaves inside a join tree, where no `*Project` exists to hang a projection on. Scoped to projection pushdown only — the packed retention format stays declined and its decision is escalated with slice 3's measurement. Argued on Q4 and the 4.1x executor gap, **not** on Q9) | planned | `0139-executor-side-narrowing-projection-pushdown.md` |
| 0140 | TPC-DS parallelism (filed 2026-09-14; `parallelism` blocks 87/99 TPC-DS queries — PG plans 66/99 parallel where goopg plans serial, zero the other way, `Parallel Hash` PG 314 vs goopg 0. Re-measures the failing set under `GOOPG_GATHER_PATHS` at HEAD rather than inheriting the R43-era list, lands the flip on the category metric, then the partial-Append producer. Independent of M0139 per the owner's **(c) = Go**) | planned | `0140-tpcds-parallelism.md` |
...
| 0145 | Jointree-first planner flow (filed 2026-09-20, owner decision; unifies the planning ROUTE with PG 18.3 per `METHODOLOGY4/plan-flow-medium-abstraction.md` — a jointree-level IR built before lowering (base-rel / semi-anti-SJInfo / appendrel entries), sublink pull-up into it, one DP pass, one Path→Node lowering. Parallel pipeline behind `GOOPG_JOINTREE_PIPELINE`; absorbs the walls blocking M0144-0003a/0003b and the M0142-0008a-3 increments; executor-substrate gaps stay out of scope) | planned | `0145-jointree-first-planner-flow.md` |
| 0146 | Post-cutover plan parity (filed 2026-09-23, owner decision delegated scope M0137–M0145; the burn-down the M0145 flow parity exposes — executor substrate (partial-inner `Parallel Hash`, row-emitting PartialAgg, per-worker Memoize, `Materialize`), join-order/candidate-pool election, statistics burn-down, `inline_cte`, lateral re-census. **Gated on the M0145-0008 flip landing** — no task selectable before it) | planned | `0146-post-cutover-plan-parity.md` |
