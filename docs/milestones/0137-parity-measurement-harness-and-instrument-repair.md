...
`-serial` defaults **true** (`cmd/estimate-audit/main.go:285`), which is why
...
- The display-vs-consumed cost seam (C3/K63) is root-caused, so cost-bearing
  artefacts can be trusted — or the residue is a ledger row with a resume point.
- B6, B8, B10 and O15 each carry a ledger row naming the mechanism and what
  would unblock them; none is left as an unowned note in METHODOLOGY3 alone.
