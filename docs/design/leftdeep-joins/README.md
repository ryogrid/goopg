...
| field | value |
| --- | --- |
| status | S0–S4 landed; S5 (the PG-shaped search) implemented and behind `GOOPG_PGSHAPED_DP`, **flag OFF** — acceptance run 1 is a documented NO-GO ([09](09-verification-and-acceptance.md) §3.1, `analysis/leftdeep-joins/2026-08-05-p59-s5-acceptance.txt`); the re-run is blocked on ONE open defect, P5.9-f (flag-ON Q17 addresses a column past the tuple width — §5.20; its "hang" was a Gather swallowing that error, not a timing regression); S6/S7 blocked on the re-run |
