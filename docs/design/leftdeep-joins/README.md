...
| field | value |
| --- | --- |
| status | S0–S4 landed; S5 (the PG-shaped search) implemented and behind `GOOPG_PGSHAPED_DP`, **flag OFF** — acceptance run 1 is a documented NO-GO ([09](09-verification-and-acceptance.md) §3.1, `analysis/leftdeep-joins/2026-08-05-p59-s5-acceptance.txt`); **run 1's last open defect P5.9-f is CLOSED** (§5.21: the decorrelated aggregate is a foreign coordinate scope, and the splice's join key relied on an undeclared name-rebind — flag-ON Q17 now MATCHes the flag-OFF arm on values), so the **full bar re-run (P5.9) is unblocked**; S6/S7 blocked on the re-run |
