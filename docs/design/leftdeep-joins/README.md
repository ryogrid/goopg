...
| field | value |
| --- | --- |
| status | S0–S4 landed; S5 (the PG-shaped search) implemented and behind `GOOPG_PGSHAPED_DP`, **flag OFF** — **acceptance run 3 is a third documented NO-GO** ([09](09-verification-and-acceptance.md) §3.5, `analysis/leftdeep-joins/2026-08-05-p59run3-s5-acceptance.txt`), **and the first that fails on performance alone: clause 1 PASSES** (23 MATCH; the one VALUE-DIFF is Q5, the BASELINE's defect M0119-0011, where flag-ON agrees with PG 18.3 and goopg's default planner is wrong by ~24×). Flag-owned wrong answers across three runs: 4 → 2 → **0**, and TPC-DS SF0.5 under the flag returns **MISMATCH=0 CKMISMATCH=0** across 99 queries. Clause 5 PASS (third consecutive); Q9's named ≤ 170.9 s bar PASSES at 54.95 s. What remains is two named defects. **P5.9-h — the estimate collapse, now HALF CLOSED** ([09](09-verification-and-acceptance.md) §3.6): the `rows=1` was minted after the search by `EstimateRows`, which answered 1 for every index scan on the equality-probe convention — wrong for the bound-less full scan P5.4c-ii-b introduced. One arm fixed it and `parity_violations` went **6 → 0** on the five carrying queries (Q12's `orders` leaf `rows=1` → `rows=1500000`). **But the plan shapes and timings did not move**, so "the timing gap IS the estimate collapse" is refuted: what is left of -h is the COST question of why a full ordered index scan of `orders` beats a Seq Scan + sort under the flag. **P5.9-i — the 7 TPC-DS CTE/UNION-ALL aborts, CLOSED** ([09](09-verification-and-acceptance.md) §3.7): the assertion was right to fire and the arms were innocent — `reresolveJoinByName`'s `predRebind` could not tell a resolution MISS from an AMBIGUITY, so its cross-side fallback rebound a correctly-bound reference onto another CTE scan's identically-named column (`(Name, SourceTableIdx)` is not a column identity ACROSS scopes, only within one). DS05 under the flag went `ERROR=7` → `PASS=6 … ERROR=0 TIMEOUT=1`, five with PG-identical checksums. The TIMEOUT was **Q47, a NEW defect (P5.9-j), now CLOSED** ([09](09-verification-and-acceptance.md) §3.8): not a search-order defect and not an estimate defect — the 1-row estimate for `{v1,v1_lag}` is what **PG estimates too** (verified against the oracle), and PG escapes it only because its CTE scan publishes pathkeys and its merge join costs 375.55 with no sort. goopg's merge must sort twice, so hash (968.55) and plain nested loop (968.53) decided the plan at a margin of **0.02** — and the loop was under-charged by exactly the term that would have settled it: `final_cost_nestloop` charges `cpu_per_tuple` on `ntuples = outer × inner` ("number of tuples processed (not number emitted!)"), while `nestloopCost` charged the `cpu_tuple_cost` half on the join's OUTPUT rows, which is smallest precisely on the plans the term exists to deter. Q47 **8 m 40 s → 13 s**, DS05 subset under the flag `PASS=6 TIMEOUT=1` → **`PASS=7 TIMEOUT=0`**, flag-OFF arm unchanged with identical checksums. Shape moved the right way: divergences 67 → 46, matched joinrels 21 → 32. **P5.9-h's remaining COST half is now CLOSED too, by P5.9-k** ([09](09-verification-and-acceptance.md) §3.9), and the suspect -h named was innocent: `costIndexScan` is roughly right, and the defect was a MISSING term on the other side of the comparison. `costSortRun` implemented only `cost_sort`'s comparison half — justified in its own comment by "TPC-H sorts are small dimension outputs", a premise this phase invalidates, since a merge join sorts a JOIN INPUT and Q12's is 5 997 241 lineitem rows. The hash rival has been charged its spill since P5.7-a, so for Q12 one candidate was billed **1 326 616** and the other **0** for spilling the same 4.7 GB through the same work_mem budget — the asymmetry [04](04-cost-and-cardinality.md) §1 forbids, as a whole missing term. With `cost_tuplesort`'s disk branch reproduced (sized through the same `hashsize.EntryBytes` model the hash side's `spillPages` uses), the five carrying queries go Q7 26.71→16.29, Q9 54.95→15.86, Q10 22.93→5.65, Q12 20.79→9.82, Q18 74.71→29.79 s — **ON/OFF 2.61× → 1.007×**, every digest unchanged, and the §5 audit's ON arm reduced to the OFF arm's single pre-existing Q18 violation. **Run 4 of the bar is now unblocked** |
