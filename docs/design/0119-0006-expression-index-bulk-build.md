...
- expression result kind has no B-tree encoding (`encodeArbiterExprKey` covers
  string + int) or fails to evaluate → `key == nil`, and the caller's existing
...
- `encodeArbiterExprKey` still encodes only `KindString` and `KindInt`; an
  expression over any other result type (numeric, date/timestamp, bool) is
  silently not indexed on **both** paths. Widening it is a single shared change
  now that the build reuses it.
