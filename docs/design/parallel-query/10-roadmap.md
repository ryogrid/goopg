...
| Cooperative parallel hash **build** | The build side is the small side by planner construction (`IsSmallDimensionSide`, `internal/planner/cardinality.go:167`); this is the one place PG's design is genuinely more advanced ([07](07-parallel-hash-join.md) §3.1) | **MET — M0128-P2.1 measured 2026-08-07.** Build time is 35–41 % of total for medium/large dims (customer 150K, orders 1.5M) in parallel hash joins. Follow-up: producer/consumer scan+filter parallelisation (§3.1) then genuinely concurrent build. Measurement: `analysis/m0128-p2.1-hash-build-measurement.md`. |
| Parallel `MultiHashJoin` | **Moot — `MultiHashJoin` was deleted 2026-08-07 (M0127-P6.2, commit `4e08d4b7`)**; the row is kept for history. Original reason: no PG counterpart, so no oracle plan; widens the plan-gate surface substantially ([07](07-parallel-hash-join.md) §6) | — |
