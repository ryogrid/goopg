...
| B-tree read path | **SAFE** | `internal/access/btree/btree.go:609-625` states the contract: readers take no tree-wide lock, synchronising through per-page `Slot.RLock()` latches with Lehman-Yao right-link recovery. `rightmostLeafBlk` is `atomic.Uint64` (`:661`) precisely so concurrent readers cannot tear. Designed in [`0002-0002-btree-concurrency.md`](../0002-0002-btree-concurrency.md). |
