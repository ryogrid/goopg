...
| 0003-0008 | [Subqueries (M0003)](0003-0008-subqueries.md) | draft | Scalar uncorrelated subqueries — `(SELECT …)` in expression position. parser.SubqueryExpr wraps the inner SelectStmt; planner.SubqueryExpr wraps the planned inner Node (catalog threaded through resolveContext.cat). Executor's evalSubquery builds, opens, drains, validates 1×1 cardinality, returns the cell (NULL on empty, 21000 on multi-row, 42601 on multi-column). IN/EXISTS/correlated deferred. |
