...
| M10 | `= ANY` / `<> ALL` forms | operator-ANY/ALL variants of M1/M2 | ANY/ALL forms follow the same NULL algebra; goopg maps them onto `InExpr` (`AnyOp`/`AllOp` fields). **LIVE BUG found by this row** [measured-at-HEAD a91d2a8d]: `<> ALL` unnests to a **semi** join instead of an anti join, returning the exact complement (`{1}` where PG returns `{2,3}`). Unnested path only; the SubPlan path is correct. Upstream never pulls up ALL sublinks at all (`pull_up_sublinks_qual_recurse` handles ANY and EXISTS only) — fix owned by Stage 4 |
...
| M12 | EXISTS with inner-only residual + LIMIT/DISTINCT/aggregate bodies | `EXISTS(SELECT … LIMIT 1)`, `EXISTS(SELECT DISTINCT …)`, `EXISTS(SELECT count(*) …)` | EXISTS over an aggregate body is **always true** (aggregate returns a row); LIMIT/DISTINCT are no-ops for EXISTS truth value. **TWO LIVE BUGS found by this row** [measured-at-HEAD a91d2a8d], both unnested-path-only: (a) a `LIMIT` inside the EXISTS body survives pull-up and becomes a **global** limit on the semi-join build side, so only one correlation key can ever match (`{1}` vs PG's `{1,3}`); (b) an ungrouped-aggregate body — a tautology — becomes a selective semi-join filter (`{3}` vs PG's `{1,2,3,4}`). Upstream guards both in `simplify_EXISTS_query` (`postgres/src/backend/optimizer/prep/prepjointree.c`), which is the fix template — Stage 4 |
