...
`fc76b20fd`) — verdict first, implementation scoped to what the verdict
leaves standing.** Base `c67051743` (B-01c slice (c)); the tip's two merge
commits touch no executor file, so every citation below holds within
~10 lines.
...
### 9.1 Source-falsification pass (`internal/`, at `fc76b20fd`)

Every falsifiable claim in §§0–6 was checked against the tree. All hold:

- **(A) run formation pre-exists: CONFIRMED.** `flushChunk`
  (`operators.go:1315`) calls `o.sortChunk(o.rows)` (`:1316`) before
  writing — a sorted run, not a whole-chunk dump. `initMerge` (`:1400`)
  opens one reader per run plus the keyvals-carrying tail source
  (`:1417`); `popMerge` (`:1432`) is pop-emit-advance-push over one
  `sortHeap` built with `less: o.lessKeyVals` (`:1403`). Comparator
  unification (M0134-0191) confirmed at the same site.
- **(B) threshold outlier: CONFIRMED.** `sortChunkBytes = 256 MiB` is a
  constant (`:898`); `sortOp` never reads `ctx.WorkMem` (no match in
  `operators.go`). Siblings do: `materializeOp`
  (`operators_material.go:83`), subquery cache (`subq_cache.go:23-24`),
  hash join (`join_batch.go`), TID bitmap (`tidbitmap.go:261`). Sort is
  the only outlier — the "planner/executor disagreement" stands, with
  `cost_funcs.go:306-327` pricing `cp.workMem` on one side and the
  executor ignoring it on the other.
- **Cost-model port: CONFIRMED.** `tuplesortMergeOrder(cp.workMem)` at
  `cost_funcs.go:320`, MINORDER 6 / MAXORDER 500 with the BLCKSZ-derived
  overheads at `:427-437`, all as §0's table claims.
- **§5 Close-hygiene defect: CONFIRMED, including latency.** `Close`
  (`:1343-1364`) clears `rows`, `ctids`, `idx`, `ctx`, `heap`,
  `spillFiles` — but not `keyvals`, `mergeReady`, `sortErr` or
  `peakBytes`. Reachability re-checked: the only Close+Open rescan is the
  correlated-subquery path (`expr.go:10887`), gated on
  `planIsIndexScanBased` (`expr.go:11066`), whose whitelist
  (IndexScan / keyed BitmapHeapScan / Project / Aggregate / Filter)
  admits no `Sort`. Latent, not live — fixed anyway per §6.2, three
  lines in the function E-01 owns.
- **`sort_external_test.go` gap (§4.2): CONFIRMED** — asserts spill-file
  count and row count, no ordering assertion, no DESC/NULLS-placement
  coverage.

No correction to the verdict or the landing order.

### 9.2 PG 18.3 oracle pass (`postgres/`, read-only)

- **Spill threshold is `work_mem`: CONFIRMED**
  (`tuplesort.c:695`: `state->allowedMem = Max(workMem, 64) * 1024`).
  (B)'s direction — executor reads the number the planner prices — is
  PG-faithful, not goopg-original.
- **Merge-order constants: CONFIRMED**
  (`tuplesort.c:176-179`: MINORDER 6, MAXORDER 500,
  TAPE_BUFFER_OVERHEAD BLCKSZ, MERGE_BUFFER_SIZE BLCKSZ*32;
  `tuplesort_merge_order(allowedMem)` at `:1778`). goopg's port
  transcribes them exactly; (B) extends the transcription to the
  threshold rather than inventing one.
- **Run formation by quicksort: CONFIRMED** (`tuplesort.c:26-35`:
  runs formed by exceeding workMem then `qsort()`; replacement
  selection is documented history). goopg's chunk-sort-then-write is
  the same shape — (A) needs no work on either side.
- **Single tape set: CONFIRMED** (`logtape.c`: one `LogicalTapeSet`
  multiplexed, block-level reuse). goopg's one-file-per-run is the
  honest residue §2.3 names; (C) stays ledgered behind (B).
- **Top-N bound is C-13a's: NO OBJECTION.** `tuplesort_set_bound`
  exists upstream and is correctly named here only to keep it out.
