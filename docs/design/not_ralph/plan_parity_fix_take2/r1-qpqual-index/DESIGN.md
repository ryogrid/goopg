...
| `addBaseRelBitmapPaths` / parameterised bitmap (`pathbitmap.go:160`, `:577`, via `costBitmapIndexScan` + `costbitmap.go:37`) | YES (`indexClauses` — the generation gate requires them) | ALL local conjuncts — NOT minus index-satisfied. PG's `cost_bitmap_heap_scan` charges "the full freight for all the scan clauses" (`costsize.c`: "assume they will be rechecked always" — lossy pages), i.e. `get_restriction_qual_cost`, not qpquals-minus-indexquals. The single-path site already holds `conjuncts`; the AND site and the estimators count `extractFilterConjuncts(rel.baseLeaf)` |
