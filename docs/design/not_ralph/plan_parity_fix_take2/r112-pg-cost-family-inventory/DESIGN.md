...
| HashAggregate / grouping | `costAgg`, grouping path producers | `cost_agg`, `hash_agg_set_limits`, `relation_byte_size` | Goopg's grouped aggregate has no spill arm. It is not licensed to invent a PG byte price without first establishing an executable analogue. |
