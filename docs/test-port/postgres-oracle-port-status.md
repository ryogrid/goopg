...
| RW-002 | `postgres/src/bin/pg_resetwal/t` | tap | defer | no | Deferred remainder of the pg_resetwal TAP suite: (a) the "database server was not shut down cleanly" + --force branch of 001_basic.pl — goopg v0 always shuts down via a graceful checkpoint and has no crash/unclean state to reproduce it; (b) the FINAL RESTART (001_basic.pl l.244-245) after the maximal SLRU-derived override — --next-transaction-id advances NextXID far past the bootstrap pg_xact segment and CLog.MarkUnknownAsAborted walks the whole xid range with a per-step fsync during initdb.Open, so startup looks hung; blocked on a PG-style StartupCLOG page-fill instead of the per-xid walk; and (c) 002_corrupted.pl. The pg_control read/write round-trip AND the SLRU-derived override round-trip (apply + spot-check) are now satisfied (see RW-003, loop #47). | `M0110-0004` |
