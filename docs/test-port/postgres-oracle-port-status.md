...
| RW-003 | `postgres/src/bin/pg_resetwal/t/001_basic.pl` | tap | port | yes | Ported as TestPort_PgResetwal001BasicServer in internal/testport/pgresetwal_port_test.go (M0110-0004): the pg_control read/write round-trip half of the server tier — PGDATA 0700/0600 permissions, pg_resetwal -n prints the checkpoint dump, pg_resetwal refuses while the server runs (postmaster.pid lock), pg_resetwal --pgdata succeeds after a CLEAN shutdown WITHOUT --force (enabled by wal.Checkpointer.CheckpointShutdown stamping pg_control State=DB_SHUTDOWNED via Runtime.Close), server SELECT 1 works after the reset, and a --next-oid 100000 control override is applied + spot-checked. The unclean-shutdown/--force pair and the SLRU-derived id overrides remain under RW-002. | `-` |
| RW-002 | `postgres/src/bin/pg_resetwal/t` | tap | defer | no | Deferred remainder of the pg_resetwal TAP suite: (a) the "database server was not shut down cleanly" + --force branch of 001_basic.pl — goopg v0 always shuts down via a graceful checkpoint and has no crash/unclean state to reproduce it; (b) the --commit-timestamp-ids/--multixact-ids/--multixact-offset/--oldest-transaction-id/--next-transaction-id overrides computed from real pg_commit_ts/pg_multixact/pg_xact SLRU segments; and (c) 002_corrupted.pl. Blocked on track_commit_timestamp SLRU segment emission + on-disk SLRU-segment-layout parity. The pg_control read/write round-trip prerequisite is now satisfied (see RW-003). | `M0110-0004` |
