...
gen-parser:
	...
	if [ "$$conflicts" -ne 14 ]; then \
		echo "ERROR: $$conflicts grammar conflicts (expected exactly 14 known: IF_P x7 = one per opt_if_exists_drop/opt_if_not_exists user, '(' x2 = func_call/extract vs paren, '[' x2 = subscript, ON = join, SESSION + LOCAL = optional SET scope). Keep this message in sync with the number above."; exit 1; fi; \
