...
gen-parser:
	...
	if [ "$$conflicts" -ne 22 ]; then \
		echo "ERROR: $$conflicts grammar conflicts (expected exactly 22 known: IF_P x9, SAVEPOINT = ROLLBACK TO [SAVEPOINT] name (SAVEPOINT is unreserved so it is also a valid ColId; shift wins, keyword form, matching PG) = one per opt_if_exists_drop/opt_if_not_exists user, '(' x5 = func_call/extract/TRIM vs paren, plus TIME/TIMESTAMP '(' prec ')' vs col_type_name's trailing typmod (shift wins; both spellings yield the same Args), '[' x2 = subscript, ON x2 = join ON, and DISTINCT.ON '(' vs reducing an empty opt_target_list (shift wins in both, so DISTINCT ON keeps binding), SESSION + LOCAL + CONSTRAINTS = keyword form vs empty set_scope after SET, all three shift, matching PG). Keep this message in sync with the number above."; exit 1; fi; \
